...
WindowsLinkPathsUTF8 get_windows_link_paths()
{
    ...
    WideCharToMultiByte(CP_UTF8,                           // UTF-8
    ...
    WideCharToMultiByte(CP_UTF8, 0, paths.windows_sdk_ucrt_library_path, ucrt_len, out.windows_sdk_ucrt_library_path,
                        MAX_PATH, NULL, NULL);
    int vsl_len = wcslen(paths.vs_library_path);
    WideCharToMultiByte(CP_UTF8, 0, paths.vs_library_path, vsl_len, out.vs_library_path, MAX_PATH, NULL, NULL);
...
DECLARE_INTERFACE_(IEnumSetupInstances, IUnknown)
{
    ...
    STDMETHOD(Next)
    (THIS_ _In_ ULONG celt, _Out_writes_to_(celt, *pceltFetched) ISetupInstance * *rgelt,
...
wchar_t *find_windows_kit_root_with_key(HKEY key, wchar_t *version)
{
    ...
    DWORD length =
        required_length + 2; // The +2 is for the maybe optional zero later on. Probably we are over-allocating.
    wchar_t *value = (wchar_t *)cmalloc(length);
    if (!value)
        return NULL;

    rc = RegQueryValueExW(key, version, NULL, NULL, (LPBYTE)value,
                          &required_length); // We know that version is zero-terminated...
    if (rc != 0)
        return NULL;
...
void find_windows_kit_root(Find_Result *result)
{
    ...
    LSTATUS rc = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows Kits\\Installed Roots", 0,
...
bool find_visual_studio_2017_by_fighting_through_microsoft_craziness(Find_Result *result)
{
    ...
    // NOTE(Kalinovcic): This is so stupid... These functions take references, so the code is different for C and
    // C++......
    // #ifdef __cplusplus
    ...
    while (1)
    {
        ...
        wchar_t *tools_filename =
            concat2(bstr_inst_path, L"\\VC\\Auxiliary\\Build\\Microsoft.VCToolsVersion.default.txt");
        ...
        size_t version_length =
            (size_t)(tools_file_size.QuadPart +
                     1); // Warning: This multiplication by 2 presumes there is no variable-length encoding in the
        ...
        wchar_t *library_file =
            concat2(library_path, L"\\vcruntime.lib"); // @Speed: Could have library_path point to this string, with a
                                                       // smaller count, to save on memory flailing!
        ...
        /*
           ...
           version and then reads the tools file to define the tools path - which is definitely better than what i did."
