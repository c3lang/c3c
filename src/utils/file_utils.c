...
bool filenamesplit(const char *path, char **filename_ptr, char **directory_ptr)
{
    ...
    if (file_len == 1 && path[0] == '.')
        return false;
    if (file_len == 2 && path[0] == '.' && path[1] == '.')
        return false;
    if (!file_len)
        return false;
...
const char *expand_path(const char *path)
{
    if (path[0] == '~' && path[1] == '/')
    {
        ...
        if (!home || asprintf(&ret, "%s%s", home, &path[1]) == -1)
            return &path[2];
        return ret;
    }
    return path;
...
const char *find_lib_dir(void)
{
    ...
    const char *lib_path;
    if ((lib_path = lib_find(path, "/../lib/")))
        return lib_path;
    if ((lib_path = lib_find(path, "/lib/")))
        return lib_path;
    if ((lib_path = lib_find(path, "/")))
        return lib_path;
    if ((lib_path = lib_find(path, "/../")))
        return lib_path;
    if ((lib_path = lib_find(path, "/../../lib/")))
...
void file_find_top_dir()
{
    while (1)
    {
        ...
        if (!err && S_ISREG(info.st_mode))
            return; // NOLINT(hicpp-signed-bitwise)
        ...
        if (strcmp(new_path, start_path) != 0)
            continue;
                             ...
                             const char *suffix2)
{
    ...
    while ((ent = readdir(dir)))
    {
        ...
        if (namelen == 0 || ent->d_name[0] == '.')
            continue;

        // Doesn't end with .c3
        if (namelen < 3 || (strncmp(&ent->d_name[namelen - len1], suffix1, len1) != 0 &&
                            strncmp(&ent->d_name[namelen - len2], suffix2, len2) != 0))
        {
            char *new_path = NULL;
