...
void cpio_push(Cpio *cpio, uint8_t *buffer, size_t len)
{
	...
	while ((avail = byte_buffer_available(cpio->buffer)) > 0)
	{
		switch ((enum State) cpio->state)
		{
			case IDLE:
				if (avail > sizeof(Header))
				{
					...
					const size_t mode = parse_oct_num(header->c_mode,
						sizeof(header->c_mode));
					cpio->file.size = parse_oct_num(header->c_filesize,
						sizeof(header->c_filesize));
					const size_t namlen = parse_oct_num(header->c_namesize,
					...
					{
						cpio->file.link = ccalloc(cpio->file.size + 1,
							sizeof(char));
					}
			...
			case NEED_NAME:
				...
				{
					byte_buffer_read(&cpio->buffer, (uint8_t*) cpio->file.name,
						cpio->file.name_size);
		...
		if (cpio->state == WRITING_FILE)
		{
			...
			while (cpio->to_read &&
				(avail = byte_buffer_available(cpio->buffer)) > 0)
			{
