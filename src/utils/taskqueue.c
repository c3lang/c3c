...
static void *taskqueue_thread(void *queue)
{
    ...
    while (1)
    {
        ...
        {
            was_active = false;
            if (--task_queue->active_threads == 0)
...
TaskQueueRef taskqueue_create(int threads)
{
    ...
    if (pthread_mutex_init(&queue->lock, NULL))
        error_exit("Failed to set up mutex");
    if (pthread_cond_init(&queue->notify, NULL))
        error_exit("Failed to set up cond");
    for (int i = 0; i < threads; i++)
    {
        if (pthread_create(&(queue->threads[i]), NULL, taskqueue_thread, queue))
            error_exit("Fail to set up thread pool");
    }
    return queue;
}
...
void taskqueue_add(TaskQueueRef queue_ref, Task *task)
{
    ...
    if (pthread_mutex_lock(&queue->lock) != 0)
        error_exit("Failed to lock task queue.");
    vec_add(queue->queue, task);
    if (pthread_cond_signal(&queue->notify) != 0)
        error_exit("Failed to signal tasks.");
    if (pthread_mutex_unlock(&queue->lock) != 0)
        error_exit("Failed to unlock task queue");
}
...
void taskqueue_destroy(TaskQueueRef queue_ref)
{
    ...
    queue->shutdown = true;
    if (pthread_cond_broadcast(&queue->notify))
        error_exit("Failed to signal tasks.");
    if (pthread_mutex_unlock(&queue->lock) != 0)
        error_exit("Failed to unlock task queue.");
    for (int i = 0; i < queue->thread_count; i++)
    {
        if (pthread_join(queue->threads[i], NULL) != 0)
