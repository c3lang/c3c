...
static int get_valid_string_setting(JSONObject *json, const char *key, const char *category, const char **values,
                                    int first_result, int count, const char *expected)
{
    JSONObject *value = json_obj_get(json, key);
    if (!value)
    {
...
static const char **get_valid_array(JSONObject *table, const char *key, const char *category, bool mandatory)
{
    ...
    if (!value)
    {
        if (mandatory)
        {
            error_exit("Error reading %s: %s was missing a mandatory '%s' field, please add it.", PROJECT_JSON,
                       category, key);
        }
        return NULL;
    }
    ...
    {
        error_exit("Error reading %s: %s had an invalid mandatory '%s' field that was not an array, please correct it.",
                   PROJECT_JSON, category, key);
    }
    ...
    for (unsigned i = 0; i < value->array_len; i++)
    {
        ...
        {
            error_exit("Error reading %s: %s had an invalid mandatory '%s' array that did not only hold strings, "
                       "please correct it.",
                       PROJECT_JSON, category, key);
        }
...
static void load_into_build_target(JSONObject *json, const char *type, BuildTarget *target)
{
    ...
    static const char *debug_infos[3] = {
        [DEBUG_INFO_FULL] = "full", [DEBUG_INFO_NONE] = "none", [DEBUG_INFO_LINE_TABLES] = "line-tables"};
    DebugInfo info =
        get_valid_string_setting(json, "debug-info", type, debug_infos, 0, 3, "one of 'full' 'line-table' or 'none'.");
    ...
    int reloc =
        get_valid_string_setting(json, "reloc", type, reloc_models, 0, 5, "'none', 'pic', 'PIC', 'pie' or 'PIE'.");
    int x86vec =
        get_valid_string_setting(json, "x86vec", type, vector_capability, 0, 5, "none, mmx, sse, avx or avx512");
    ...
    if (csource_dirs)
        target->csource_dirs = csource_dirs;
    if (version)
        target->version = version;
    if (langrev)
        target->langrev = langrev;
    if (source_dirs)
        target->source_dirs = source_dirs;
    if (libraries)
        target->libraries = libraries;
    if (info > -1)
        target->debug_info = info;
    if (cpu)
        target->cpu = cpu;
    if (reloc > -1)
        target->reloc_model = (RelocModel)reloc;
    if (x86vec > -1)
        target->feature.x86_vector_capability = x86vec;

    if (arch_os_string)
    {
        ArchOsTarget arch_os = arch_os_target_from_string(arch_os_string);
        if (arch_os == ARCH_OS_TARGET_DEFAULT)
            error_exit("Error reading %s: %s target was not valid.", PROJECT_JSON, type);
...
static void project_add_target(Project *project, BuildTarget *default_target, JSONObject *json, const char *name,
                               const char *type)
{
    ...
    *target = *default_target;
    ...
    VECEACH(project->targets, i)
    {
        ...
        {
            error_exit("More %s contained more than one target with the name %s. Please make all target names unique.",
                       PROJECT_JSON, target->name);
        }
    }
...
static void project_add_targets(Project *project, JSONObject *project_data)
{
    ...
    static const char *target_desc[4] = {[TARGET_TYPE_EXECUTABLE] = "Executable",
    ...
    for (unsigned i = 0; i < targets_json->member_len; i++)
    {
        ...
        int type = get_valid_string_setting(object, "type", "Target type", targets, 0, 4,
                                            "a target type like 'executable' or 'static-lib'");
        if (type < 0)
            error_exit("Target %s did not contain 'type' key.", key);
...
BuildTarget *project_select_target(Project *project, const char *optional_target)
{
    ...
    {
        error_exit("No targets could be found in %s. Please define at least one target, for example an 'executable' "
                   "and try again.",
                   PROJECT_JSON);
    }
    if (!optional_target)
    ...
    {
        BuildTarget *target = project->targets[i];
        if (strcmp(target->name, optional_target) == 0)
            return target;
    }
