...
static void usage(void)
{
    ...
    OUTPUT(
        "  --safe                - Set mode to 'safe', generating runtime traps on overflows and contract violations.");
...
static int parse_multi_option(const char *start, unsigned count, const char **elements)
{
    ...
    if (select < 0)
        error_exit("error: %.*s invalid option '%s' given.", (int)(start - arg), start, arg);
    return select;
}
...
static void parse_option(BuildOptions *options)
{
    ...
    case '-':
        ...
        if (match_longopt("symtab"))
        {
            ...
            if (symtab < 1024)
                OUTPUT("Expected a valid positive integer >= 1024.");
        ...
        if (match_longopt("threads"))
        {
            ...
            if (threads < 1)
                OUTPUT("Expected a valid integer 1 or higher.");
            if (threads > MAX_THREADS)
                OUTPUT("Cannot exceed %d threads.", MAX_THREADS);
        ...
        {
            if (at_end() || next_is_opt())
                error_exit("error: --lib needs a directory.");
            if (options->lib_count == MAX_LIB_DIRS)
                error_exit("Max %d libraries may be specified.", MAX_LIB_DIRS);
...
BuildOptions parse_arguments(int argc, const char *argv[])
{
    ...
    BuildOptions build_options = {.path = ".",
                                  .emit_llvm = false,
                                  .emit_bitcode = true,
    ...
    for (arg_index = 1; arg_index < arg_count; arg_index++)
    {
        ...
        if (build_options.command == COMMAND_COMPILE_RUN || build_options.command == COMMAND_COMPILE ||
            build_options.command == COMMAND_COMPILE_ONLY || build_options.command == COMMAND_GENERATE_HEADERS)
        {
