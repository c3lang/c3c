...
static inline bool parse_top_level_block(Context *context, Decl ***decls, _TokenType end1, _TokenType end2, _TokenType end3)
{
...
static inline Decl *parse_ct_case(Context *context)
{
	...
	while (1)
	{
		_TokenType type = context->tok.type;
...
static inline bool is_function_start(Context *context)
{
	...
	_TokenType tok = TOKTYPE(current);
	while (1)
	{
...
{
	_TokenType next_tok = context->next_tok.type;
	switch (context->tok.type)
	{
		case TYPELIKE_TOKENS:
...
bool parse_struct_body(Context *context, Decl *parent)
{
	...
	{
		_TokenType token_type = context->tok.type;
...
static inline Decl *parse_define_ident(Context  *context, Visibility visibility)
{
	...
	_TokenType alias_type = context->tok.type;
