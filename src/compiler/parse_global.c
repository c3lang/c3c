...
static inline bool parse_decl_initializer(ParseContext *c, Decl *decl, bool allow_void)
{
    if (try_consume(c, TOKEN_VOID))
    {
        ...
        {
            SEMA_ERROR_LAST(
                "'void' is not allowed here, it's only allowed for non constant global and local variables.");
            return false;
        }
...
static inline Decl *parse_ct_if_top_level(ParseContext *c)
{
    ...
    if (!parse_top_level_block(c, &ct->ct_if_decl.then, TOKEN_CT_ENDIF, TOKEN_CT_ELIF, TOKEN_CT_ELSE))
    ...
    while (tok_is(c, TOKEN_CT_ELIF))
    {
        ...
        if (!parse_top_level_block(c, &ct_elif->ct_elif_decl.then, TOKEN_CT_ENDIF, TOKEN_CT_ELIF, TOKEN_CT_ELSE))
            return poisoned_decl;
    ...
    if (tok_is(c, TOKEN_CT_ELSE))
    {
        ...
        if (!parse_top_level_block(c, &ct_else->ct_else_decl, TOKEN_CT_ENDIF, TOKEN_CT_ENDIF, TOKEN_CT_ENDIF))
            return poisoned_decl;
    }
...
static inline Decl *parse_ct_case(ParseContext *c)
{
    ...
    while (1)
    {
        ...
        if (type == TOKEN_CT_DEFAULT || type == TOKEN_CT_CASE || type == TOKEN_CT_ENDSWITCH)
            break;
...
bool parse_module(ParseContext *c)
{
    ...
    {
        SEMA_ERROR_HERE("'module' should be followed by a plain identifier, not a string. Did you accidentally put the "
                        "module name between \"\"?");
        return false;
    }
...
static bool consume_type_name(ParseContext *c, const char *type)
{
    ...
    if (!consume(c, TOKEN_TYPE_IDENT, "'%s' should be followed by the name of the %s.", type, type))
        return false;
    return true;
}
...
bool consume_const_name(ParseContext *c, const char *type)
{
    ...
    if (!consume(c, TOKEN_CONST_IDENT, "A constant name was expected here, did you forget it?"))
        return false;
    return true;
}
...
{
    *had_error = false;
    if (!tok_is(c, TOKEN_IDENT) || peek(c) != TOKEN_SCOPE)
        return NULL;
...
Decl *parse_decl_after_type(ParseContext *c, TypeInfo *type)
{
    ...
    if (tok_is(c, TOKEN_EQ))
    {
        ...
        if (!parse_decl_initializer(c, decl, true))
            return poisoned_decl;
    }
    return decl;
}
...
static Decl *parse_const_declaration(ParseContext *c, Visibility visibility)
{
    ...
    if (!consume_const_name(c, "const"))
        return poisoned_decl;

    CONSUME_OR_RET(TOKEN_EQ, poisoned_decl);

    if (!parse_decl_initializer(c, decl, false))
        return poisoned_decl;
 ...
 *  ;
 */
static inline bool parse_param_decl(ParseContext *c, Visibility parent_visibility, Decl ***parameters,
                                    bool require_name)
{
    ...
    {
        if (!parse_decl_initializer(c, param, false))
            return poisoned_decl;
    }
...
bool parse_parameters(ParseContext *c, Visibility visibility, Decl ***params_ref)
{
    ...
    while (!tok_is(c, TOKEN_EOS) && !tok_is(c, TOKEN_RPAREN))
    {
        ...
        if (!parse_attributes(c, &param->attributes))
            return false;
 ...
 */
static inline bool parse_parameter_list(ParseContext *c, Visibility parent_visibility, FunctionSignature *signature,
                                        bool is_interface)
{
    Decl **decls = NULL;
    CONSUME_OR_RET(TOKEN_LPAREN, false);
    if (!parse_parameters(c, parent_visibility, &decls))
        return false;
...
bool parse_struct_body(ParseContext *c, Decl *parent)
{
    ...
    while (!tok_is(c, TOKEN_RBRACE))
    {
        ...
        if (token_type == TOKEN_STRUCT || token_type == TOKEN_UNION || token_type == TOKEN_BITSTRUCT)
        {
            ...
            {
                TRY_CONSUME_OR_RET(TOKEN_COLON, "':' followed by bitstruct type (e.g. 'int') was expected here.",
                                   poisoned_decl);
                ASSIGN_TYPE_OR_RET(member->bitstruct.base_type, parse_type(c), poisoned_decl);
                if (!parse_bitstruct_body(c, member))
                    return decl_poison(parent);
            }
            else
            {
                if (!parse_attributes(c, &member->attributes))
                    return false;
                if (!parse_struct_body(c, member))
                    return decl_poison(parent);
            }
            ...
            {
                SEMA_ERROR(member, "Can't add another member: the count would exceed maximum of %d elements.",
                           MAX_MEMBERS);
                return false;
            }
            continue;
        ...
        while (1)
        {
            ...
            {
                SEMA_ERROR(member,
                           "'Inline' can only be applied to a single member, so please define it on its own line.");
                return false;
            }
        }
...
static inline Decl *parse_struct_declaration(ParseContext *c, Visibility visibility)
{
    ...
    if (!consume_type_name(c, type_name))
        return poisoned_decl;
...
static inline Decl *parse_bitstruct_declaration(ParseContext *c, Visibility visibility)
{
    ...
    if (!consume_type_name(c, "bitstruct"))
        return poisoned_decl;
...
static bool parse_macro_arguments(ParseContext *c, Visibility visibility, Decl ***params_ref, DeclId *body_param_ref)
{
    ...
    *body_param_ref = 0;
    ...
    if (!parse_parameters(c, visibility, params_ref))
        return false;
    ...
    if (try_consume(c, TOKEN_EOS))
    {
        ...
        if (!consume_ident(c, "variable name"))
            return false;
        if (try_consume(c, TOKEN_LPAREN))
        {
            if (!parse_parameters(c, visibility, &body_param->body_params))
                return false;
            CONSUME_OR_RET(TOKEN_RPAREN, false);
        }
...
static inline Decl *parse_define_ident(ParseContext *c, Visibility visibility)
{
    ...
    if (c->tok != alias_type)
    {
        ...
        {
            SEMA_ERROR(decl,
                       "A type alias must start with an upper case letter and contain at least one lower case letter.");
            return poisoned_decl;
        }
...
static inline Decl *parse_define_attribute(ParseContext *c, Visibility visibility)
{
    ...
    if (alias_type != TOKEN_TYPE_IDENT)
    {
        ...
        {
            SEMA_ERROR_HERE("A user defined attribute must start with an uppercase character, followed by at least one "
                            "lower case.");
            return false;
        }
    ...
    if (try_consume(c, TOKEN_LPAREN))
    {
        if (!parse_parameters(c, visibility, &parameters))
            return false;
    ...
    if (try_consume(c, TOKEN_EQ))
    {
        if (!parse_attributes(c, &attributes))
            return false;
    }
 ...
 */
static inline bool parse_func_macro_header(ParseContext *c, bool rtype_is_optional, TypeInfoId *rtype_ref,
                                           TypeInfoId *method_type_ref, const char **name_ref, SourceSpan *name_span)
{
    TypeInfo *rtype = NULL;
...
static inline Decl *parse_macro_declaration(ParseContext *c, Visibility visibility)
{
    ...
    if (kind == DECL_GENERIC)
        advance_and_verify(c, TOKEN_GENERIC);
    ...
    if (!parse_func_macro_header(c, true, rtype_ref, method_type_ref, &decl->name, &decl->span))
        return poisoned_decl;

    const char *block_parameter = NULL;
    if (!parse_macro_arguments(c, visibility, &decl->macro_decl.parameters, &decl->macro_decl.body_param))
        return poisoned_decl;
...
static inline Decl *parse_optenum_declaration(ParseContext *c, Visibility visibility)
{
    ...
    if (!consume_type_name(c, "optenum"))
        return poisoned_decl;
                                   ...
                                   Visibility parent_visibility)
{
    ...
    {
        if (!parse_param_decl(c, parent_visibility, parameters_ref, true))
            return false;
...
static inline Decl *parse_enum_declaration(ParseContext *c, Visibility visibility)
{
    ...
    if (!consume_type_name(c, "enum"))
        return poisoned_decl;

    TypeInfo *type = NULL;
    if (try_consume(c, TOKEN_COLON))
    {
        if (!parse_enum_spec(c, &type, &decl->enums.parameters, visibility))
            return poisoned_decl;
    }
    ...
    while (!try_consume(c, TOKEN_RBRACE))
    {
        ...
        {
            Expr **result = NULL;
            if (!parse_arg_list(c, &result, TOKEN_RPAREN, NULL))
                return poisoned_decl;
...
static inline Decl *parse_func_definition(ParseContext *c, Visibility visibility, AstId docs, bool is_interface)
{
    ...
    if (!parse_func_macro_header(c, false, rtype_id_ref, method_type_ref, &func->name, &func->span))
        return poisoned_decl;
    if (!parse_parameter_list(c, visibility, &(func->func_decl.function_signature), is_interface))
        return poisoned_decl;
    if (!parse_attributes(c, &func->attributes))
        return poisoned_decl;
...
static inline bool parse_import(ParseContext *c)
{
    ...
    while (1)
    {
        if (!tok_is(c, TOKEN_IDENT))
        {
            ...
            if (tok_is(c, TOKEN_STRING))
            {
                SEMA_ERROR_HERE("An import should be followed by a plain identifier, not a string. Did you "
                                "accidentally put the module name between \"\"?");
                return false;
            }
...
static inline bool parse_doc_contract(ParseContext *c, AstId **docs_ref, DocDirectiveKind kind)
{
    ...
    advance(c);
    ASSIGN_EXPR_OR_RET(ast->doc_stmt.contract.decl_exprs, parse_expression_list(c, kind == DOC_DIRECTIVE_CHECKED),
                       false);
    const char *end = start;
    while (*++end != '\n' && *end != '\0')
        end++;
    end--;
    if (end > c->data.lex_start)
        end = c->data.lex_start;
    while (end[-1] == ' ')
        end--;
    scratch_buffer_clear();
    switch (kind)
...
static inline bool parse_doc_param(ParseContext *c, AstId **docs_ref)
{
    ...
    if (try_consume(c, TOKEN_LBRACKET))
    {
        ...
        if (!consume_ident(c, "Expected 'in', 'inout' or 'out'"))
            return false;
        if (modifier == kw_in)
        {
            mod = PARAM_IN;
...
{
    *docs_ref = 0;
    if (!try_consume(c, TOKEN_DOCS_START))
        return true;
    ...
    while (1)
    {
        ...
        case TOKEN_DOCS_PARAM:
            if (!parse_doc_param(c, &last))
                return false;
            break;
        case TOKEN_DOCS_RETURN:
            advance(c);
            if (!consume(c, TOKEN_STRING, "Expected a string description."))
                return false;
            break;
        case TOKEN_DOCS_REQUIRE:
            if (!parse_doc_contract(c, &docs_ref, DOC_DIRECTIVE_REQUIRE))
                return false;
            break;
        case TOKEN_DOCS_CHECKED:
            if (!parse_doc_contract(c, &docs_ref, DOC_DIRECTIVE_CHECKED))
                return false;
            break;
        case TOKEN_DOCS_ENSURE:
            if (!parse_doc_contract(c, &docs_ref, DOC_DIRECTIVE_ENSURE))
                return false;
            break;
        case TOKEN_DOCS_OPTRETURN:
            if (!parse_doc_errors(c, &docs_ref))
                return false;
            break;
...
Decl *parse_top_level_statement(ParseContext *c)
{
    ...
    case TOKEN_CT_ASSERT:
        if (!check_no_visibility_before(c, visibility))
            return poisoned_decl;
        {
            ...
            if (docs)
            {
                SEMA_ERROR(astptr(docs),
                           "Unexpected doc comment before $assert, did you mean to use a regular comment?");
                return poisoned_decl;
            }
            return decl;
        }
