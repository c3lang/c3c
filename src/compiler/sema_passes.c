    ...
    VECEACH(module->units, index)
    {
        ...
        for (unsigned i = 0; i < imports; i++)
        {
            ...
            for (unsigned j = 0; j < i; j++)
            {
                ...
                {
                    SEMA_ERROR(import, "Module '%s' was imported more than once, please remove the duplicates.",
                               path->module);
...
static inline bool sema_analyse_top_level_switch(SemaContext *context, Decl *ct_switch)
{
    ...
    for (unsigned i = 0; i < case_count; i++)
    {
        ...
        if (expr)
        {
            if (is_type)
            {
                if (!sema_analyse_ct_expr(context, expr))
                    return false;
            ...
            for (unsigned j = 0; j < i; j++)
            {
                ...
                // Default.
                if (!other_case->ct_case_decl.expr)
                    continue;
                ExprConst *other_const = &other_case->ct_case_decl.expr->const_expr;
                ExprConst *other_const_to =
                    other_case->ct_case_decl.to_expr ? &other_case->ct_case_decl.to_expr->const_expr : other_const;
...
static inline bool analyse_func_body(SemaContext *context, Decl *decl)
{
    if (!decl->func_decl.body)
        return true;
    if (!sema_analyse_function_body(context, decl))
...
void sema_analysis_pass_functions(Module *module)
{
    ...
    VECEACH(module->units, index)
    {
        ...
        if (unit->main_function)
            analyse_func_body(&context, unit->main_function);
        sema_context_destroy(&context);
    }
