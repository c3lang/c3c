...
#define EXPECT_IDENT_FOR_OR(_name, _res)                                                                               \
    do                                                                                                                 \
    {                                                                                                                  \
        if (!expect_ident(c, _name))                                                                                   \
            return _res;                                                                                               \
    } while (0)
#define EXPECT_OR_RET(_tok, _res)                                                                                      \
    do                                                                                                                 \
    {                                                                                                                  \
        if (!expect(c, _tok))                                                                                          \
            return _res;                                                                                               \
    } while (0)
#define CONSUME_OR_RET(_tok, _res)                                                                                     \
    do                                                                                                                 \
    {                                                                                                                  \
        if (!expect(c, _tok))                                                                                          \
            return _res;                                                                                               \
...
#define CONSUME_EOS_OR_RET(_res)                                                                                       \
    do                                                                                                                 \
    {                                                                                                                  \
...
#define RETURN_AFTER_EOS(_ast)                                                                                         \
    RANGE_EXTEND_PREV(ast);                                                                                            \
    CONSUME_EOS_OR_RET(poisoned_ast);                                                                                  \
    return _ast
#define TRY_EXPECT_OR_RET(_tok, _message, _type)                                                                       \
    do                                                                                                                 \
    {                                                                                                                  \
        if (!tok_is(c, _tok))                                                                                          \
        {                                                                                                              \
            SEMA_ERROR_HERE(_message);                                                                                 \
            return _type;                                                                                              \
        }                                                                                                              \
    } while (0)
#define TRY_CONSUME_OR_RET(_tok, _message, _type)                                                                      \
    do                                                                                                                 \
    {                                                                                                                  \
        if (!consume(c, _tok, _message))                                                                               \
            return _type;                                                                                              \
    } while (0)
#define TRY_CONSUME(_tok, _message) TRY_CONSUME_OR_RET(_tok, _message, poisoned_ast)
#define CHECK_EXPR_OR_RET(_expr)                                                                                       \
    do                                                                                                                 \
    {                                                                                                                  \
        if (!expr_ok(_expr))                                                                                           \
            return _expr;                                                                                              \
...
{
    if (token_type >= TOKEN_VOID && token_type <= TOKEN_TYPEID)
        return true;
    if (token_type >= TOKEN_ALIAS && token_type <= TOKEN_WHILE)
        return true;
    return false;
}
