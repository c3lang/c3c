...
static inline bool sema_expr_setup_call_analysis(SemaContext *context, CalledDeclContext *
                                                 callee,
...
static inline bool sema_call_check_invalid_body_arguments(SemaContext *context, Expr *call, CalledDeclContext *callee);
static inline bool sema_call_evaluate_arguments(SemaContext *context, CalledDeclContext *callee, Expr *call, bool *optional, bool *no_match_ref);
...
static inline bool sema_expr_analyse_decl_element(SemaContext *context, DesignatorElement *element, Type *type, Decl **member_ref, ArraySize *index_ref, Type **return_type, unsigned i, SourceLocId loc, bool *is_missing);
...
Expr *sema_expr_analyse_ct_arg_index(SemaContext *context, SubscriptIndex *index)
{
	...
	{
		RETURN_VAL_SEMA_ERROR(poisoned_expr, index_expr, "Expected the argument index here, but found a value of type %s.", type_quoted_error_string(index_expr->type));
	}
	if (!sema_cast_const(index_expr))
	{
		RETURN_VAL_SEMA_ERROR(poisoned_expr, index_expr, "Vaarg functions need a constant argument, but this is a runtime value.");
	}
...
Expr *sema_resolve_string_ident(SemaContext *context, Expr *inner, bool report_missing)
{
	...
	switch (token)
	{
		...
		{
			if (path) goto NO_PATH;
			TypeInfo *info = type_info_new_base(type_from_token(token), inner->loc);
		...
		{
			TypeInfo *info = type_info_new(TYPE_INFO_IDENTIFIER, inner->loc);
...
{
	Expr *embedded_struct = expr_new(EXPR_ACCESS_RESOLVED, parent->loc);
...
static inline bool sema_expr_analyse_ternary(SemaContext *context, Type *infer_type, Expr *expr)
{
	...
	else
	{
		...
		else
		{
			...
			Decl *temp = decl_new_generated_var(cond->type, VARDECL_LOCAL, cond->loc);
...
static inline bool sema_expr_analyse_enum_constant(SemaContext *context, Expr *expr, const char *name, Decl *decl)
{
	...
	if (!sema_display_deprecated_warning_on_use(context, decl, expr->loc)) return expr_poison(expr), true;
...
static inline bool sema_expr_analyse_identifier(SemaContext *context, Type *to, Expr *expr, bool *failed_ref)
{
	...
	if (!decl)
	{
		...
		decl = sema_resolve_symbol(context, expr->unresolved_ident_expr.ident, expr->unresolved_ident_expr.path, expr->loc);
		(void)decl;
...
static inline bool sema_expr_resolve_ct_identifier(SemaContext *context, Expr *expr)
{
	...
	Decl *decl = sema_resolve_symbol(context, expr->ct_ident_expr.identifier, NULL, expr->loc);
	if (!decl)
	{
		return expr_poison(expr);
	}
...
INLINE bool sema_set_default_argument(SemaContext *context, CalledDeclContext *callee, Expr *call, Decl *param,
                                      bool *no_match_ref, Expr **expr_ref, bool *optional)
{
	...
	if (arg->resolve_status != RESOLVE_DONE)
	{
		...
		{
			inlined_at = (InliningSpan) { .loc = call->loc };
	...
	Expr *function_scope_arg = expr_new(EXPR_DEFAULT_ARG, arg->loc);
...
INLINE Expr **sema_splat_struct_insert(SemaContext *context, Expr **args, Expr *arg, Decl *strukt, ArrayIndex index)
{
	...
	Decl *temp = decl_new_generated_var(arg->type, VARDECL_LOCAL, arg->loc);
...
INLINE Expr **sema_splat_arraylike_insert(SemaContext *context, Expr **args, Expr *arg, ArraySize len, ArrayIndex index, Decl ***macro_va_decl_ref)
{
	...
	if (macro_va_decl_ref)
	{
		vec_add(*macro_va_decl_ref, temp);
		args[index] = subscript;
	}
	else
	{
		args[index] = two;
										 ...
										 bool *no_match_ref)
{
	...
	Expr *last_named_arg = INVALID_PTR; // NOLINT
	Expr *last = NULL;
...
SPLAT_NORMAL:;
	...
	for (int i = 0; i < func_param_count; i++)
	{
		...
		if (!has_named || !param->name)
		{
			...
			if (variadic != VARIADIC_NONE && i > vaarg_index)
			{
				print_error_after(sourcelocptr(last->loc), "Expected '%s: ...' after this argument.", param->name);
				RETURN_NOTE_FUNC_DEFINITION;
			}
													 ...
													 bool optional, const char *name, bool *no_match_ref)
{
	...
	CalledDeclContext callee = {
			.macro = false,
			.definition = decl,
			.call_location = expr->loc,
			.name = name,
	...
	FOREACH_IDX(i, Decl *, param, sig->params)
	{
		...
		if (!arg)
		{
			...
			else
			{
				...
				compound_init->expr_compound_literal.type_info = type_info_new_base(param->type, param->loc);
				if (!sema_analyse_expr_rvalue(context, compound_init)) goto END_CONTRACT;
				arg = compound_init;
				if (is_safe) expr->call_expr.va_is_splat = true;
			}
		}
		Decl *new_param = decl_new_generated_var(arg->type, VARDECL_LOCAL, expr->loc);
...
static inline bool sema_expr_setup_call_analysis(SemaContext *context, CalledDeclContext *callee,
                                                 ...
                                                 BlockExit **block_exit_ref, InliningSpan *span_ref)
{
	...
	if (span_ref)
	{
		*span_ref = (InliningSpan){ call_expr->loc, context->inlined_at };
	}
	else
	{
	...
	context_change_scope_with_flags(macro_context, SCOPE_MACRO, call_expr->loc);
	macro_context->block_return_defer = macro_context->active_scope.defer_last;

	return true;
}

static inline void sema_update_link_from_macro(SemaContext *context, ResolvedAttrData *attributes)
{
	ASSERT(attributes && attributes->links);
	if (context->call_env.kind != CALL_ENV_FUNCTION && context->call_env.kind != CALL_ENV_FUNCTION_STATIC) return;
	...
	{
		decl_create_resolved_attributes(func);
	}
	const char **updated = func->attrs_resolved->links;
	FOREACH(const char *, link, attributes->links)
	{
...
static inline bool sema_analyse_macro_body_params(SemaContext *context, Decl *decl, Expr *call_expr, Decl **body_params, bool *no_match_ref)
{
	...
	for (unsigned j = 0; j < expected_body_params; j++)
	{
		...
		// Check that the argument doesn't have default values
		...
		if (!type && expected_type_info) type = expected_type_info->type;

		// Check that types match, no conversion, just look at the canonical type.
		...
		// Assign the resulting type to the argument
		body_arg->type = type;
		...
		if (type)
		{
			switch (sema_resolve_storage_type(context, type))
			{
				case STORAGE_ERROR:
					return false;
				case STORAGE_NORMAL:
					if (!sema_set_alloca_alignment(context, type, &body_arg->alignment)) return false;
					break;
				default:
					break;
			}
	...
	// Handle failure in `$defined`
NO_MATCH_REF:
	*no_match_ref = true;
	return false;
}

static inline bool sema_analyse_macro_params(SemaContext *context, SemaContext *macro_context, Expr *call_expr, CalledDeclContext *callee, Decl **params, AstId **next_ref)
{
	FOREACH_IDX(idx, Decl *, param, params)
	{
		...
		if (!param)
		{
			if (callee->macro_va_decls)
			{
				Decl *decl_arr = decl_new(DECL_DECLARRAY, "", call_expr->loc);
				decl_arr->decls = callee->macro_va_decls;
				FOREACH(Decl *, d, callee->macro_va_decls)
				{
					sema_add_local(macro_context, d);
				}
				params[idx] = decl_arr;
			}
			continue;
		}
		if (!sema_add_local(macro_context, param)) return false;
		...
		if (param->var.init_expr)
		{
			...
			if (param_type && param->var.type_info && (param->var.out_param || param->var.not_null))
			{
				...
				{
					RETURN_SEMA_ERROR(param->var.init_expr, "Expected a pointer, slice or interface value for '%s'.", param->name);
				}
			}
			if (param->var.not_null && param_type && (param_type->type_kind == TYPE_POINTER || param_type->type_kind == TYPE_SLICE || param_type->type_kind == TYPE_INTERFACE || param_type->type_kind == TYPE_ANY))
			{
				...
				binary->binary_expr.right = exprid(expr_new_const_null(expr->loc, type_voidptr));
				binary->binary_expr.operator = BINARYOP_NE;
				if (!sema_analyse_expr_rhs(context, type_bool, binary, false, NULL, false)) return false;
				const char *string = callee->struct_var && idx == 0 ? "Called a method on a null value." : "Passed null to a ref ('&') parameter.";
				if (expr_is_const_bool(binary) && !binary->const_expr.b)
				{
					sema_error_at(context, param->var.init_expr->loc, string);
					return false;
				}

				Ast *assert = new_ast(AST_ASSERT_STMT, param->var.init_expr->loc);
				assert->assert_stmt.is_ensure = true;
				assert->assert_stmt.expr = exprid(binary);
				Expr *comment_expr = expr_new_const_string(expr->loc, string);
				assert->assert_stmt.message = exprid(comment_expr);
				ast_append(next_ref, assert);
			}
		}
	}
	return true;
								  ...
								  bool call_var_optional, bool *no_match_ref)
{
	...
	ResolvedAttrData *attributes = decl_get_resolved_attributes(decl);

	// Handle @link
	if (attributes && attributes->links) sema_update_link_from_macro(context, attributes);
	...
	sema_display_deprecated_warning_on_use(context, decl, call_expr->loc);
	...
	FOREACH_IDX(i, Decl *, param, params)
	{
		if (i == vararg_index)
		{
			...
			{
				if (!sema_analyse_expr_rvalue(context, args[i] = call_expr->call_expr.vasplat)) return false; // NOLINT
			}
			else
			{
				Expr **exprs = call_expr->call_expr.varargs;
				Expr *literal = expr_new(EXPR_COMPOUND_LITERAL, exprs ? exprs[0]->loc : param->loc);
	...
	if (!sema_analyse_macro_body_params(context, decl, call_expr, body_params, no_match_ref)) return false;

	DynamicScope old_scope = context->active_scope;
	// Create a scope, since the macro itself will not.
	context_change_scope_with_flags(context, SCOPE_NONE, call_expr->loc);
	...
	InliningSpan span;
	if (!sema_expr_setup_call_analysis(context, &callee, &macro_context, call_expr, rtype,
		macro_body ? macro_body->macro_body_expr.body : NULL, body_params, params, block_exit_ref, &span))
	{
		goto EXIT_FAIL;
	}
	...
	if (!sema_analyse_macro_params(context, &macro_context, call_expr, &callee, params, &next)) goto EXIT_FAIL;
...
static bool sema_call_analyse_body_expansion(SemaContext *macro_context, Expr *call, bool *no_match_ref)
{
	...
	for (unsigned i = 0; i < expressions; i++)
	{
		...
		if (has_optional_arg)
		{
			sema_error_at(macro_context, expr->loc, "Optional arguments are not permitted in a body invocation.");
			return false;
		}
		switch (param->var.kind)
		{
...
static inline bool sema_expr_resolve_subscript_index(SemaContext *context, Expr *expr, Expr *subscripted, Expr *index, Type **current_type_ref, Expr **current_expr_ref, Type **subscript_type_ref, Decl **overload_ref, int64_t *index_ref, bool is_ref, OperatorOverload overload_type, bool* missing_ref)
{
	...
	if (sema_cast_const(index) && expr_is_const_int(index) && (size = sema_len_from_expr(len_expr)) >= 0) // NOLINT
	{
...
static inline bool sema_expr_analyse_subscript(SemaContext *context, Expr *expr, bool *failed_ref)
{
	...
	if (overload)
	{
		if (start_from_end)
		{
			...
			bool addr = expr_may_ref(current_expr);
			if (addr)
			{
				expr_insert_addr(current_expr);
			}
			if (!sema_analyse_expr_rvalue(context, current_expr)) return false;
			Decl *temp = decl_new_generated_var(current_expr->type, VARDECL_PARAM, current_expr->loc);
			Expr *decl = expr_generate_decl(temp, expr_copy(current_expr));
			...
			if (addr)
			{
				if (!sema_expr_rewrite_insert_deref(context, current_expr)) return false;
			}
			Expr *var_for_len = expr_variable(temp);
			Expr *len_expr = expr_new(EXPR_CALL, expr->loc);
			if (!sema_insert_method_call(context, len_expr, len, var_for_len, NULL, false)) return false;
...
static inline bool sema_expr_analyse_range(SemaContext *context, Range *range, ArrayIndex len, RangeEnv env, FlatType *indexed_type)
{
	switch (range->status)
	{
		...
		case RESOLVE_RUNNING:
		{
			SourceLoc loc = *sourcelocptr(exprptr(range->start)->loc);
			if (range->end) loc = extend_loc_with_token(&loc, sourcelocptr(exprptr(range->end)->loc));
			sema_error_at(context, make_loc(loc), "Recursive definition of range.");
			range->status = RESOLVE_NOT_DONE;
			return false;
		}
		default:
 ...
 * 3. .@foo -> It is a macro.
 * 4. .$eval(...) -> resolve the eval and retry.
...
RETRY:
	...
	sema_error_at(context, loc, "Expected an identifier here.");
	return NULL;
ALREADY_RESOLVED:
	if (in_hash)
	{
		sema_error_at(context, loc, "An expression cannot already be resolved when used as '.foo'. "
	...
	sema_error_at(context, loc, "This expression was already resolved to an identifier before it was used.");
	return NULL;
}
...
static inline bool sema_expr_replace_with_enum_array(SemaContext *context, Expr *enum_array_expr, Decl *enum_decl)
{
	...
	enum_array_expr->expr_compound_literal.type_info = type_info_new_base(type_get_slice(kind), loc);
...
static inline bool sema_expr_replace_with_enum_name_array(SemaContext *context, Expr *enum_array_expr, Decl *enum_decl)
{
	...
	enum_array_expr->expr_kind = EXPR_COMPOUND_LITERAL;
	enum_array_expr->expr_compound_literal.initializer = initializer;
	enum_array_expr->expr_compound_literal.type_info = type_info_new_base(type_get_slice(type_string), loc);
	enum_array_expr->resolve_status = RESOLVE_NOT_DONE;
	return sema_analyse_expr_rvalue(context, enum_array_expr);
}
...
{
	 if (kw == type_property_list[TYPE_PROPERTY_HAS_TAG])
	 {
		 expr->expr_kind = EXPR_TYPECALL;
		 expr->type_call_expr = (ExprTypeCall) { .type = parent, .property = TYPE_PROPERTY_HAS_TAG };
		 return true;
	 }
	if (kw == type_property_list[TYPE_PROPERTY_GET_TAG])
	{
		expr->expr_kind = EXPR_TYPECALL;
		expr->type_call_expr = (ExprTypeCall) { .type = parent, .property = TYPE_PROPERTY_GET_TAG };
		return true;
	}
...
static inline bool sema_expr_analyse_type_access(SemaContext *context, Expr *expr, Type *parent_type, Expr *identifier, bool *missing_ref)
{
	...
	if (!type_may_have_sub_elements(canonical))
	{
		...
		if (!member)
		{
			...
			RETURN_SEMA_ERROR(expr, "'%s' does not have a member or method '%s'.", type_to_error_string(parent_type), name);
		}
...
static inline bool sema_expr_analyse_member_access(SemaContext *context, Expr *expr, Expr *parent, Expr *identifier, bool *missing_ref)
{
	...
	Decl *underlying_type_decl = decl->type->decl;
...
static inline bool sema_expr_analyse_reflection_cname(SemaContext *context UNUSED, Expr *expr, Decl *decl)
{
	switch (decl->decl_kind)
	{
		case DECL_VAR:
...
static inline void sema_expr_analyse_reflection_name(SemaContext *context UNUSED, Expr *expr, Decl *decl, bool is_qualified)
{
...
{
	switch (access->expr_kind)
	{
		case EXPR_IDENTIFIER:
		{
			AlignSize align_size = access->ident_expr->alignment;
			if (!align_size) return -1;
			if (alignment_ref) *alignment_ref = align_size;
			return 0;
		}
		case EXPR_ACCESS_RESOLVED:
		{
			ArrayIndex offset = sema_recursive_analyse_alignment_offset(access->access_resolved_expr.parent, alignment_ref);
			if (offset < 0) return -1;
			return offset + access->access_resolved_expr.ref->offset;
		}
		default:
			return 0;
	}
}
static inline BoolErr sema_expr_analyse_reflection_alignment(SemaContext *context, Expr *expr, Expr *reflect, Decl *member)
{
	if (member)
	{
	...
	if (sema_resolve_storage_type(context, type) != STORAGE_NORMAL) return BOOL_FALSE;
	AlignSize align;
	if (!sema_set_alignment(context, type, &align, false)) return BOOL_ERR;
	switch (reflect->expr_kind)
	{
		case EXPR_IDENTIFIER:
			align = reflect->ident_expr->alignment;
			break;
		case EXPR_ACCESS_RESOLVED:
		{
			ArrayIndex offset = sema_recursive_analyse_alignment_offset(reflect, &align);
			if (offset < 0) return BOOL_FALSE;
			align = type_min_alignment(align, offset);
			break;
		}
		default:
			break;
	}
	if (!align) return BOOL_FALSE;
	expr_rewrite_const_int(expr, type_sz, align);
	return BOOL_TRUE;
}

static inline bool sema_expr_analyse_reflection_offset(Expr *expr, Expr *reflect)
{
	...
	if (reflect->expr_kind != EXPR_ACCESS_RESOLVED) return false;
	ArrayIndex offset = sema_recursive_analyse_alignment_offset(reflect, NULL);
	if (offset < 0) return false;
	expr_rewrite_const_int(expr, type_sz, offset);
	return true;
}

INLINE bool sema_expr_rewrite_to_is_ordered(SemaContext *context, Expr *expr, Type *type)
{
...
bool type_is_ct_only(Type *type)
{
...
static bool sema_expr_analyse_reflection_access(SemaContext *context, Expr *expr, Expr *parent, Expr *identifier, bool *missing_ref)
{
	ASSERT_SPAN(expr, identifier->expr_kind == EXPR_UNRESOLVED_IDENTIFIER);

	Expr *reflect = parent->const_expr.reflection;
	const char *name = identifier->unresolved_ident_expr.ident;
	bool is_const = identifier->unresolved_ident_expr.is_const;
	if (is_const)
	{
		if (missing_ref) goto MISSING_REF;
		RETURN_SEMA_ERROR(expr, "There is no reflection property '%s'.", name);
	}
	...
	if (name == kw_alignment)
	{
		switch (sema_expr_analyse_reflection_alignment(context, expr, reflect, member))
		{
			...
			case BOOL_FALSE: goto FAILED;
		}
		UNREACHABLE
	}
	if (name == kw_type)
	{
		if (type_is_ct_only(type)) goto FAILED;
		expr_rewrite_const_typeid(expr, type);
		return true;
	}
	if (name == kw_offset)
	{
	...
	if (member)
	{
		...
		{
			expr->expr_kind = EXPR_TYPECALL;
			expr->type_call_expr = (ExprTypeCall) { .type = member, .property = TYPE_PROPERTY_GET_TAG };
			return true;
		}
		if (name == kw_has_tag)
		{
	...
	if (name == kw_qname) return sema_expr_analyse_reflection_name(context, expr, decl, true), true;
	if (name == kw_cname)
	{
...
static inline void sema_create_const_membersof(Expr *expr, Type *type, AlignSize alignment, AlignSize offset)
{
	...
	for (unsigned i = 0; i < count; i++)
	{
		...
		Expr *expr_element = expr_new(EXPR_CONST, expr->loc);
...
static inline Expr *create_method_copy(Decl *method, SourceLocId loc)
{
	size_t namestr_len = strlen(method->name);
	const char *namestr = str_copy(method->name, namestr_len);
	Expr *expr_element = expr_new(EXPR_CONST, loc);
	expr_element->resolve_status = RESOLVE_DONE;
...
static inline void append_to_method_list(Decl **methods, Expr ***method_exprs_ref, SourceLocId loc)
{
...
static inline void sema_append_interface_methods(Decl *interface, Expr ***method_exprs_ref, SourceLocId loc)
{
	append_to_method_list(interface->interface_methods, method_exprs_ref, loc);
	FOREACH(TypeInfo *, type_info, interface->interfaces)
	{
		sema_append_interface_methods(type_info->type->decl, method_exprs_ref, loc);
	}
}
static inline void append_extension_methods(Type *type, Decl **extensions, Expr ***method_exprs_ref, SourceLocId loc)
{
...
CONTINUE:
	if (type_is_user_defined(type))
	{
		...
		{
			sema_append_interface_methods(decl, &method_exprs, expr->loc);
		}
		// Look through natively defined methods.
		Methods *methods = decl->method_table;
		if (methods) append_to_method_list(methods->methods, &method_exprs, expr->loc);
	}
	else
	{
		append_extension_methods(type, compiler.context.method_extension_list, &method_exprs, expr->loc);
	}
											   ...
											   Type *parent_type)
{
	...
	switch (property)
	{
		...
		case TYPE_PROPERTY_HAS_EQUALS:
			return sema_expr_rewrite_to_has_equals(context, expr, type);
		case TYPE_PROPERTY_IS_SUBSTRUCT:
...
static bool sema_expr_analyse_type_property(SemaContext *context, Expr *expr, bool *missing_ref)
{
	Expr *parent = expr->type_property_expr.type;
	if (missing_ref) *missing_ref = false;
	if (!sema_analyse_expr(context, parent)) return false;
	Type *parent_type = parent->type_expr->type;
	const char *name = expr->type_property_expr.property;
	...
	switch (parent->expr_kind)
	{
		...
		case EXPR_CONST:
			switch (parent->const_expr.const_kind)
			{
				...
				case CONST_REFLECTION:
					return sema_expr_analyse_reflection_access(context, expr, parent, identifier, missing_ref);
				case CONST_MEMBER:
	...
	do
	{
		...
		for (ArrayIndex i = 0; i < (ArrayIndex)count; i++)
		{
			...
			if (kind == EXPR_SPLAT)
			{
				...
				new_args = sema_splat_arraylike_insert(context, exprs, inner, len, i, macro_va_decl_ref);
SPLAT_DONE:
				if (!new_args) return false;
				exprs = new_args;
				count = vec_size(exprs);
...
static bool sema_expr_fold_hash(SemaContext *context, Expr *expr)
{
	...
	while (expr->expr_kind == EXPR_HASH_IDENT)
	{
		...
		Decl *decl = sema_resolve_symbol(context, expr->hash_ident_expr.identifier, NULL, expr->loc);
...
static bool sema_expr_analyse_assign(SemaContext *context, Expr *expr, Expr *left, Expr *right, bool *failed_ref)
{
	...
	{
		Decl *temp = decl_new_generated_var(right->type, VARDECL_LOCAL, right->loc);
...
INLINE bool sema_rewrite_op_assign(SemaContext *context, Expr *expr, Expr *left, Expr *right, BinaryOp new_op)
{
	...
	Decl *variable = decl_new_generated_var(type_get_ptr(left->type), VARDECL_LOCAL, left->loc);
	...
	if (left->expr_kind == EXPR_SUBSCRIPT_ASSIGN)
	{
		...
		Decl *parent_val = decl_new_generated_var(parent->type, VARDECL_LOCAL, parent->loc);
		Expr **list = NULL;
...
static bool sema_expr_analyse_op_assign(SemaContext *context, Expr *expr, Expr *left, Expr *right, BinaryOp operator)
{
	...
	if (type_is_user_defined(canonical))
	{
		...
		bool reverse = false;
		Decl *candidate = expr_may_ref(left) ? sema_find_typed_operator(context, mapped_overload, expr->loc, left, right, &reverse) : NULL;
...
static bool sema_replace_with_overload(SemaContext *context, Expr *expr, Expr *left, Expr *right, Type *left_type, OperatorOverload* operator_overload_ref)
{
	...
	bool reverse;
	Decl *overload = sema_find_typed_operator(context, *operator_overload_ref, expr->loc, left, right, &reverse);
...
{
	Ast *ast = ast_new(AST_DECLARE_STMT, init->loc);
	Decl *var = decl_new_generated_var(init->type, VARDECL_LOCAL, init->loc);
...
static bool sema_rewrite_slice_comparison(SemaContext *context, Expr *expr, Expr *left, Expr *right, Type *max)
{
	...
	{
		Expr *len_left = expr_new(EXPR_SLICE_LEN, left->loc);
	...
	{
		cond_expr = expr_new_binary(default_loc, expr_variable(index), expr_variable(len_var_left), BINARYOP_LT);
	}
	ast->for_stmt.cond = exprid(expr_new_cond(cond_expr));
	Expr *update = expr_new(EXPR_UNARY, default_loc);
	...
	Expr *left_check = expr_new(EXPR_SUBSCRIPT, default_loc);
	...
	Expr *right_check = expr_new(EXPR_SUBSCRIPT, default_loc);
...
static bool sema_analyse_assign_mutate_overloaded_subscript(SemaContext *context, Expr *main, Expr *subscript_expr, Type *type)
{
	...
	Decl *result_val = decl_new_generated_var(result_type, VARDECL_LOCAL, main->loc);
	...
	Expr *get_expr = expr_new(EXPR_ACCESS_RESOLVED, increased->loc);
...
static inline bool sema_expr_analyse_decl_element(SemaContext *context, DesignatorElement *element, Type *type, Decl **member_ref, ArraySize *index_ref, Type **return_type, unsigned i, SourceLocId loc,
												  bool *is_missing)
{
...
static inline bool sema_expr_analyse_generic_ident(SemaContext *context, Expr *expr)
{
	...
	{
		RETURN_SEMA_ERROR(parent, "Generic resolution of this identifier has become deeply nested, it was aborted after reaching %d recursions.", compiler.generic_depth);
	}
...
static inline bool sema_expr_analyse_lambda(SemaContext *context, Type *target_type, Expr *expr)
{
	...
	if (!sig->rtype)
	{
		...
		sig->rtype = type_info_id_new_base(typeget(to_sig->rtype), expr->loc);
	}
	...
	FOREACH_IDX(i, Decl *, param, sig->params)
	{
		...
		if (!to_sig) goto FAIL_NO_INFER;
		param->var.type_info = type_info_id_new_base(to_sig->params[i]->type, param->loc);
	}
	...
	RETRY:
		switch (main_expr->expr_kind)
		{
			...
			case EXPR_TYPE_PROPERTY:
				if (!sema_expr_analyse_type_property(active_context, main_expr, &failed))
				{
					if (!failed) goto FAIL;
					success = false;
				}
			...
			case EXPR_HASH_IDENT:
			{
				if (unroll_hash)
				{
					Decl *decl = sema_resolve_symbol(active_context, main_expr->hash_ident_expr.identifier, NULL, main_expr->loc);
					if (!decl) goto FAIL;
					if (decl_is_defaulted_var(decl))
					{
						success = false;
			...
			case EXPR_BINARY:
				...
				break;
			case EXPR_CT_REFLECT:
				if (!sema_expr_analyse_ct_reflect(context, main_expr, &failed))
				{
					if (!failed) goto FAIL;
					success = false;
				}
...
static inline bool sema_expr_analyse_ct_stringify(SemaContext *context, Expr *expr)
{
	...
	while (true)
	{
		switch (inner->expr_kind)
		{
			...
			case EXPR_HASH_IDENT:
			{
				Decl *decl = sema_resolve_symbol(context, inner->ct_ident_expr.identifier, NULL, inner->loc);
	...
	const char *desc = span_to_string(inner->loc);
	if (!desc)
	{
...
{
	Expr *inner = expr->inner_expr;
	bool old_no_eval = context->call_env.in_no_eval;
	context->call_env.in_no_eval = true;
	bool success = sema_analyse_expr(context, inner);
	context->call_env.in_no_eval = old_no_eval;
	if (!success) return false;
	expr->const_expr = (ExprConst) { .reflection = inner, .const_kind = CONST_REFLECTION };
	expr->expr_kind = EXPR_CONST;
...
static inline bool sema_analyse_expr_dispatch(SemaContext *context, Expr *expr)
{
	switch (expr->expr_kind)
	{
		...
		case EXPR_VECTOR_TO_ARRAY:
			UNREACHABLE
		case EXPR_TYPE_PROPERTY:
			return sema_expr_analyse_type_property(context, expr, NULL);
		case EXPR_LENGTHOF:
						   ...
						   bool as_binary)
{
	...
	if (to && allow_optional && to_canonical != rhs_type_canonical && rhs_type_canonical == type_fault)
	{
		...
		{
			if (no_match_ref) goto NO_MATCH_REF;
			print_error_after(sourcelocptr(expr->loc), "You need to add a trailing '~' here to make this an optional.");
			return false;
		}
	}
...
bool sema_cast_const(Expr *expr)
{
	...
	switch (expr->expr_kind)
	{
		...
		case EXPR_SUBSCRIPT:
		{
			...
			if (!sema_cast_const(parent)) return false;
			if (flat_type->type_kind != TYPE_SLICE && !type_is_any_arraylike(flat_type) && flat_type->type_kind != TYPE_UNTYPEDLIST) return false;
...
TokenType sema_splitpathref(const char *string, ArraySize len, Path **path_ref, const char **ident_ref)
{
	...
	*ident_ref = NULL;
	...
	if (path_end > 0)
	{
		*path_ref = path_create_from_string(string, path_end, 0);
		string += path_end + 2;
		len -= path_end + 2;
	}
	while (len > 0)
...
{
	SourceLocId original_loc = method_call->loc;
