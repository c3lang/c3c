...
static bool binary_arithmetic_promotion(SemaContext *context, Expr *left, Expr *right, Type *left_type,
                                        Type *right_type, Expr *parent, const char *error_message);
...
static bool sema_decay_array_pointers(Expr *expr)
{
    ...
    if (!pointer_type || !type_is_arraylike(pointer_type))
        return true;
                         ...
                         bool report_missing)
{
    ...
    if (!interned_version)
    {
        if (report_missing)
        {
            if (*path_ref)
            {
                SEMA_ERROR(inner, "'%s::%.*s' could not be found, did you spell it right?", (*path_ref)->module,
                           (int)len, string);
            }
            else
            {
...
static inline bool both_any_integer_or_integer_vector(Expr *left, Expr *right)
{
    ...
    if (type_is_integer(flatten_left) && type_is_integer(flatten_right))
        return true;

    if (flatten_left->type_kind != TYPE_VECTOR || flatten_right->type_kind != TYPE_VECTOR)
        return false;
...
{
    if (eval_kind != CONSTANT_EVAL_ANY)
        return false;
...
bool expr_cast_is_constant_eval(Expr *expr, ConstantEvalKind eval_kind)
{
    ...
    case CAST_VECARR:
    case CAST_ARRVEC:
        if (eval_kind == CONSTANT_EVAL_FOLDABLE)
            return false;
...
static bool expr_binary_is_constant_eval(Expr *expr, ConstantEvalKind eval_kind)
{
    ...
    if (!expr_is_constant_eval(left, eval_kind))
        return false;
    if (!expr_is_constant_eval(right, eval_kind))
        return false;
    return true;
}
...
static inline bool expr_list_is_constant_eval(Expr **exprs, ConstantEvalKind eval_kind)
{
    VECEACH(exprs, i)
    {
        if (!expr_is_constant_eval(exprs[i], eval_kind))
            return false;
    }
    return true;
}
...
static bool sema_bit_assignment_check(Expr *right, Decl *member)
{
    ...
    if (!IS_CONST(right) || !type_is_integer(right->type))
        return true;

    unsigned bits = member->var.end_bit - member->var.start_bit + 1;

    // If we have enough bits to fit, then we're done.
    if (bits >= type_bit_size(right->type) || int_is_zero(right->const_expr.ixx))
        return true;
...
RETRY:
    ...
    case EXPR_RETHROW:
        return false;
    case EXPR_IDENTIFIER:
        if (expr->identifier_expr.decl->decl_kind != DECL_VAR)
            return true;
    ...
    case EXPR_TYPEOFANY:
        ...
        goto RETRY;
    case EXPR_SLICE:
        if (expr->slice_expr.start && !exprid_is_constant_eval(expr->slice_expr.start, CONSTANT_EVAL_FOLDABLE))
            return false;
        if (expr->slice_expr.end && !exprid_is_constant_eval(expr->slice_expr.end, CONSTANT_EVAL_FOLDABLE))
            return false;
        return exprid_is_constant_eval(expr->slice_expr.expr, eval_kind);
    case EXPR_SUBSCRIPT:
        if (!exprid_is_constant_eval(expr->subscript_expr.index, eval_kind))
            return false;
...
static inline bool sema_cast_ident_rvalue(SemaContext *context, Expr *expr)
{
    ...
    case VARDECL_CONST:
        ...
        if (type_is_abi_aggregate(decl->type))
            return true;
...
static inline bool sema_type_error_on_binop(Expr *expr)
{
    ...
    SEMA_ERROR(expr, "%s is not defined in the expression %s %s %s.", c,
               type_quoted_error_string(exprptr(expr->binary_expr.left)->type), c,
               type_quoted_error_string(exprptr(expr->binary_expr.right)->type));
    return false;
}
...
static inline bool sema_expr_analyse_ternary(SemaContext *context, Expr *expr)
{
    ...
    if (left)
    {
        if (!sema_analyse_cond_expr(context, cond))
            return expr_poison(expr);
        if (!sema_analyse_expr(context, left))
            return expr_poison(expr);
    ...
    {
        // Elvis
        if (!sema_analyse_expr(context, cond))
            return expr_poison(expr);
    ...
    if (!sema_analyse_expr(context, right))
        return expr_poison(expr);
    ...
    if (left_canonical != right_canonical)
    {
        ...
        if (!max)
        {
            SEMA_ERROR(expr, "Cannot find a common parent type of '%s' and '%s'", type_to_error_string(left_canonical),
                       type_to_error_string(right_canonical));
            return false;
        }
        Type *no_fail_max = type_no_fail(max);
        if (!cast_implicit(left, no_fail_max) || !cast_implicit(right, max))
            return false;
    }
...
{
    if (to->canonical->type_kind != TYPE_ENUM && to->canonical->type_kind != TYPE_ERRTYPE)
        return false;
...
static inline bool sema_expr_analyse_identifier(SemaContext *context, Type *to, Expr *expr)
{
    ...
    {
        if (find_possible_inferred_identifier(to, expr))
            return true;
    }
    ...
    if (decl->resolve_status != RESOLVE_DONE)
    {
        if (!sema_analyse_decl(context, decl))
    ...
    if (decl->decl_kind == DECL_VAR)
    {
        ...
        case VARDECL_CONST:
            if (!decl->type)
            {
                ...
                if (!sema_analyse_expr(context, copy))
                    return false;
...
static inline bool sema_expr_analyse_macro_expansion(SemaContext *context, Expr *expr)
{
    ...
    if (!sema_analyse_expr_lvalue(context, inner))
        return false;
    Decl *decl;
    switch (inner->expr_kind)
    {
...
RETRY:
    ...
    return true;
CONVERT_IF_BIGGER:
    if (type_size(to) <= type_size(from))
        return true;
CONVERT:
...
static inline bool sema_expr_analyse_binary_arithmetic_subexpr(SemaContext *context, Expr *expr, const char *error,
                                                               bool bool_is_allowed)
{
    Expr *left = exprptr(expr->binary_expr.left);
    Expr *right = exprptr(expr->binary_expr.right);

    // 1. Analyse both sides.
    if (!sema_expr_analyse_binary_subexpr(context, expr, left, right))
        return false;

    if (!sema_promote_binary_top_down(context, expr, left, right))
        return false;

    Type *left_type = type_no_fail(left->type)->canonical;
    Type *right_type = type_no_fail(right->type)->canonical;

    if (bool_is_allowed && left_type == type_bool && right_type == type_bool)
        return true;
...
static inline bool sema_expr_analyse_intrinsic_fp_invocation(SemaContext *context, Expr *expr, Decl *decl,
                                                             bool *failable)
{
...
static inline bool expr_may_unpack_as_vararg(Expr *expr, Type *variadic_base_type)
{
    ...
    case TYPE_POINTER:
        if (canonical->pointer->type_kind == TYPE_ARRAY)
            return canonical->pointer->array.base == base_type;
        return false;
    default:
        return false;
    }
...
static inline bool sema_expand_call_arguments(SemaContext *context, CalledDecl *callee, Expr *call, Expr **args,
                                              unsigned func_param_count, Variadic variadic, bool *failable)
{
    ...
    for (unsigned i = 0; i < num_args; i++)
    {
        ...
        if (arg->expr_kind == EXPR_DESIGNATOR)
        {
            ...
            {
                SEMA_ERROR(arg, "Vararg parameters may not be named parameters, use normal parameters instead.",
                           params[index]->name);
                return false;
            }
        ...
        if (i >= func_param_count)
        {
            ...
            if (variadic == VARIADIC_NONE)
            {
                ...
                SEMA_ERROR(
                    arg, "This argument would would exceed the number of parameters, did you add too many arguments?");
                return false;
            }
            ...
            if (call->call_expr.unsplat_last)
            {
                ...
                if (i < num_args - 1)
                {
                    SEMA_ERROR(arg, "This looks like a variable argument before an unpacked variable which isn't "
                                    "allowed. Did you add too many arguments?");
                    return false;
                }
            }
...
static inline bool sema_expr_analyse_call_invocation(SemaContext *context, Expr *call, CalledDecl callee,
                                                     bool *failable)
{
    // 1. Check body arguments.
    if (!sema_check_invalid_body_arguments(context, call, &callee))
        return false;
    ...
    if (callee.variadic == VARIADIC_TYPED || callee.variadic == VARIADIC_ANY)
    {
        ...
        Type *last_type =
            callee.macro ? callee.params[func_param_count - 1]->type : callee.param_types[func_param_count - 1];
    ...
    if (!sema_expand_call_arguments(context, &callee, call, args, func_param_count, callee.variadic, failable))
        return false;
    ...
    for (unsigned i = 0; i < num_args; i++)
    {
        ...
        if (i >= func_param_count)
        {
            ...
            if (variadic_type)
            {
                ...
                if (call->call_expr.unsplat_last)
                {
                    ...
                    if (!expr_may_unpack_as_vararg(arg, variadic_type))
                    {
                        SEMA_ERROR(arg, "It's not possible to unpack %s as vararg of type %s",
                                   type_quoted_error_string(arg->type), type_quoted_error_string(variadic_type));
                        return false;
                    }
                }
                else
                {
                    ...
                    if (!sema_analyse_expr_rhs(context, variadic_type, arg, true))
                        return false;
                }
            ...
            if (callee.variadic == VARIADIC_RAW)
            {
                ...
                if (!callee.macro)
                {
                    if (!expr_promote_vararg(context, arg))
                        return false;
                }
                // Set the argument at the location.
                *failable |= IS_FAILABLE(arg);
        ...
        case VARDECL_PARAM_REF:
            // &foo
            if (!sema_analyse_expr_lvalue(context, arg))
                return false;
            if (type && type->canonical != arg->type->canonical)
            {
                SEMA_ERROR(arg, "'%s' cannot be implicitly cast to '%s'.", type_to_error_string(arg->type),
        ...
        case VARDECL_PARAM:
            // foo
            if (!sema_analyse_expr_rhs(context, type, arg, true))
                return false;
            if (IS_FAILABLE(arg))
                *failable = true;
            if (param && !param->alignment)
        ...
        case VARDECL_PARAM_CT:
            ...
            if (!expr_is_constant_eval(arg, CONSTANT_EVAL_ANY))
            {
                SEMA_ERROR(
                    arg,
                    "A compile time parameter must always be a constant, did you mistake it for a normal paramter?");
                return false;
            }
            break;
        case VARDECL_PARAM_CT_TYPE:
                                                     ...
                                                     bool failable)
{
    ...
    if (!sema_expr_analyse_call_invocation(context, expr, callee, &failable))
        return false;
...
static inline bool sema_expr_analyse_var_call(SemaContext *context, Expr *expr, Type *func_ptr_type, bool failable)
{
    ...
    {
        SEMA_ERROR(expr, "Only macros, functions and function pointers maybe invoked, this is of type '%s'.",
                   type_to_error_string(func_ptr_type));
        return false;
    }
    expr->call_expr.is_pointer_call = true;
    return sema_expr_analyse_func_invocation(context, func_ptr_type->pointer->func.prototype, NULL, expr, NULL, NULL,
                                             failable);
}
...
static inline Type *unify_returns(SemaContext *context)
{
    ...
    VECEACH(context->returns, i)
    {
        ...
        if (!max)
        {
            SEMA_ERROR(return_stmt, "Cannot find a common parent type of %s and %s", rtype, common_type);
                                               ...
                                               bool failable)
{
    ...
    return sema_expr_analyse_func_invocation(context, decl->type->func.prototype, &decl->func_decl.function_signature,
                                             expr, decl, struct_var, failable);
}
...
static bool sema_check_expr_compile_time(SemaContext *context, Expr *expr)
{
    ...
    case EXPR_MACRO_BLOCK: {
        ...
        while (current)
        {
            if (!sema_check_stmt_compile_time(context, ast_next(&current)))
                return false;
        }
        return true;
    }
...
bool sema_expr_analyse_macro_call(SemaContext *context, Expr *call_expr, Expr *struct_var, Decl *decl, bool failable)
{
    ...
    CalledDecl callee = {.macro = true,
                         .block_parameter =
                             decl->macro_decl.body_param ? declptr(decl->macro_decl.body_param)->name : NULL,
                         .params = params,
                         .param_count = vec_size(params),
                         .struct_var = struct_var};

    if (!sema_expr_analyse_call_invocation(context, call_expr, callee, &failable))
        return false;
    ...
    for (unsigned i = 0; i < expected_body_params; i++)
    {
        ...
        if (!sema_resolve_type_info(context, body_arg->var.type_info))
            return false;
        ...
        if (body_param->var.type_info)
        {
            ...
            if (declare_type != body_arg->type)
            {
                SEMA_ERROR(body_arg->var.type_info, "This parameter should be '%s' but was '%s'",
                           type_to_error_string(declare_type), type_quoted_error_string(body_arg->type));
                return false;
            }
        }
        if (!body_arg->alignment)
            body_arg->alignment = type_alloca_alignment(body_arg->type);
    }
    ...
    macro_context.original_inline_line =
        context->original_inline_line ? context->original_inline_line : call_expr->span.row;

    VECEACH(params, i)
    {
        Decl *param = params[i];
        if (!sema_add_local(&macro_context, param))
            goto EXIT_FAIL;
    }
    ...
    while (current)
    {
        if (!sema_analyse_statement(&macro_context, ast_next(&current)))
            goto EXIT_FAIL;
    }
    ...
    else
    {
        if (!vec_size(macro_context.returns))
        {
            ...
            {
                SEMA_ERROR(decl, "Missing return in macro that should evaluate to %s.",
...
static inline Decl *sema_generate_generic_function(SemaContext *context, Expr *call_expr, Expr *struct_var, Decl *decl,
                                                   const char *mangled_name)
{
    TODO return NULL;
}

static inline bool sema_expr_analyse_generic_call(SemaContext *context, Expr *call_expr, Expr *struct_var, Decl *decl,
                                                  bool failable)
{
    ...
    for (unsigned i = 0; i < explicit_args; i++)
    {
        ...
        {
            if (!sema_analyse_expr_rhs(context, param->var.type_info->type, arg, true))
                return false;
        }
        else
        {
    ...
    {
        TODO if (generic_cache[i]->extname == mangled_name)
        {
...
static bool sema_analyse_body_expansion(SemaContext *macro_context, Expr *call)
{
    ...
    {
        Expr *expr = args[i];
        if (!sema_analyse_expr(macro_context, expr))
            return false;
    }
...
bool sema_expr_analyse_general_call(SemaContext *context, Expr *expr, Decl *decl, Expr *struct_var, bool is_macro,
                                    bool failable)
{
    expr->call_expr.is_type_method = struct_var != NULL;
    if (decl == NULL)
    {
        return sema_expr_analyse_var_call(
            context, expr, type_flatten_distinct_failable(exprptr(expr->call_expr.function)->type), failable);
    }
    switch (decl->decl_kind)
    {
    case DECL_MACRO:
...
static inline bool sema_expr_analyse_builtin_call(SemaContext *context, Expr *expr)
{
    ...
    {
        if (!sema_analyse_expr(context, args[i]))
            return false;
...
static inline bool sema_expr_analyse_call(SemaContext *context, Expr *expr)
{
    ...
    if (!sema_analyse_expr_lvalue(context, func_expr))
        return false;
                                      ...
                                      bool *remove_from_end)
{
    ...
    if (index_expr->expr_kind != EXPR_CONST)
        return true;
        ...
        if (!end_index && idx >= len)
        {
            ...
            SEMA_ERROR(index_expr,
                       is_vector ? "Index out of bounds, was %lld, exceeding max vector width %lld."
                                 : "Array index out of bounds, was %lld, exceeding max array index %lld.",
                       (long long)idx, (long long)len - 1);
            return false;
        }
        break;
    }
    case TYPE_SUBARRAY:
        ...
        {
            SEMA_ERROR(index_expr,
                       "Array index out of bounds, index from end (%lld) must be greater than zero or it will exceed "
                       "the max array index.",
                       (long long)idx);
            return false;
        }
    ...
    {
        SEMA_ERROR(index_expr,
                   "Array index out of bounds, using a negative array index is only allowed with pointers.");
        return false;
    }
    return true;
}
...
static inline ConstInitializer *initializer_for_index(ConstInitializer *initializer, uint32_t index)
{
    ...
    case CONST_INIT_ARRAY: {
        ...
        VECEACH(sub_values, i)
        {
            ...
            if (init->init_array_value.index == index)
                return init->init_array_value.element;
        }
        return NULL;
    }
...
static inline bool sema_expr_analyse_subscript(SemaContext *context, Expr *expr, bool is_addr)
{
    ...
    if (!sema_analyse_expr_lvalue(context, subscripted))
        return false;
    ...
    if (current_type == type_complist)
    {
        ...
        while (current_expr->expr_kind == EXPR_CT_IDENT)
        ...
        if (i < 0 || i >= size)
        {
            if (expr->subscript_expr.from_back)
            {
                SEMA_ERROR(
                    ...
                    (long long)(size - i), (long long)size);
            }
            else
            {
                SEMA_ERROR(index,
                           size > 1 ? "An index of '%lld' is out of range, a value between 0 and %lld was expected."
        ...
        if (!sema_cast_rvalue(context, indexed_expr))
            return false;
        expr_replace(expr, indexed_expr);
        return true;
    }

    if (!sema_cast_rvalue(context, current_expr))
        return false;
    ...
    if (!inner_type)
    {
        Decl *decl = NULL;
        if (is_addr)
            decl = sema_find_operator(context, current_expr, OVERLOAD_ELEMENT_REF);
        if (!decl)
        {
            ...
            {
                SEMA_ERROR(expr,
                           "A function or macro with '@operator(%s)' is not defined for %s, so you need && to take the "
    ...
    if (!expr_check_index_in_range(context, current_type, index, false, expr->subscript_expr.from_back,
                                   &remove_from_back))
        return false;
    if (remove_from_back)
        expr->subscript_expr.from_back = false;
    ...
    else
    {
        ...
        {
            if (sema_expr_index_const_list(current_expr, index, expr))
                return true;
        }
    }
...
static inline bool sema_expr_analyse_slice(SemaContext *context, Expr *expr)
{
    ...
    if (!sema_analyse_expr(context, subscripted))
        return false;
    bool failable = IS_FAILABLE(subscripted);
    ...
    if (!sema_analyse_expr(context, start))
        return false;
    if (end && !sema_analyse_expr(context, end))
        return false;

    // Fix index sizes
    if (!expr_cast_to_index(start))
        return false;
    if (end && !expr_cast_to_index(end))
        return false;
    ...
    if (!expr_check_index_in_range(context, type, start, false, expr->slice_expr.start_from_back, &remove_from_end))
        return false;
    if (remove_from_end)
        expr->slice_expr.start_from_back = false;
    remove_from_end = false;
    if (end && !expr_check_index_in_range(context, type, end, true, expr->slice_expr.end_from_back, &remove_from_end))
        return false;
    if (remove_from_end)
...
{
    if (!sema_analyse_expr(context, expr->inner_expr))
        return false;
    *expr = *expr->inner_expr;
    return true;
}
...
static inline bool sema_expr_analyse_type_access(SemaContext *context, Expr *expr, TypeInfo *parent, bool was_group,
                                                 bool is_macro, Expr *identifier)
{
...
static inline bool sema_expr_analyse_access(SemaContext *context, Expr *expr)
{
    ...
    if (!sema_analyse_expr_lvalue(context, parent))
        return false;
    ...
    if (is_pointer)
    {
        if (!sema_cast_rvalue(context, parent))
            return false;
...
static Type *sema_find_type_of_element(SemaContext *context, Type *type, DesignatorElement **elements,
                                       unsigned *curr_index, bool *is_constant, bool *did_report_error,
                                       MemberIndex *max_index, Decl **member_ptr)
{
    ...
    if (element->kind == DESIGNATOR_ARRAY || element->kind == DESIGNATOR_RANGE)
    {
        *member_ptr = NULL;
        ...
        {
            SEMA_ERROR(element->index_expr, "The index may must be less than the array length (which was %llu).",
        ...
        if (element->kind == DESIGNATOR_RANGE)
        {
            ...
            if (max_index && *max_index < end_index)
                *max_index = end_index;
        }
        return base;
    }
...
static Type *sema_expr_analyse_designator(SemaContext *context, Type *current, Expr *expr, MemberIndex *max_index,
                                          Decl **member_ptr)
{
    ...
    *member_ptr = NULL;
    for (unsigned i = 0; i < vec_size(path); i++)
    {
        ...
        Type *new_current = sema_find_type_of_element(context, current, path, &i, &is_constant, &did_report_error,
                                                      i == 0 ? max_index : NULL, &member_found);
        if (!new_current)
        {
            if (!did_report_error)
...
static void sema_update_const_initializer_with_designator(ConstInitializer *const_init, DesignatorElement **curr,
                                                          DesignatorElement **end, Expr *value);
...
static inline void sema_update_const_initializer_with_designator(ConstInitializer *const_init, DesignatorElement **curr,
                                                                 DesignatorElement **end, Expr *value)
{
...
static bool sema_expr_analyse_designated_initializer(SemaContext *context, Type *assigned, Expr *initializer)
{
    ...
    VECEACH(init_expressions, i)
    {
        ...
        if (!result)
            return false;
        Expr *value = expr->designator_expr.value;
        if (!sema_analyse_expr_rhs(context, result, value, true))
            return false;
        if (member && member->decl_kind == DECL_VAR && member->var.kind == VARDECL_BITMEMBER)
        {
            if (!sema_bit_assignment_check(value, member))
                return false;
        }
...
static inline void not_enough_elements(Expr *initializer, int element)
{
    ...
    SEMA_ERROR(initializer->initializer_list[element - 1],
               "Too few elements in initializer, there should be elements after this one.");
}
/**
...
static inline bool sema_expr_analyse_struct_plain_initializer(SemaContext *context, Decl *assigned, Expr *initializer)
{
    ...
    if (assigned->decl_kind == DECL_UNION)
        expected_members = 1;
    ...
    for (MemberIndex i = 0; i < max_loop; i++)
    {
        ...
        if (!sema_analyse_expr_rhs(context, members[i]->type, element, true))
            return false;
        if (member->decl_kind == DECL_VAR && member->var.kind == VARDECL_BITMEMBER)
        {
            if (!sema_bit_assignment_check(element, members[i]))
                return false;
        }
    ...
    if (failable)
        initializer->type = type_get_failable(initializer->type);
...
static inline bool sema_expr_analyse_array_plain_initializer(SemaContext *context, Type *assigned, Expr *initializer)
{
    ...
    if (assigned->type_kind != TYPE_ARRAY && assigned->type_kind != TYPE_VECTOR)
        expected_members = size;
    ...
    VECEACH(elements, i)
    {
        ...
        if (!sema_analyse_expr_rhs(context, inner_type, element, true))
            return false;
        failable = failable || IS_FAILABLE(element);
    }
    assert(initializer->type);
...
static inline bool sema_expr_analyse_untyped_initializer(SemaContext *context, Expr *initializer)
{
    ...
    {
        Expr *element = elements[i];
        if (!sema_analyse_expr(context, element))
            return false;
        if (is_const && element->expr_kind != EXPR_CONST)
            is_const = false;
        if (!failable_expr && IS_FAILABLE(element))
            failable_expr = element;
...
static inline bool sema_expr_analyse_initializer(SemaContext *context, Type *external_type, Type *assigned, Expr *expr)
{
    ...
    if (assigned->type_kind == TYPE_UNTYPED_LIST || assigned->type_kind == TYPE_ARRAY ||
        assigned->type_kind == TYPE_INFERRED_ARRAY || assigned->type_kind == TYPE_SUBARRAY ||
...
static inline bool sema_expr_analyse_initializer_list(SemaContext *context, Type *to, Expr *expr)
{
    ...
    case TYPE_SUBARRAY: {
        ...
        if (!sema_expr_analyse_initializer(context, type, type, expr))
            return false;
        expr->resolve_status = RESOLVE_DONE;
    ...
    case TYPE_POINTER:
        SEMA_ERROR(expr, "Pointers cannot be initialized using an initializer list, instead you need to take the "
                         "address of an array.");
        return false;
    default:
        break;
    ...
    SEMA_ERROR(expr, "'%s' cannot use compound literal initialization, did you intend to use a cast?",
               type_to_error_string(to));
    return false;
}
...
static inline bool sema_expr_analyse_cast(SemaContext *context, Expr *expr)
{
    ...
    if (!sema_analyse_expr(context, inner) || !success)
        return false;
...
static inline bool sema_expr_analyse_slice_assign(SemaContext *context, Expr *expr, Type *left_type, Expr *right,
                                                  bool is_unwrapped)
{
    // 1. Evaluate right side to required type.
    if (!sema_analyse_expr_rhs(context, left_type->array.base, right, false))
        return false;
                                         ...
                                         bool is_unwrapped)
{
    ...
    // 1. Evaluate right side to required type.
    if (!sema_analyse_expr_rhs(context, left_type, right, true))
        return false;
    if (IS_FAILABLE(right) && !type_is_failable(left_type))
    {
        if (is_unwrapped)
        {
            SEMA_ERROR(exprptr(expr->binary_expr.left),
                       "The variable is unwrapped in this context, if you don't want to unwrap it, use () around the "
                       "variable to suppress unwrapping, like 'catch err = (x)' and 'try (x)'.");
            return false;
        }
        if (!left_type)
            left_type = type_no_fail(right->type);
...
{
    if (!sema_expr_begin_analyse(expr))
        return expr_ok(expr);
    ...
    if (!decl)
    {
        SEMA_ERROR(expr, "The compile time variable '%s' was not defined in this scope.",
...
{
    if (!sema_expr_analyse_ct_identifier_lvalue(context, left))
        return false;

    // 3. Evaluate right side to required type.
    if (!sema_expr_analyse_assign_right_side(context, expr, left->type, right, false))
        return false;
...
static bool sema_expr_analyse_ct_type_identifier_assign(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (!sema_analyse_expr_lvalue(context, right))
        return false;
...
static bool sema_expr_analyse_assign(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (!sema_analyse_expr_lvalue(context, left))
        return false;

    // 2. Check assignability
    if (!sema_expr_check_assign(context, left))
        return false;

    bool is_unwrapped_var = expr_is_unwrapped_ident(left);

    // 3. Evaluate right side to required type.
    if (!sema_expr_analyse_assign_right_side(context, expr, left->type, right, is_unwrapped_var))
        return false;
    ...
    {
        if (!sema_bit_assignment_check(right, left->access_expr.ref))
            return false;
...
static bool sema_expr_analyse_ct_common_assign(SemaContext *context, Expr *expr, Expr *left)
{
    ...
    if (!sema_expr_analyse_binary(context, expr))
        return false;
...
static bool sema_expr_analyse_common_assign(SemaContext *context, Expr *expr, Expr *left, Expr *right, bool int_only)
{
    ...
    if (!sema_analyse_expr(context, right))
        return false;
    if (!cast_implicit(right, no_fail))
        return false;
...
static bool sema_expr_analyse_add_sub_assign(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (left_type_canonical->type_kind == TYPE_POINTER)
    {
        ...
        if (!sema_decay_array_pointers(left))
            return false;
        expr->type = left->type;
        ...
        {
            SEMA_ERROR(right,
                       "The right side was '%s' but only integers are valid on the right side of %s when the left side "
                       "is a pointer.",
    ...
    if (!cast_implicit(right, left->type))
        return false;
...
static Type *numeric_arithmetic_promotion(Type *type)
{
    ...
    case ALL_SIGNED_INTS:
        if (type->builtin.bitsize < platform_target.width_c_int)
            return type_cint;
        return type;
...
{
    if (*left_type_ref == *right_type_ref)
        return;
...
static Type *defer_iptr_cast(Expr *maybe_pointer, Expr *maybe_diff)
{
    ...
    // (iptr)((char*)(ptr) +- 1)
    if (maybe_pointer->expr_kind == EXPR_CAST && maybe_pointer->cast_expr.kind == CAST_PTRXI &&
...
static bool sema_expr_analyse_sub(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (left_type->type_kind == TYPE_POINTER)
    {
        ...
        {
            if (!sema_decay_array_pointers(right))
                return false;
            ...
            {
                SEMA_ERROR(expr,
                           "'%s' - '%s' is not allowed. Subtracting pointers of different types from each other is not "
                           "possible.",
        ...
        {
            SEMA_ERROR(expr, "Cannot subtract '%s' from '%s'", type_to_error_string(right_type),
        ...
        {
            SEMA_ERROR(expr, "Cannot subtract a '%s' from a pointer, please first cast it to '%s'.",
                       type_to_error_string(right_type), type_to_error_string(type_iptrdiff));
            return false;
        }

        // 6. Convert to iptrdiff
        if (!cast_implicit(right, type_iptrdiff))
            return true;
    ...
    if (!binary_arithmetic_promotion(context, left, right, left_type, right_type, expr,
...
static bool sema_expr_analyse_add(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    Type *cast_to_iptr = defer_iptr_cast(left, right);
    if (!cast_to_iptr)
        cast_to_iptr = defer_iptr_cast(right, left);
    ...
    if (left_type->type_kind == TYPE_POINTER)
    {
        ...
        if (!type_is_integer(right_type))
        {
            SEMA_ERROR(right, "A value of type '%s' cannot be added to '%s', an integer was expected here.",
                       type_to_error_string(right->type), type_to_error_string(left->type));
            return false;
        }
...
static bool sema_expr_analyse_mult(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (!sema_expr_analyse_binary_arithmetic_subexpr(context, expr, "It is not possible to multiply %s by %s.", false))
        return false;
...
static bool sema_expr_analyse_div(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (!sema_expr_analyse_binary_arithmetic_subexpr(context, expr, "Cannot divide %s by %s.", false))
        return false;
...
static bool sema_expr_analyse_mod(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (!sema_expr_analyse_binary_arithmetic_subexpr(context, expr, NULL, false))
        return false;
...
static bool sema_expr_analyse_bit(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (!sema_expr_analyse_binary_arithmetic_subexpr(context, expr, NULL, true))
        return false;
...
static bool sema_expr_analyse_shift(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (expr->binary_expr.widen && !sema_widen_top_down(left, expr->type))
        return false;

    // 3. Promote lhs using the usual numeric promotion.
    if (!cast_implicit(left, numeric_arithmetic_promotion(type_no_fail(left->type))))
        return false;
...
static bool sema_expr_analyse_and_or(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    if (!sema_expr_analyse_binary_subexpr(context, expr, left, right))
        return false;
    if (!cast_implicit(left, type_bool) || !cast_implicit(right, type_bool))
        return false;
...
static void cast_to_max_bit_size(SemaContext *context, Expr *left, Expr *right, Type *left_type, Type *right_type)
{
    ...
    if (bit_size_left < bit_size_right)
    {
        Type *to = left->type->type_kind < TYPE_U8 ? type_int_signed_by_bitsize(bit_size_right)
                                                   : type_int_unsigned_by_bitsize(bit_size_right);
        bool success = cast_implicit(left, to);
        assert(success);
        return;
    }
    Type *to = right->type->type_kind < TYPE_U8 ? type_int_signed_by_bitsize(bit_size_left)
...
static bool sema_is_unsigned_always_false_comparison(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    if (context->active_scope.flags & SCOPE_MACRO)
        return true;
    if (!expr_is_const(left) && !expr_is_const(right))
        return true;
    ...
    if (expr_is_const(left) && type_is_unsigned(type_flatten_distinct(right->type)))
    {
        ...
        if (!int_is_zero(left->const_expr.ixx))
            return true;
        switch (expr->binary_expr.operator)
        {
        case BINARYOP_GT:
            SEMA_ERROR(left, "Comparing '0 > unsigned expression' can never be true, and is only allowed inside of "
                             "macro expansions.");
            return false;
        case BINARYOP_GE:
            SEMA_ERROR(left, "Comparing '0 >= unsigned expression' is the same as 0 == expr and is a common bug, "
    ...
    if (!expr_is_const(right) || !type_is_unsigned(type_flatten_distinct(left->type)))
        return true;
    if (int_is_neg(right->const_expr.ixx))
    {
    ...
    case BINARYOP_LT:
        ...
        return false;
    case BINARYOP_LE:
        SEMA_ERROR(right, "Comparing 'unsigned expression <= 0' is the same as expr == 0 and is a common bug, "
                          "so is only allowed inside of macro expansions.");
        return false;
    default:
        return true;
...
static bool sema_expr_analyse_comp(SemaContext *context, Expr *expr, Expr *left, Expr *right)
{
    ...
    if (left_type->type_kind == TYPE_VECTOR && right_type->type_kind == TYPE_VECTOR)
    {
        if (left_type->array.len == right_type->array.len)
        {
            ...
            if (left_vec == right_vec)
                goto DONE;
            if (type_size(left_vec) != type_size(right_vec))
                goto DONE;
            if (type_is_integer(left_vec) && type_is_integer(right_vec))
                goto DONE;
        }
        SEMA_ERROR(expr, "Vector types %s and %s cannot be compared.", type_quoted_error_string(left->type),
    ...
    if (!max)
    {
        SEMA_ERROR(expr, "%s and %s are different types and cannot be compared.", type_quoted_error_string(left->type),
                   type_quoted_error_string(right->type));
        return false;
    }
    ...
    if (!is_equality_type_op)
    {
        ...
        {
            SEMA_ERROR(expr,
                       "%s can only be compared using '!=' and '==' it cannot be ordered, did you make a mistake?",
                       type_quoted_error_string(left->type));
            return false;
        }
...
DONE:
    ...
    {
        if (!sema_is_unsigned_always_false_comparison(context, expr, left, right))
            return false;
    }
...
static bool sema_expr_analyse_deref(SemaContext *context, Expr *expr)
{
    ...
    Expr *inner = expr->unary_expr.expr;
    if (!sema_analyse_expr(context, inner))
        return false;
    ...
    {
        SEMA_ERROR(inner, "Cannot dereference a value of type %s, it must be a pointer.",
...
static inline bool sema_take_addr_of_var(Expr *expr, Decl *decl)
{
    ...
    case VARDECL_CONST:
        ...
        {
            SEMA_ERROR(expr,
                       "The constant is not typed, either type it or use && to take the reference to a temporary.");
        ...
        return true;
    case VARDECL_PARAM_EXPR:
        SEMA_ERROR(expr, "It is not possible to take the address of a captured expression, but you can use && to take "
                         "a reference to the temporary value.");
        return false;
    case VARDECL_PARAM_CT:
    case VARDECL_PARAM_CT_TYPE:
    case VARDECL_LOCAL_CT_TYPE:
...
static bool sema_take_addr_of(Expr *inner)
{
    ...
    case EXPR_UNARY:
        if (inner->unary_expr.operator== UNARYOP_DEREF)
            return true;
        break;
    case EXPR_ACCESS:
    ...
    SEMA_ERROR(inner, "To take the address of a temporary value, use '&&' instead of '&'.",
               type_to_error_string(inner->type));
    return false;
}
...
static bool sema_expr_analyse_neg(SemaContext *context, Expr *expr)
{
    ...
    if (!sema_analyse_expr(context, inner))
        return false;
    if (expr->unary_expr.widen && !sema_widen_top_down(inner, expr->type))
        return false;
    ...
    if (!cast_implicit(inner, result_type))
        return false;
...
static bool sema_expr_analyse_bit_not(SemaContext *context, Expr *expr)
{
    ...
    if (!type_is_integer_or_bool_kind(type_flatten_distinct(canonical)))
    {
        ...
        if (vector_type && (type_is_integer(vector_type) || vector_type == type_bool))
            goto VALID_VEC;
...
static bool sema_expr_analyse_not(SemaContext *context, Expr *expr)
{
    ...
    {
        SEMA_ERROR(expr, "The use of '!' on %s is not allowed as it can't be converted to a boolean value.",
...
static inline bool sema_expr_analyse_ct_incdec(SemaContext *context, Expr *expr, Expr *inner)
{
    ...
    if (!sema_expr_analyse_ct_identifier_lvalue(context, inner))
        return false;
...
static inline bool sema_expr_analyse_incdec(SemaContext *context, Expr *expr)
{
    ...
    if (!sema_expr_check_assign(context, inner))
        return false;
    ...
    if (!sema_decay_array_pointers(inner))
        return false;
...
static inline bool sema_expr_analyse_or_error(SemaContext *context, Expr *expr)
{
    ...
    if (expr->or_error_expr.widen && !sema_widen_top_down(inner, expr->type))
        return false;
    ...
    if (expr->or_error_expr.is_jump)
    {
        if (!sema_analyse_statement(context, expr->or_error_expr.or_error_stmt))
            return false;
        expr->type = inner->type;
        return true;
    }
    ...
    if (!sema_analyse_expr(context, else_expr))
        return false;
    if (expr->or_error_expr.widen && !sema_widen_top_down(else_expr, expr->type))
        return false;
    ...
    if (!cast_implicit(inner, common))
        return false;
    if (!cast_implicit(else_expr, common))
        return false;
...
static inline bool sema_expr_analyse_rethrow(SemaContext *context, Expr *expr)
{
    ...
    {
        SEMA_ERROR(expr, "This expression implicitly returns with a failable result, but the function does not allow "
                         "failable results. Did you mean to use 'else' instead?");
        return false;
    }
...
{
    if (!sema_resolve_type_info(context, expr->expr_compound_literal.type_info))
        return false;
    ...
    if (!sema_expr_analyse_initializer_list(context, type, expr->expr_compound_literal.initializer))
        return false;
...
static inline bool decl_is_local(Decl *decl)
{
    ...
    return kind == VARDECL_PARAM_CT_TYPE || kind == VARDECL_PARAM || kind == VARDECL_PARAM_CT ||
           kind == VARDECL_LOCAL || kind == VARDECL_LOCAL_CT_TYPE || kind == VARDECL_LOCAL_CT ||
           kind == VARDECL_PARAM_REF || kind == VARDECL_PARAM_EXPR || kind == VARDECL_BITMEMBER ||
           kind == VARDECL_MEMBER;
}

static bool sema_expr_analyse_type_var_path(SemaContext *context, Expr *expr, ExprFlatElement **elements,
                                            Type **type_ref, Decl **decl_ref)
{
    if (!sema_analyse_expr_lvalue(context, expr))
        return false;
    Expr *current = expr;
    Decl *decl = NULL;
    Type *type = NULL;
...
static inline bool sema_expr_analyse_flat_element(SemaContext *context, ExprFlatElement *element, Type *type,
                                                  Decl **member_ref, ArraySize *index_ref, Type **return_type,
                                                  unsigned i, SourceSpan loc, bool *is_missing)
{
...
static inline bool sema_expr_analyse_ct_alignof(SemaContext *context, Expr *expr)
{
    ...
    if (!sema_expr_analyse_type_var_path(context, main_var, &path, &type, &decl))
        return false;
    ...
    VECEACH(path, i)
    {
        ...
        ArraySize index;
        Type *result_type;
        if (!sema_expr_analyse_flat_element(context, element, type, &member, &index, &result_type, i,
                                            i == 0 ? main_var->span : expr->span, NULL))
            return false;
        if (member)
        {
...
RETRY:
    ...
    case TYPE_INFO_VECTOR: {
        ArraySize size;
        if (!sema_resolve_array_like_len(context, type_info, &size))
            return poisoned_type;
...
RETRY:
    ...
    case EXPR_BUILTIN:
        if (!sema_expr_analyse_builtin(context, main_var, false))
            goto NOT_DEFINED;
        break;
    case EXPR_CT_EVAL: {
    ...
    VECEACH(flat_path, i)
    {
        ...
        Type *ret_type;
        bool missing = false;
        if (!sema_expr_analyse_flat_element(context, element, type, &member, &index, &ret_type, i,
                                            i == 0 ? main_var->span : expr->span, &missing))
        {
            if (missing)
                goto NOT_DEFINED;
            return false;
...
static inline bool sema_expr_analyse_ct_stringify(SemaContext *context, Expr *expr)
{
    ...
    if (!desc)
    {
        SEMA_ERROR(
            expr,
            "Failed to stringify hash variable contents - they must be a single line and not exceed 255 characters.");
        return false;
    }
...
static inline BuiltinFunction builtin_by_name(const char *name)
{
    ...
    {
        if (builtin_list[i] == name)
            return (BuiltinFunction)i;
    }
    return BUILTIN_NONE;
}
...
static inline bool sema_expr_analyse_builtin(SemaContext *context, Expr *expr, bool throw_error)
{
    ...
    {
        if (throw_error)
            SEMA_ERROR(expr, "Unsupported builtin '%s'.", builtin_char);
        return false;
    }
...
static inline bool sema_analyse_expr_dispatch(SemaContext *context, Expr *expr)
{
    ...
    case EXPR_VARIANTSWITCH:
        UNREACHABLE
    case EXPR_STRINGIFY:
        if (!sema_expr_analyse_ct_stringify(context, expr))
            return false;
        return true;
    case EXPR_ARGV_TO_SUBARRAY:
        ...
        return true;
    case EXPR_DECL:
        if (!sema_analyse_var_decl(context, expr->decl_expr, true))
            return false;
...
bool sema_analyse_cond_expr(SemaContext *context, Expr *expr)
{
    ...
    {
        SEMA_ERROR(expr,
                   "A failable %s cannot be implicitly converted to a regular boolean value, use 'try(<expr>)' "
...
bool sema_analyse_expr_rhs(SemaContext *context, Type *to, Expr *expr, bool allow_failable)
{
    ...
    if (!sema_analyse_inferred_expr(context, to, expr))
        return false;
    if (to && !cast_implicit(expr, to))
        return false;
...
static inline bool sema_cast_rvalue(SemaContext *context, Expr *expr)
{
    ...
    case EXPR_MACRO_BODY_EXPANSION:
        ...
        {
            SEMA_ERROR(expr, "'@%s' must be followed by ().",
                       declptr(context->current_macro->macro_decl.body_param)->name);
            return false;
        }
        break;
    case EXPR_BUILTIN:
    ...
    case EXPR_MACRO_EXPANSION:
        ...
        return expr_poison(expr);
    case EXPR_CT_IDENT:
        if (!sema_cast_ct_ident_rvalue(context, expr))
            return false;
        break;
    case EXPR_IDENTIFIER:
        if (!sema_cast_ident_rvalue(context, expr))
            return false;
        break;
    case EXPR_UNARY: {
        if (expr->unary_expr.operator!= UNARYOP_DEREF)
            break;
        Expr *inner = expr->inner_expr;
        if (inner->expr_kind != EXPR_IDENTIFIER)
            break;
...
bool sema_analyse_expr_lvalue(SemaContext *context, Expr *expr)
{
    ...
    case RESOLVE_NOT_DONE:
        expr->resolve_status = RESOLVE_RUNNING;
        if (!sema_analyse_expr_dispatch(context, expr))
            return expr_poison(expr);
        expr->resolve_status = RESOLVE_DONE;
        return true;
    case RESOLVE_RUNNING:
...
MemberIndex sema_get_initializer_const_array_size(SemaContext *context, Expr *initializer, bool *may_be_array,
                                                  bool *is_const_size)
{
...
void insert_widening_type(Expr *expr, Type *infer_type)
{
    ...
    case EXPR_BINARY:
        ...
        case BINARYOP_BIT_XOR:
        case BINARYOP_BIT_AND:
            if (!expr_is_simple(exprptr(expr->binary_expr.left)) || !expr_is_simple(exprptr(expr->binary_expr.right)))
                return;
    ...
    case EXPR_OR_ERROR:
        if (!expr_is_simple(expr->or_error_expr.expr))
            return;
        if (!expr->or_error_expr.is_jump && !expr_is_simple(expr->or_error_expr.or_error_expr))
            return;
        expr->type = infer_type;
    ...
    case EXPR_GROUP:
        ...
        return;
    case EXPR_TERNARY:
        if (!exprid_is_simple(expr->ternary_expr.else_expr))
            return;
        if (expr->ternary_expr.then_expr && !exprid_is_simple(expr->ternary_expr.else_expr))
            return;
        expr->type = infer_type;
...
bool sema_analyse_inferred_expr(SemaContext *context, Type *infer_type, Expr *expr)
{
    ...
    case EXPR_INITIALIZER_LIST:
    case EXPR_DESIGNATED_INITIALIZER_LIST:
        if (!sema_expr_analyse_initializer_list(context, infer_type, expr))
            return expr_poison(expr);
        break;
    case EXPR_EXPR_BLOCK:
        if (!sema_expr_analyse_expr_block(context, infer_type, expr))
            return expr_poison(expr);
        break;
    case EXPR_IDENTIFIER:
        if (!sema_expr_analyse_identifier(context, infer_type, expr))
            return expr_poison(expr);
        break;
    default:
    ...
    if (!sema_cast_rvalue(context, expr))
        return false;
    expr->resolve_status = RESOLVE_DONE;
    return true;
}
