...
LLVMValueRef llvm_emit_local_decl(GenContext *c, Decl *decl)
{
    ...
    if (decl->var.is_static)
    {
        ...
        if (IS_FAILABLE(decl))
        {
            ...
            decl->var.failable_ref =
                LLVMAddGlobal(c->module, llvm_get_type(c, type_anyerr), scratch_buffer_to_string());
        }
...
void llvm_emit_for_stmt(GenContext *c, Ast *ast)
{
    ...
    if (ast->for_stmt.init)
        llvm_emit_expr(c, &value, exprptr(ast->for_stmt.init));
    ...
    if (!inc_block && !body_block && !cond_block)
    {
        if (loop == LOOP_INFINITE)
        {
            ...
            llvm_emit_debug_output(c, "Infinite loop found", file->name, c->cur_func_decl->extname,
                                   loc.row ? loc.row : 1);
...
static void llvm_emit_switch_body_if_chain(GenContext *c, Ast **cases, Ast *default_case, BEValue *switch_value,
...
static void llvm_emit_switch_jump_table(GenContext *c, Ast **cases, Ast *default_case, BEValue *switch_value,
                                        LLVMBasicBlockRef exit_block)
{
    ...
    switch_value->value =
        LLVMBuildSub(c->builder, switch_value->value, llvm_const_int(c, switch_value->type, min.i.low), "");
    max = int_sub(max, min);
...
static void gencontext_emit_switch_body(GenContext *c, BEValue *switch_value, Ast *switch_ast)
{
    ...
    Type *switch_type = switch_ast->ast_kind == AST_IF_CATCH_SWITCH_STMT ? type_lowering(type_anyerr)
                                                                         : exprptr(switch_ast->switch_stmt.cond)->type;
    ...
    c->current_block = NULL;
    LLVMValueRef switch_stmt =
        LLVMBuildSwitch(c->builder, switch_current_val.value,
                        default_case ? default_case->case_stmt.backend_block : exit_block, case_count);
    for (unsigned i = 0; i < case_count; i++)
    {
        Ast *case_stmt = cases[i];
        LLVMBasicBlockRef block = case_stmt->case_stmt.backend_block;
...
static inline void llvm_emit_assert_stmt(GenContext *c, Ast *ast)
{
    ...
    if (!exprid)
    {
        ...
        llvm_emit_debug_output(c, "Unreachable statement reached.", file->name, c->cur_func_decl->extname,
                               row ? row : 1);
...
static inline void llvm_emit_asm_stmt(GenContext *c, Ast *ast)
{
    ...
    LLVMValueRef asm_fn =
        LLVMGetInlineAsm(asm_fn_type, (char *)ast->asm_stmt.body->const_expr.string.chars,
                         ast->asm_stmt.body->const_expr.string.len, scratch_buffer_to_string(),
                         global_context.scratch_buffer_len, ast->asm_stmt.is_volatile, true, LLVMInlineAsmDialectIntel
...
void llvm_emit_debug_output(GenContext *c, const char *message, const char *file, const char *func, unsigned line)
{
    ...
    if (platform_target.arch == ARCH_TYPE_WASM32 || platform_target.arch == ARCH_TYPE_WASM64)
        os = OS_TYPE_WASI;
    switch (os)
    {
    case OS_TYPE_WIN32:
