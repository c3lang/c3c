...
{
    return (Int){type_kind_is_unsigned(type) ? i128_from_float_unsigned(d) : i128_from_float_signed(d), type};
}
...
Int128 i128_shl64(Int128 op1, uint64_t amount)
{
    ...
    if (amount > 127)
        return (Int128){0, 0};
    if (amount == 64)
        return (Int128){op1.low, 0};
    if (amount > 64)
        return (Int128){op1.low << (amount - 64), 0};
...
Int128 i128_lshr64(Int128 op1, uint64_t amount)
{
    ...
    if (amount > 127)
        return (Int128){0, 0};
    if (amount == 64)
        return (Int128){0, op1.high};
    if (amount > 64)
        return (Int128){0, op1.high >> (amount - 64)};
    op1.low >>= amount;
...
Int128 i128_ashr64(Int128 op1, uint64_t amount)
{
    if (amount == 0)
        return op1;
    if (!ISNEG(op1.high))
        return i128_lshr64(op1, amount);
    if (amount > 127)
        return (Int128){UINT64_MAX, UINT64_MAX};
    if (amount == 64)
        return (Int128){UINT64_MAX, op1.high};
    if (amount > 64)
        return (Int128){UINT64_MAX, (uint64_t)(((int64_t)op1.high) >> (amount - 64))};
...
{
    if (op2.high != 0)
        return ISNEG(op1.high) ? (Int128){UINT64_MAX, UINT64_MAX} : (Int128){0, 0};
...
static CmpRes int_icompare(Int op1, int64_t op2)
{
    ...
    if (op1.i.high || op2 < 0 || op1.i.low > op2)
        return CMP_GT;
    if (op1.i.low < op2)
        return CMP_LT;
    return CMP_EQ;
...
{
    if (type_kind_is_signed(op1.type) && i128_is_neg(op1.i))
        return CMP_LT;
    if (op1.i.high || op1.i.low > op2)
        return CMP_GT;
    if (op1.i.low < op2)
        return CMP_LT;
    return CMP_EQ;
...
static CmpRes int_compare(Int op1, Int op2)
{
    if (type_kind_is_signed(op1.type))
    {
        if (type_kind_is_signed(op2.type))
            return i128_scomp(op1.i, op2.i);
        if (int_is_neg(op1))
            return CMP_LT;
...
bool int_fits(Int op1, TypeKind kind)
{
    ...
    if (is_signed)
    {
        if (op_is_signed)
        {
            if (i128_scomp(op1.i, min) == CMP_LT)
                return false;
            if (i128_scomp(op1.i, max) == CMP_GT)
                return false;
            return true;
        }
    ...
    if (op_is_signed)
    {
        if (i128_is_neg(op1.i))
            return false;
        if (i128_ucomp(op1.i, max) == CMP_GT)
            return false;
        return true;
    }
    // In the unsigned case, we don't need to test the lower limit.
...
Int int_conv(Int op, TypeKind to_type)
{
    ...
    if (to_bitsize == bitsize || to_bitsize == 128)
...
{
    return isfinite(x) && x > -1 && x < ldexp(1, 128);
}

bool i128_can_convert_from_double_signed(double x)
{
    return isfinite(x) && x >= -ldexp(1, 127) && x < ldexp(1, 127);
}
