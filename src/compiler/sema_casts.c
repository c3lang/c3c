...
{
    if (insert_runtime_cast_unless_const(expr, CAST_PTRXI, type))
        return true;
...
{
    if (insert_runtime_cast_unless_const(expr, CAST_PTRBOOL, type))
        return true;
...
{
    if (insert_runtime_cast_unless_const(expr, CAST_PTRPTR, type))
        return true;
...
{
    if (insert_runtime_cast_unless_const(expr, CAST_BOOLINT, type))
        return true;
...
{
    if (insert_runtime_cast_unless_const(expr, CAST_BOOLFP, type))
        return true;
...
{
    if (insert_runtime_cast_unless_const(expr, CAST_INTBOOL, type))
        return true;
...
{
    if (insert_runtime_cast_unless_const(expr, CAST_FPBOOL, type))
        return true;
...
{
    if (insert_runtime_cast_unless_const(expr, CAST_FPFP, type))
        return true;
...
bool float_to_integer(Expr *expr, Type *canonical, Type *type)
{
    ...
    if (insert_runtime_cast_unless_const(expr, is_signed ? CAST_FPSI : CAST_FPUI, type))
        return true;
...
static bool int_conversion(Expr *expr, CastKind kind, Type *canonical, Type *type)
{
    ...
    if (expr->expr_kind == EXPR_CAST && expr->cast_expr.kind == CAST_PTRXI && type_size(type) <= type_size(expr->type))
    {
        expr->type = type;
        return true;
    }
    if (insert_runtime_cast_unless_const(expr, kind, type))
        return true;
...
bool cast_may_explicit(Type *from_type, Type *to_type, bool ignore_failability, bool is_const)
{
    ...
    if (type_is_failable(from_type) && !type_is_failable(to_type))
    {
        if (from_type->failable == type_void || !from_type->failable)
        {
            ...
            if (to_type->type_kind == TYPE_ERRTYPE || to_type->type_kind == TYPE_ANYERR)
                return true;
        }
        if (!ignore_failability)
            return false;
    }
    ...
    {
        if (from_type->type_kind == TYPE_ARRAY &&
            type_flatten_distinct(from_type->array.base) == type_flatten_distinct(to_type->array.base))
            return true;
        return false;
    }
    ...
    case ALL_UNSIGNED_INTS:
        ...
        if (to_kind == TYPE_POINTER && is_const)
            return true;
        FALLTHROUGH;
    case TYPE_ENUM:
        // Allow conversion int/enum -> float/bool/enum int/enum -> pointer is only allowed if the int/enum is pointer
        // sized.
        if (type_is_integer(to_type) || type_is_float(to_type) || to_type == type_bool || to_kind == TYPE_ENUM)
            return true;
        // TODO think about this, maybe we should require a bitcast?
        if (to_kind == TYPE_POINTER && type_is_pointer_sized(from_type))
            return true;
        return false;
    case ALL_FLOATS:
    ...
    case TYPE_POINTER:
        ...
        if ((type_is_integer(to_type) && type_size(to_type) >= type_size(type_iptr)) || to_type == type_bool ||
            to_kind == TYPE_POINTER)
            return true;
        // Special subarray conversion: someType[N]* -> someType[]
        if (to_kind == TYPE_SUBARRAY && from_type->pointer->type_kind == TYPE_ARRAY &&
            from_type->pointer->array.base == to_type->array.base)
            return true;
        return false;
    case TYPE_ANY:
    ...
    case TYPE_STRUCT:
        ...
        {
            if (cast_may_explicit(from_type->decl->strukt.members[0]->type, to_type, false, false))
                return true;
        }
        FALLTHROUGH;
    case TYPE_UNION:
    ...
    case TYPE_SUBARRAY:
        return to_kind == TYPE_POINTER;
    case TYPE_VECTOR:
        return type_is_structurally_equivalent(type_get_array(from_type->array.base, (uint32_t)from_type->array.len),
                                               to_type);
    case TYPE_UNTYPED_LIST:
...
{
    if (type_is_failable_any(type))
        return true;
    if (!type_is_failable_type(type))
        return false;
...
bool cast_may_implicit(Type *from_type, Type *to_type, bool is_simple_expr, bool failable_allowed)
{
    ...
    if (to == type_anyerr && type_may_convert_to_anyerr(from_type))
        return true;

    // 2. any! => may implicitly to convert to any.
    if (type_is_failable_any(from_type))
        return failable_allowed;
    ...
    if (to == type_anyerr && from->type_kind == TYPE_ERRTYPE)
        return true;
    ...
    if (type_is_pointer(to))
    {
        ...
        if (from->type_kind == TYPE_POINTER)
        {
            ...
            if (to == type_voidptr || from == type_voidptr)
                return true;
...
bool may_convert_float_const_implicit(Expr *expr, Type *to_type)
{
    ...
    {
#if LONG_DOUBLE
        ERROR_NODE(expr, "The value '%Lg' is out of range for %s, so you need an explicit cast to truncate the value.",
                   expr->const_expr.fxx.f, type_quoted_error_string(to_type));
#else
        SEMA_ERROR(expr, "The value '%g' is out of range for %s, so you need an explicit cast to truncate the value.",
                   expr->const_expr.fxx.f, type_quoted_error_string(to_type));
#endif
        return false;
    }
...
Expr *recursive_may_narrow_float(Expr *expr, Type *type)
{
    ...
    case EXPR_TERNARY: {
        Expr *res = recursive_may_narrow_floatid(
            expr->ternary_expr.then_expr ? expr->ternary_expr.then_expr : expr->ternary_expr.cond, type);
        if (res)
            return res;
...
Expr *recursive_may_narrow_int(Expr *expr, Type *type)
{
    ...
    case EXPR_RETVAL:
        ...
        return NULL;
    case EXPR_LEN:
        if (type_size(type) < type_size(type_cint))
            return expr;
        return NULL;
    case EXPR_OR_ERROR: {
    ...
    case EXPR_TERNARY: {
        Expr *res = recursive_may_narrow_intid(
            expr->ternary_expr.then_expr ? expr->ternary_expr.then_expr : expr->ternary_expr.cond, type);
        if (res)
            return res;
...
static void sema_error_const_int_out_of_range(Expr *expr, Expr *problem, Type *to_type)
{
    ...
    {
        SEMA_ERROR(problem, "The unicode character U+%04x cannot fit in a %s.", (uint32_t)expr->const_expr.ixx.i.low,
                   type_quoted_error_string(to_type));
        return;
    }
    const char *error_value =
        expr->const_expr.is_hex ? int_to_str(expr->const_expr.ixx, 16) : expr_const_to_error_string(&expr->const_expr);
    SEMA_ERROR(problem, "The value '%s' is out of range for %s, so you need an explicit cast to truncate the value.",
               error_value, type_quoted_error_string(to_type));
}
...
static inline bool cast_maybe_string_lit_to_char_array(Expr *expr, Type *expr_canonical, Type *to_canonical)
{
    if (expr->expr_kind != EXPR_CONST || expr->const_expr.const_kind != CONST_STRING)
        return false;
    if (expr_canonical->type_kind != TYPE_POINTER)
        return false;
    if (to_canonical->type_kind != TYPE_ARRAY && to_canonical->type_kind != TYPE_INFERRED_ARRAY)
        return false;
    if (to_canonical->array.base != type_char)
        return false;
    Type *pointer = expr_canonical->pointer;
    if (pointer->type_kind != TYPE_ARRAY)
        return false;
    if (pointer->array.base != type_char)
        return false;
...
bool cast_implicit(Expr *expr, Type *to_type)
{
    ...
    if (expr_canonical == to_canonical)
        return true;
    ...
    if (!cast_may_implicit(expr_canonical, to_canonical, is_simple, true))
    {
        if (!cast_may_explicit(expr_canonical, to_canonical, false, expr->expr_kind == EXPR_CONST))
        {
            ...
            {
                SEMA_ERROR(expr, "A failable %s cannot be converted to %s.", type_quoted_error_string(expr->type),
                           type_quoted_error_string(to_type));
                return false;
            }
            ...
            {
                SEMA_ERROR(expr,
                           "You can only convert pointers to 'variant', take the address of this expression first.");
                return false;
            }
            SEMA_ERROR(expr, "You cannot cast %s into %s even with an explicit cast, so this looks like an error.",
        ...
        if (expr->expr_kind == EXPR_CONST && expr->const_expr.narrowable && is_narrowing)
        {
            ...
            if (type_is_float(expr_flatten) && type_is_float(to_flatten))
            {
                ...
                if (problem)
                {
                    SEMA_ERROR(
                        problem,
                        "The value '%s' is out of range for %s, so you need an explicit cast to truncate the value.",
        ...
        if (type_is_float(expr_canonical) && type_is_float(to_canonical) && is_narrowing)
        {
            ...
            if (problem)
            {
                ...
                {
                    SEMA_ERROR(problem, "The value '%s' is out of range for %s.",
                               expr_const_to_error_string(&problem->const_expr), type_quoted_error_string(to_type));
                }
                else
                {
        ...
        SEMA_ERROR(
            expr,
            "Implicitly casting %s to %s is not permitted, but you can do an explicit cast using '(<type>)(value)'.",
            type_quoted_error_string(type_no_fail(expr->type)), type_quoted_error_string(type_no_fail(to_type)));
        return false;
    }
...
OK:
    ...
    if (expr->expr_kind == EXPR_CONST && expr->const_expr.narrowable)
    {
        ...
        {
            if (!may_convert_float_const_implicit(expr, to_type))
                return false;
        }
        else if (type_is_integer(expr->type))
        {
            if (!may_convert_int_const_implicit(expr, to_type))
                return false;
        }
    }
    cast(expr, to_type);
    // Allow narrowing after widening
    if (type_is_numeric(to_type) && expr->expr_kind == EXPR_CONST &&
        type_size(expr_canonical) < type_size(to_canonical))
    {
        expr->const_expr.narrowable = true;
    }
    if (expr->expr_kind == EXPR_CAST)
        expr->cast_expr.implicit = true;
    return true;
}
...
static bool cast_inner(Expr *expr, Type *from_type, Type *to, Type *to_type)
{
    ...
    case TYPE_BOOL:
        ...
        if (type_is_integer(to))
            return bool_to_int(expr, to, to_type);
        if (type_is_float(to))
            return bool_to_float(expr, to, to_type);
        break;
    case TYPE_ANYERR:
        if (to->type_kind == TYPE_BOOL)
            return insert_cast(expr, CAST_EUBOOL, to_type);
        if (to->type_kind == TYPE_ERRTYPE)
            return insert_cast(expr, CAST_EUER, to_type);
        if (type_is_integer(to))
            return insert_cast(expr, CAST_EUINT, to_type);
        break;
    case ALL_SIGNED_INTS:
        if (type_is_integer_unsigned(to))
            return int_conversion(expr, CAST_SIUI, to, to_type);
        if (type_is_integer_signed(to))
            return int_conversion(expr, CAST_SISI, to, to_type);
        if (type_is_float(to))
            return int_to_float(expr, CAST_SIFP, to, to_type);
        if (to == type_bool)
            return integer_to_bool(expr, to_type);
        if (to->type_kind == TYPE_POINTER)
            return int_to_pointer(expr, to_type);
        if (to->type_kind == TYPE_ENUM)
            return lit_integer_to_enum(expr, to, to_type);
        break;
    case ALL_UNSIGNED_INTS:
        if (type_is_integer_unsigned(to))
            return int_conversion(expr, CAST_UIUI, to, to_type);
        if (type_is_integer_signed(to))
            return int_conversion(expr, CAST_UISI, to, to_type);
        if (type_is_float(to))
            return int_to_float(expr, CAST_UIFP, to, to_type);
        if (to == type_bool)
            return integer_to_bool(expr, to_type);
        if (to->type_kind == TYPE_POINTER)
            return int_to_pointer(expr, to_type);
    ...
    case ALL_FLOATS:
        if (type_is_integer(to))
            return float_to_integer(expr, to, to_type);
        if (to == type_bool)
            return float_to_bool(expr, to_type);
        if (type_is_float(to))
            return float_to_float(expr, to, to_type);
        break;
    case TYPE_POINTER:
        if (type_is_integer(to))
            return pointer_to_integer(expr, to_type);
        if (to->type_kind == TYPE_BOOL)
            return pointer_to_bool(expr, to_type);
        if (to->type_kind == TYPE_POINTER)
            return pointer_to_pointer(expr, to_type);
        if (to->type_kind == TYPE_SUBARRAY)
            return insert_cast(expr, CAST_APTSA, to_type);
        if (to->type_kind == TYPE_ANY)
            return insert_cast(expr, CAST_PTRANY, to_type);
        break;
    case TYPE_ANY:
        if (to->type_kind == TYPE_POINTER)
            return insert_cast(expr, CAST_ANYPTR, to_type);
        break;
    case TYPE_ENUM:
        if (type_is_integer(to))
            return enum_to_integer(expr, from_type, to, to_type);
        if (type_is_float(to))
            return enum_to_float(expr, from_type, to, to_type);
        if (to == type_bool)
            return enum_to_bool(expr, from_type, to_type);
        if (to->type_kind == TYPE_POINTER)
            return enum_to_pointer(expr, from_type, to_type);
        break;
    case TYPE_ERRTYPE:
        if (to->type_kind == TYPE_ANYERR)
            return err_to_anyerr(expr, to_type);
        if (to == type_bool)
            return err_to_bool(expr, to_type);
        if (type_is_integer(to))
            return insert_cast(expr, CAST_ERINT, to_type);
        break;
    case TYPE_FLEXIBLE_ARRAY:
        return false;
    case TYPE_ARRAY:
        if (to->type_kind == TYPE_VECTOR)
            return insert_cast(expr, CAST_ARRVEC, to_type);
        FALLTHROUGH;
    case TYPE_STRUCT:
    case TYPE_UNION:
    ...
    case TYPE_SUBARRAY:
        if (to->type_kind == TYPE_POINTER)
            return insert_cast(expr, CAST_SAPTR, to);
        if (to->type_kind == TYPE_BOOL)
            return subarray_to_bool(expr);
        break;
    case TYPE_VECTOR:
        if (to->type_kind == TYPE_ARRAY)
            return insert_cast(expr, CAST_VECARR, to);
        break;
    }
    UNREACHABLE
}
...
bool cast(Expr *expr, Type *to_type)
{
    ...
    {
        if (type_is_failable(from_type))
            return voidfail_to_error(expr, to_type);
    }
