...
bool sema_expr_analyse_str_wide(SemaContext *context, Expr *expr, BuiltinFunction func)
{
	...
	if (BUILTIN_WIDESTRING == func)
	{
		switch (compiler.build.arch_os_target)
		{
			...
			case WINDOWS_X64:
			case MINGW_X64:
				func = BUILTIN_WIDESTRING_16;
				break;
			default:
				func = BUILTIN_WIDESTRING_32;
				break;
		}
	}
	...
	switch (func) {
		case BUILTIN_WIDESTRING_16: bytes_per_unit = sizeof(unsigned short); break;
		case BUILTIN_WIDESTRING_32: bytes_per_unit = sizeof(unsigned int); break;
		default: UNREACHABLE;
	}

	ArraySize len = (original_len + 1) * bytes_per_unit;   // inflate to unit size +null-term
	char *new_string = malloc_string(len);

	// Iterate over the original array of bytes. Since we know the unit size of the resultant array, we must
	...
	const char *scroll = string;
	const char *end = (const char *)((size_t)string + original_len);
	while (scroll < end)
	{
		if ((0xC0 & *(scroll++)) != 0xC0) continue;

		// This assumes the full codepoint will always fit into one unit -- this is checked during UTF-8 to 16/32 conversions.
		int bytes_wide = 1;
	...
	ASSERT(!(len % bytes_per_unit));

	const char *err_string = NULL;
	switch (func)   // NOTE: only 16 and 32 needed here, since first switch-case converts base wchar_t to 16 or 32.
	{
		case BUILTIN_WIDESTRING_16:
			err_string = parse_wide_string_16(new_string, string, (const char *)((size_t)string + original_len));
			break;
		case BUILTIN_WIDESTRING_32:
			err_string = parse_wide_string_32(new_string, string, (const char *)((size_t)string + original_len));
			break;
		default: UNREACHABLE;
	}
	...
	if (NULL != err_string)
	{
		RETURN_SEMA_ERROR(inner, err_string);
	}

	new_string[len] = 0;

	expr->expr_kind = EXPR_CONST;
	expr->const_expr.const_kind = CONST_BYTES;
	expr->const_expr.bytes.ptr = new_string;
	expr->const_expr.bytes.len = len;
	expr->resolve_status = RESOLVE_DONE;
	expr->type = bytes_per_unit == sizeof(unsigned short)
		? type_get_array(type_ushort, (len / bytes_per_unit))
		: type_get_array(type_uint, (len / bytes_per_unit));
