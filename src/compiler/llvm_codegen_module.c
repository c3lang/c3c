...
void gencontext_init_file_emit(GenContext *c, CompilationUnit *unit)
{
    if (active_target.debug_info != DEBUG_INFO_NONE)
    {
        ...
        unit->llvm.debug_file =
            LLVMDIBuilderCreateFile(c->debug.builder, filename, strlen(filename), dir_path, strlen(dir_path));
        ...
        const char *sdk = "";
        unsigned dwo_id = 0;
        unit->llvm.debug_compile_unit = LLVMDIBuilderCreateCompileUnit(
            c->debug.builder, LLVMDWARFSourceLanguageC, unit->llvm.debug_file, DWARF_PRODUCER_NAME,
            strlen(DWARF_PRODUCER_NAME), is_optimized, dwarf_flags, strlen(dwarf_flags), runtime_version,
            debug_output_file, strlen(debug_output_file), emission_kind, dwo_id, split_debug_inlining,
            emit_debug_info_for_profiling
#if LLVM_VERSION_MAJOR >= 11
            ,
            sysroot, strlen(sysroot), sdk, strlen(sdk)
#endif
        );
    }
}
