...
static inline bool sema_check_no_duplicate_parameter(Decl **decls, Decl *current, unsigned current_index,
                                                     unsigned count)
{
...
static inline bool sema_analyse_struct_member(SemaContext *context, Decl *decl)
{
    ...
    case DECL_VAR:
        ...
        if (!sema_resolve_type_info_maybe_inferred(context, decl->var.type_info, true))
...
static bool sema_analyse_union_members(SemaContext *context, Decl *decl, Decl **members)
{
    ...
    if (decl->is_packed && !decl->alignment)
        decl->alignment = 1;

    // 2. Otherwise pick the highest of the natural alignment and the given alignment.
    if (!decl->is_packed)
        decl->alignment = MAX(decl->alignment, max_alignment);
...
static bool sema_analyse_struct_members(SemaContext *context, Decl *decl, Decl **members)
{
    ...
    for (unsigned i = 0; i < member_count; i++)
    {
        ...
        for (unsigned j = 0; j < count; j++)
        {
            ...
            if (!sema_analyse_attribute(context, attribute, ATTR_GLOBAL))
                return false;
            if (attribute->name == kw_align)
            {
                ...
                if (member_alignment > decl->alignment)
                    decl->alignment = member_alignment;
            }
        }
    ...
    // 2. Otherwise pick the highest of the natural alignment and the given alignment.
    if (!decl->is_packed)
        decl->alignment = MAX(decl->alignment, natural_alignment);
...
static bool sema_analyse_struct_union(SemaContext *context, Decl *decl)
{
    ...
    VECEACH(decl->attributes, i)
    {
        ...
        bool had = false;
#define SET_ATTR(_X)                                                                                                   \
    had = decl->func_decl._X;                                                                                          \
    decl->func_decl._X = true;                                                                                         \
    break
        switch (attribute)
        {
...
#undef SET_ATTR
    ...
    if (!vec_size(members))
    {
        SEMA_ERROR(decl, decl->decl_kind == DECL_UNION ? "Zero sized unions are not permitted."
                                                       : "Zero sized structs are not permitted.");
        return false;
    }
    if (decl->name)
    {
...
static inline bool sema_analyse_bitstruct_member(SemaContext *context, Decl *decl, unsigned index, bool allow_overlap)
{
    ...
    {
        SEMA_ERROR(member->var.type_info,
                   "%s is not supported in a bitstruct, only enums, integer and boolean values may be used.",
    ...
    else
    {
        ...
        {
            SEMA_ERROR(member, "Only booleans may use non-range indices, try using %d..%d instead.", start_bit,
                       start_bit);
            return false;
        }
    }
...
static bool sema_analyse_bitstruct(SemaContext *context, Decl *decl)
{
    VECEACH(decl->attributes, i)
    {
        ...
        bool had = false;
#define SET_ATTR(_X)                                                                                                   \
    had = decl->bitstruct._X;                                                                                          \
    decl->bitstruct._X = true;                                                                                         \
    break
        switch (attribute)
        {
...
#undef SET_ATTR
    ...
    DEBUG_LOG("Beginning analysis of %s.", decl->name ? decl->name : "anon");
    if (!sema_resolve_type_info(context, decl->bitstruct.base_type))
        return false;
    ...
    {
        SEMA_ERROR(decl->bitstruct.base_type,
                   "The type of the bitstruct cannot be %s but must be an integer or an array of integers.",
...
static inline bool sema_analyse_function_param(SemaContext *context, Decl *param, bool is_function, bool *has_default)
{
    *has_default = false;
    ...
    if (!sema_analyse_attributes_for_var(context, param))
        return false;
    ...
    if (param->var.init_expr)
    {
        ...
        if (!sema_analyse_expr_rhs(context, param->type, expr, true))
            return false;
        if (IS_FAILABLE(expr))
        {
...
static inline Type *sema_analyse_function_signature(SemaContext *context, CallABI abi, FunctionSignature *signature,
                                                    bool is_real_function)
{
    bool all_ok = true;
...
static inline bool sema_analyse_typedef(SemaContext *context, Decl *decl)
{
    ...
    {
        Type *func_type =
            sema_analyse_function_signature(context, CALL_C, &decl->typedef_decl.function_signature, false);
        if (!func_type)
            return false;
...
static inline bool sema_analyse_distinct(SemaContext *context, Decl *decl)
{
    ...
    {
        Type *func_type = sema_analyse_function_signature(context, CALL_C,
                                                          &decl->distinct_decl.typedef_decl.function_signature, false);
        if (!func_type)
            return false;
    ...
    if (!sema_resolve_type_info(context, info))
        return false;
...
static inline bool sema_analyse_enum(SemaContext *context, Decl *decl)
{
    ...
    if (!sema_resolve_type_info(context, decl->enums.type_info))
        return false;
    ...
    {
        SEMA_ERROR(decl->enums.type_info, "The enum type must be an integer type not '%s'.",
    ...
    for (unsigned i = 0; i < enums; i++)
    {
        ...
        if (!expr)
        {
            ...
            {
                SEMA_ERROR(enum_value, "The enum value would implicitly be %s which does not fit in %s.",
                           i128_to_string(value, 10, type_is_signed(canonical)), type_quoted_error_string(type));
                return false;
            }
...
static bool sema_analyse_operator_common(Decl *method, TypeInfo **rtype_ptr, Decl ***params_ptr, uint32_t parameters)
{
    ...
    {
        SEMA_ERROR(params[parameters], "Too many parameters, '%s' expects only %u.", method->name,
                   (unsigned)parameters);
        return false;
    }
    if (param_count < parameters)
...
static inline bool sema_analyse_operator_element_at(Decl *method)
{
    TypeInfo *rtype;
    Decl **params;
    if (!sema_analyse_operator_common(method, &rtype, &params, 2))
        return false;
...
static inline bool sema_analyse_operator_element_set(Decl *method)
{
    TypeInfo *rtype;
    Decl **params;
    if (!sema_analyse_operator_common(method, &rtype, &params, 3))
        return false;
    if (rtype->type->canonical != type_void)
    {
...
static inline bool sema_analyse_operator_len(Decl *method)
{
    TypeInfo *rtype;
    Decl **params;
    if (!sema_analyse_operator_common(method, &rtype, &params, 1))
        return false;
...
static inline bool unit_add_method_like(CompilationUnit *unit, Type *parent_type, Decl *method_like)
{
    ...
    if (method_like->operator&& !sema_check_operator_method_validity(method_like))
        return false;
...
static inline bool sema_analyse_method(SemaContext *context, Decl *decl)
{
    ...
    if (!sema_resolve_type_info(context, parent_type))
        return false;
    if (!type_may_have_sub_elements(parent_type->type))
    {
        SEMA_ERROR(decl, "Methods can not be associated with '%s'", type_to_error_string(parent_type->type));
        return false;
    }
...
AttributeType sema_analyse_attribute(SemaContext *context, Attr *attr, AttributeDomain domain)
{
    ...
    case ATTRIBUTE_OPERATOR: {
        Expr *expr = attr->expr;
        if (!expr || expr->expr_kind != EXPR_IDENTIFIER)
            goto FAILED_OP_TYPE;
    ...
    FAILED_OP_TYPE:
        SEMA_ERROR(attr, "'operator' requires an operator type argument: '%s', '%s' or '%s'.", kw_elementat,
                   kw_elementref, kw_len);
        return ATTRIBUTE_NONE;
    }
    case ATTRIBUTE_ALIGN:
        if (!attr->expr)
        ...
        if (!sema_analyse_expr(context, attr->expr))
            return false;
...
static inline bool sema_analyse_doc_header(AstId doc, Decl **params, Decl **extra_params, bool *pure_ref)
{
    while (doc)
    {
        ...
        if (directive_kind != DOC_DIRECTIVE_PARAM)
            continue;
...
{
    if (decl == context->unit->main_function)
        return true;
    ...
    if (rtype->type_kind == TYPE_FAILABLE_ANY)
        is_err_return = true;
    ...
    case 2:
        ...
        {
            SEMA_ERROR(params[0], "Expected a parameter of type %s for a C-style main.",
...
#define SET_ATTR(_X)                                                                                                   \
    ...
    break
        ...
        case ATTRIBUTE_SECTION:
            ...
            if (!sema_check_section(context, decl, attr))
                return decl_poison(decl);
            break;
        case ATTRIBUTE_ALIGN:
            had = decl->alignment != 0;
        ...
        case ATTRIBUTE_NORETURN:
            SET_ATTR(attr_noreturn);
        case ATTRIBUTE_INLINE:
            SET_ATTR(attr_inline);
        case ATTRIBUTE_WEAK:
            SET_ATTR(attr_weak);
...
#undef SET_ATTR
    ...
    {
        if (!sema_analyse_method(context, decl))
            return decl_poison(decl);
    }
    else
    {
        ...
        {
            if (!sema_analyse_main_function(context, decl))
                return decl_poison(decl);
        }
        decl_set_external_name(decl);
    }
    bool pure = false;
    if (!sema_analyse_doc_header(decl->func_decl.docs, decl->func_decl.function_signature.params, NULL, &pure))
        return decl_poison(decl);
...
static bool sema_analyse_macro_method(SemaContext *context, Decl *decl)
{
    ...
    if (!sema_resolve_type_info(context, parent_type_info))
        return false;
    Type *parent_type = parent_type_info->type;
    if (!type_may_have_sub_elements(parent_type))
    {
        SEMA_ERROR(parent_type_info, "Methods can not be associated with '%s'", type_to_error_string(parent_type));
        return false;
    }
...
static inline bool sema_analyse_macro(SemaContext *context, Decl *decl)
{
    ...
    if (decl->macro_decl.rtype && !sema_resolve_type_info(context, rtype))
        return decl_poison(decl);
    ...
    for (unsigned i = 0; i < param_count; i++)
    {
        ...
        if (!sema_check_no_duplicate_parameter(parameters, param, i, param_count))
            return decl_poison(decl);
        param->resolve_status = RESOLVE_DONE;
    }
    ...
    for (unsigned i = 0; i < body_param_count; i++)
    {
        ...
        {
        case VARDECL_PARAM:
            if (param->var.type_info && !sema_resolve_type_info(context, param->var.type_info))
                return decl_poison(decl);
            break;
        case VARDECL_PARAM_EXPR:
        case VARDECL_PARAM_CT:
        ...
        if (!sema_check_no_duplicate_parameter(body_parameters, param, i, body_param_count))
            return decl_poison(decl);
        param->resolve_status = RESOLVE_DONE;
    }
    bool pure = false;
    if (!sema_analyse_doc_header(decl->macro_decl.docs, decl->macro_decl.parameters, body_parameters, &pure))
        return decl_poison(decl);
    if (decl->macro_decl.type_parent)
    {
        if (!sema_analyse_macro_method(context, decl))
            return decl_poison(decl);
    }
    decl->type = type_void;
    return true;
...
bool sema_analyse_var_decl_ct(SemaContext *context, Decl *decl)
{
    ...
    case VARDECL_LOCAL_CT_TYPE:
        ...
        {
            if (!sema_analyse_expr_lvalue(context, init))
                return false;
        ...
        break;
    case VARDECL_LOCAL_CT:
        if (decl->var.type_info && !sema_resolve_type_info(context, decl->var.type_info))
            return false;
        if (decl->var.type_info)
        {
            ...
            if ((init = decl->var.init_expr))
            {
                if (!sema_analyse_expr_rhs(context, decl->type, init, false))
                    return false;
        ...
        else
        {
            if ((init = decl->var.init_expr))
            {
                if (!sema_analyse_expr(context, init))
                    return false;
                if (!expr_is_constant_eval(init, CONSTANT_EVAL_ANY))
                {
                    SEMA_ERROR(init, "Expected a constant expression assigned to %s.", decl->name);
...
bool sema_analyse_var_decl(SemaContext *context, Decl *decl, bool local)
{
    ...
    if (!sema_analyse_attributes_for_var(context, decl))
        return false;
    ...
    else
    {
        ...
        if (!sema_add_local(context, decl))
            return decl_poison(decl);
    }
    ...
    if (decl->var.kind == VARDECL_CONST)
    {
        ...
        if (!decl->var.type_info)
        {
            if (!sema_analyse_expr(context, init_expr))
                return false;
            decl->type = init_expr->type;
            if (!decl->alignment)
                decl->alignment = type_alloca_alignment(decl->type);
            if (!sema_analyse_decl_type(context, decl->type, init_expr->span))
                return false;
    ...
    if (!sema_resolve_type_info_maybe_inferred(context, decl->var.type_info, decl->var.init_expr != NULL))
        return decl_poison(decl);

    decl->type = decl->var.type_info->type;
    if (!sema_analyse_decl_type(context, decl->type, decl->var.type_info->span))
        return false;
...
static CompilationUnit *unit_copy(Module *module, CompilationUnit *unit)
{
    ...
    copy->module = module;
    assert(!unit->functions && !unit->macro_methods && !unit->methods && !unit->enums && !unit->ct_ifs &&
           !unit->types && !unit->external_symbol_list);
    return copy;
}
...
static bool sema_analyse_parameterized_define(SemaContext *c, Decl *decl)
{
    ...
    NameResolve name_resolve = {.path = decl_path, .span = span, .symbol = name};
    ...
    VECEACH(decl->define_decl.generic_params, i)
    {
        ...
        if (!sema_resolve_type_info(c, type_info))
            return decl_poison(decl);
        if (i != 0)
            scratch_buffer_append_char('.');
...
bool sema_analyse_decl(SemaContext *context, Decl *decl)
{
    ...
    {
    case DECL_BITSTRUCT:
        if (!sema_analyse_bitstruct(context, decl))
            return decl_poison(decl);
        decl_set_external_name(decl);
        break;
    case DECL_STRUCT:
    case DECL_UNION:
        if (!sema_analyse_struct_union(context, decl))
            return decl_poison(decl);
        decl_set_external_name(decl);
        break;
    case DECL_FUNC:
        if (!sema_analyse_func(context, decl))
            return decl_poison(decl);
        break;
    case DECL_MACRO:
    case DECL_GENERIC:
        if (!sema_analyse_macro(context, decl))
            return decl_poison(decl);
        break;
    case DECL_VAR:
        if (!sema_analyse_var_decl(context, decl, false))
            return decl_poison(decl);
        decl_set_external_name(decl);
        break;
    case DECL_DISTINCT:
        if (!sema_analyse_distinct(context, decl))
            return decl_poison(decl);
        decl_set_external_name(decl);
        break;
    case DECL_TYPEDEF:
        if (!sema_analyse_typedef(context, decl))
            return decl_poison(decl);
        break;
    case DECL_ENUM:
        if (!sema_analyse_enum(context, decl))
            return decl_poison(decl);
        decl_set_external_name(decl);
        break;
    case DECL_OPTENUM:
        if (!sema_analyse_error(context, decl))
            return decl_poison(decl);
        decl_set_external_name(decl);
        break;
    case DECL_DEFINE:
        if (!sema_analyse_define(context, decl))
            return decl_poison(decl);
        break;
    case DECL_ATTRIBUTE:
        TODO case DECL_POISONED : case DECL_IMPORT : case DECL_ENUM_CONSTANT : case DECL_CT_ELSE : case DECL_CT_ELIF
            : case DECL_LABEL : case DECL_CT_SWITCH : case DECL_CT_CASE : case DECL_CT_IF : case DECL_CT_ASSERT
            : case DECL_OPTVALUE : case DECL_DECLARRAY : case DECL_BODYPARAM : UNREACHABLE
    }
    decl->resolve_status = RESOLVE_DONE;
    return true;
}
