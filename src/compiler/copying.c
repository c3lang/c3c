...
#define SCOPE_FIXUP_START                                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        CopyFixup *current = c->current_fixup;
#define SCOPE_FIXUP_END                                                                                                \
    c->current_fixup = current;                                                                                        \
    }                                                                                                                  \
    while (0)
...
TypeInfo *copy_type_info(CopyStruct *c, TypeInfo *source)
{
    ...
    if (source->resolve_status == RESOLVE_DONE)
        return copy;
    switch (source->kind)
    {
...
Decl *copy_decl(CopyStruct *c, Decl *decl)
{
    ...
    case DECL_CT_CASE:
        ...
        break;
    case DECL_ATTRIBUTE:
        TODO case DECL_DEFINE : switch (decl->define_decl.define_kind)
        {
