...
bool llvm_emit_check_block_branch(GenContext *context)
{
    ...
    if (!context->current_block_is_target && !LLVMGetFirstUse(LLVMBasicBlockAsValue(context->current_block)))
    {
...
static inline void llvm_process_parameter_value(GenContext *c, Decl *decl, ABIArgInfo *info, unsigned *index)
{
    ...
    case ABI_ARG_EXPAND_COERCE: {
        ...
        LLVMValueRef gep_first =
            llvm_emit_struct_gep_raw(c, temp, coerce_type, info->coerce_expand.lo_index, alignment, &element_align);
        ...
        if (abi_type_is_valid(info->coerce_expand.hi))
        {
            LLVMValueRef gep_second =
                llvm_emit_struct_gep_raw(c, temp, coerce_type, info->coerce_expand.hi_index, alignment, &element_align);
    ...
    case ABI_ARG_DIRECT_PAIR: {
        ...
        if (llvm_store_size(c, struct_type) > type_size(decl->type))
        {
            ...
            if (decl_alignment < struct_alignment)
                decl->alignment = decl_alignment = struct_alignment;
            coerce = llvm_emit_alloca(c, struct_type, decl_alignment, "");
            decl->backend_ref = LLVMBuildBitCast(c->builder, coerce, llvm_get_ptr_type(c, decl->type),
...
static inline void llvm_emit_parameter(GenContext *context, Decl *decl, ABIArgInfo *abi_info, unsigned *index,
                                       unsigned real_index)
{
...
void llvm_emit_return_abi(GenContext *c, BEValue *return_value, BEValue *failable)
{
    ...
    case ABI_ARG_DIRECT_PAIR: {
        LLVMTypeRef coerce_type = llvm_get_coerce_type(c, info);
        if (coerce_type == llvm_get_type(c, call_return_type))
            goto DIRECT_RETURN;
...
void llvm_emit_function_body(GenContext *context, Decl *decl)
{
    ...
    LLVMValueRef alloca_point =
        LLVMBuildAlloca(context->builder, LLVMInt32TypeInContext(context->context), "alloca_point");
                                       ...
                                       int index, int last_index)
{
    assert(last_index == index || info->kind == ABI_ARG_DIRECT_PAIR || info->kind == ABI_ARG_IGNORE ||
           ...
           info->kind == ABI_ARG_DIRECT_COERCE_INT || info->kind == ABI_ARG_DIRECT_SPLIT_STRUCT);
    ...
    case ABI_ARG_INDIRECT:
        ...
        else
        {
            if (info->attributes.by_val)
                llvm_attribute_add_type(c, function, attribute_id.byval, llvm_get_type(c, info->indirect.type), index);
...
void llvm_emit_extern_decl(GenContext *context, Decl *decl)
{
    ...
    case DECL_FUNC:
        ...
        decl->backend_ref = LLVMAddFunction(context->module, name, llvm_get_type(context, decl->type));
