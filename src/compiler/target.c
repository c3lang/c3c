...
void llvm_dump(void)
{
    ...
    static char *os[] = {
        "unknown", "ananas",  "cloudabi", "darwin",  "dragonfly", "freebsd", "fuchsia", "ios",   "kfreebsd",
        "linux",   "lv2",     "macosx",   "netbsd",  "openbsd",   "solaris", "windows", "haiku", "minix",
        "rtems",   "nacl",    "cnk",      "aix",     "cuda",      "nvcl",    "amdhsa",  "ps4",   "elfiamcu",
        "tvos",    "watchos", "mesa3d",   "contiki", "amdpal",    "hermit",  "hurd",    "wasi",  "emscripten",
    };
    for (unsigned i = 0; i < sizeof(archs) / sizeof(void *); i++)
    {
        printf("----%s---\n", archs[i]);
        printf(
            "os         end  | ptr          | i8           | i16          | i32          | i64          | i128         "
            "| f16          | f32          | f64          | f128           "
            "\n");
        for (unsigned j = 0; j < sizeof(os) / sizeof(void *); j++)
        {
            ...
            if (LLVMGetTargetFromTriple(triplet, &target, &error))
                continue;
            LLVMTargetMachineRef machine = NULL;
            if (!(machine =
                      LLVMCreateTargetMachine(target, triplet, "", "", 0, LLVMRelocDefault, LLVMCodeModelDefault)))
            {
...
{
    return os_type == OS_TYPE_TVOS || os_type == OS_TYPE_WATCHOS || os_type == OS_TYPE_MACOSX || os_type == OS_TYPE_IOS;
}
...
static bool os_target_signed_c_char_type(OsType os, ArchType arch)
{
    ...
    case ARCH_TYPE_THUMBEB:
        if (os_is_apple(os) || os == OS_TYPE_WIN32)
            return true;
        return false;
    case ARCH_TYPE_PPC:
...
static inline void target_setup_arm_abi(void)
{
    ...
    {
        if (platform_target.environment_type == ENV_TYPE_EABI || platform_target.os == OS_TYPE_UNKNOWN /* or is M */)
        {
            platform_target.arm.variant = ARM_AAPCS;
            return;
        }
...
SET_ABI:
    ...
    case ARM_AAPCS_LINUX:
        if (platform_target.float_abi == FLOAT_ABI_HARD ||
            (platform_target.float_abi != FLOAT_ABI_SOFT && (platform_target.environment_type == ENV_TYPE_GNUEABIHF ||
...
static inline void target_setup_x86_abi(BuildTarget *target)
{
    ...
    if (platform_target.environment_type == ENV_TYPE_CYGNUS || platform_target.environment_type == ENV_TYPE_GNU)
    {
...
static inline void target_setup_x64_abi(BuildTarget *target)
{
    ...
    if (target->feature.soft_float == SOFT_FLOAT_YES)
        platform_target.x64.soft_float = true;
...
{
#define STRCASE(_str, _arch)                                                                                           \
    if (slicestrcmp(slice, _str))                                                                                      \
        return _arch;
...
#define STRCASE(_str, _os)                                                                                             \
    if (slicestrcmp(os_string, _str))                                                                                  \
        return _os;
...
static unsigned os_target_supports_float128(OsType os, ArchType arch)
{
    ...
    case ARCH_TYPE_PPC:
        if (os == OS_TYPE_MACOSX)
            return false; // Only for later OS X 10.4+
        return false;
    case ARCH_TYPE_X86:
        if (os == OS_TYPE_MACOSX)
            return true;
...
static AlignData os_target_alignment_of_int(OsType os, ArchType arch, uint32_t bits)
{
    ...
    case ARCH_TYPE_ARMB:
    case ARCH_TYPE_THUMBEB:
        if ((os_is_apple(os) || os == OS_TYPE_NETBSD) && bits > 32)
            return (AlignData){32, MIN(64u, bits)};
    ...
    case ARCH_TYPE_AARCH64:
    case ARCH_TYPE_AARCH64_BE:
        if (bits < 32 && !os_is_apple(os) && os != OS_TYPE_WIN32)
            return (AlignData){bits, 32};
        return (AlignData){bits, bits};
    case ARCH_TYPE_X86:
        if (bits < 32)
            return (AlignData){bits, bits};
        if (os == OS_TYPE_ELFIAMCU)
            return (AlignData){32, 32};
        if (os == OS_TYPE_WIN32 || os == OS_TYPE_NACL)
            return (AlignData){64, 64};
...
static AlignData os_target_alignment_of_float(OsType os, ArchType arch, uint32_t bits)
{
    ...
    case ARCH_UNSUPPORTED:
        UNREACHABLE
    case ARCH_TYPE_X86:
        if (os == OS_TYPE_ELFIAMCU && bits >= 32)
            return (AlignData){32, 32};
        if (os == OS_TYPE_WIN32 || os == OS_TYPE_NACL)
        {
            return (AlignData){bits, bits};
    ...
    case ARCH_TYPE_ARMB:
        ...
        return (AlignData){bits, bits};
    case ARCH_TYPE_X86_64:
        if (bits == 128 && os == OS_TYPE_ELFIAMCU)
            return (AlignData){32, 32};
        return (AlignData){bits, bits};
    }
    UNREACHABLE
...
void *llvm_target_machine_create(void)
{
    ...
    if (platform_target.arch == ARCH_TYPE_X86_64)
    {
        ...
        if (platform_target.x64.soft_float)
            scratch_buffer_append("+soft-float,");
    }
    char *features = scratch_buffer_to_string();
    size_t len = strlen(features);
    if (len > 0)
        features[len - 1] = 0;
    void *result = LLVMCreateTargetMachine(
        target, platform_target.target_triple, platform_target.cpu ? platform_target.cpu : "", features,
        (LLVMCodeGenOptLevel)platform_target.llvm_opt_level, reloc_mode, LLVMCodeModelDefault);
    if (!result)
        error_exit("Failed to create target machine.");
    return result;
}

#define INITIALIZE_TARGET(X)                                                                                           \
    do                                                                                                                 \
    {                                                                                                                  \
...
void target_setup(BuildTarget *target)
{
    ...
    if (target->arch_os_target == ARCH_OS_TARGET_DEFAULT)
        target->arch_os_target = default_target;
    ...
    platform_target.width_c_long_long =
        os_target_c_type_bits(platform_target.os, platform_target.arch, CTYPE_LONG_LONG);
    ...
    case ARCH_TYPE_RISCV64:
    case ARCH_TYPE_RISCV32:
        platform_target.riscv.xlen =
            arch_pointer_bit_width(platform_target.os, platform_target.arch) / 8; // pointer width
    ...
    platform_target.align_max_vector = os_arch_max_alignment_of_vector(
        platform_target.os, platform_target.arch, platform_target.environment_type, platform_target.arm.variant);
    platform_target.align_max_tls =
        os_arch_max_alignment_of_tls(platform_target.os, platform_target.arch, platform_target.environment_type);
    platform_target.reloc_model =
        arch_os_reloc_default(platform_target.arch, platform_target.os, platform_target.environment_type,
    ...
    if (target->reloc_model != RELOC_DEFAULT && (target->reloc_model != RELOC_NONE || !platform_target.pic_required))
    {
