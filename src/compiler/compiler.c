...
void compiler_compile(void)
{
    ...
    bool create_exe = !active_target.no_link && !active_target.test_output &&
                      (active_target.type == TARGET_TYPE_EXECUTABLE || active_target.type == TARGET_TYPE_TEST);
    ...
    if (create_exe)
    {
        ...
        {
            compiler_print_bench();
            if (!obj_format_linking_supported(platform_target.object_format) ||
                !linker(output_name, obj_files, output_file_count))
            {
...
static const char **target_expand_source_names(const char **dirs, const char *suffix1, const char *suffix2,
                                               bool error_on_mismatch)
{
    ...
    VECEACH(dirs, i)
    {
        ...
        if (strcmp(&name[name_len - len1], suffix1) != 0 &&
            (name_len < 5 || strcmp(&name[name_len - len2], suffix2) != 0))
            goto INVALID_NAME;
        vec_add(files, name);
        continue;
    INVALID_NAME:
...
void print_syntax(BuildOptions *options)
{
    ...
    if (options->print_operators)
    {
        ...
        for (int i = 1; i < TOKEN_LAST; i++)
        {
            if (i == TOKEN_DOCS_START || i == TOKEN_DOCS_END)
                continue;
            const char *name = token_type_to_string((TokenType)i);
            char first_char = name[0];
            if (first_char == '$' || first_char == '@' || (first_char >= 'a' && first_char <= 'z') ||
...
void compile()
{
    ...
    if (!vec_size(active_target.sources))
...
{
    uint32_t len_needed =
        (uint32_t)sprintf(&global_context.scratch_buffer[global_context.scratch_buffer_len], "%lld", (long long)i);
...
{
    uint32_t len_needed = (uint32_t)sprintf(&global_context.scratch_buffer[global_context.scratch_buffer_len], "%llu",
