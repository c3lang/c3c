...
#define add_arg(opt) vec_add(*args, opt)
    ...
    add_arg("-lmingw32");
    add_arg(join_strings(
        (const char *[]){root, "\\lib\\clang\\", LLVM_VERSION_STRING, "\\lib\\windows\\libclang_rt.builtins-x86_64.a"},
        4));
    add_arg("-lunwind");
    add_arg("-lmoldname");
    add_arg("-lmingwex");
...
static bool link_exe(const char *output_file, const char **files_to_link, unsigned file_count)
{
    ...
    case OS_TYPE_WIN32:
        ...
        if (NULL == getenv("MSYSTEM"))
        {
            ...
            // Studio\\2019\\BuildTools\\VC\\Tools\\MSVC\\14.28.29910\\lib\\x64 C:\\Program Files (x86)\\Microsoft
            // Visual Studio\\2019\\BuildTools\\VC\\Tools\\MSVC\\14.28.29910\\atlmfc\\lib\\x64 C:\\Program Files
            // (x86)\\Windows Kits\\10\\Lib\\10.0.19041.0\\ucrt\\x64 C:\\Program Files (x86)\\Windows
...
void platform_compiler(const char **files, unsigned file_count, const char *flags)
{
    ...
    const bool pie_set =
        flags != NULL && (strstr(flags, "-fno-PIE") || // This is a weird case, but probably don't set PIE if
                          strstr(flags, "-fno-pie") || // it is being set in user defined cflags.
                          strstr(flags, "-fpie") || strstr(flags, "-fPIE")); // strcasestr is apparently nonstandard >:(
    if (!pie_set)
    {
        append_fpie_pic_options(platform_target.reloc_model, &parts);
    }
