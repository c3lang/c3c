...
static inline bool ast_poison(Ast *ast)
{
...
{
    return expr_is_simple(exprptr(expr_id));
}
...
static inline bool expr_poison(Expr *expr)
{
...
#define RANGE_EXTEND_PREV(x)                                                                                           \
    do                                                                                                                 \
    {                                                                                                                  \
        (x)->span = extend_span_with_token((x)->span, c->prev_span);                                                   \
    } while (0)
...
#define DECL_TYPE_KIND_REAL(k_, t_)                                                                                    \
    ...
    if (k_ == TYPE_TYPEDEF)                                                                                            \
        k_ = (t_)->canonical->type_kind;

#define IS_FAILABLE(element_) (type_is_failable((element_)->type))
#define type_is_failable(type_)                                                                                        \
    ((type_)->type_kind == TYPE_FAILABLE || (type_)->canonical->type_kind == TYPE_FAILABLE_ANY)
...
{
    Type *type = expr->type;
    if (!add_failable || type->type_kind == TYPE_FAILABLE)
        return type;
...
static inline bool type_is_builtin(TypeKind kind)
{
    return kind >= TYPE_VOID && kind <= TYPE_TYPEID;
}
static inline bool type_kind_is_signed(TypeKind kind)
{
...
static inline bool type_kind_is_unsigned(TypeKind kind)
{
    return kind >= TYPE_U8 && kind <= TYPE_U128;
}
static inline bool type_kind_is_any_integer(TypeKind kind)
{
    return kind >= TYPE_I8 && kind <= TYPE_U128;
}
static inline bool type_is_signed(Type *type)
{
    return type->type_kind >= TYPE_I8 && type->type_kind < TYPE_U8;
}
static inline bool type_is_unsigned(Type *type)
{
...
static inline bool type_info_ok(TypeInfo *type_info)
{
    return !type_info || type_info->kind != TYPE_INFO_POISON;
}
...
{
    DeclKind kind = decl->decl_kind;
    return (kind == DECL_UNION) | (kind == DECL_STRUCT) | (kind == DECL_BITSTRUCT) | (kind == DECL_ENUM) |
           (kind == DECL_DISTINCT);
}

static inline DeclKind decl_from_token(TokenType type)
{
    if (type == TOKEN_STRUCT)
        return DECL_STRUCT;
    if (type == TOKEN_UNION)
        return DECL_UNION;
    if (type == TOKEN_BITSTRUCT)
        return DECL_BITSTRUCT;
    UNREACHABLE
}
...
#define ASSIGN_AST_OR_RET(_assign, _ast_stmt, _res)                                                                    \
    Ast *TEMP(_ast) = (_ast_stmt);                                                                                     \
    if (!ast_ok(TEMP(_ast)))                                                                                           \
        return _res;                                                                                                   \
    _assign = TEMP(_ast)
#define ASSIGN_ASTID_OR_RET(_assign, _ast_stmt, _res)                                                                  \
    Ast *TEMP(_ast) = (_ast_stmt);                                                                                     \
    if (!ast_ok(TEMP(_ast)))                                                                                           \
        return _res;                                                                                                   \
    _assign = astid(TEMP(_ast))
#define ASSIGN_EXPR_OR_RET(_assign, _expr_stmt, _res)                                                                  \
    Expr *TEMP(_expr) = (_expr_stmt);                                                                                  \
    if (!expr_ok(TEMP(_expr)))                                                                                         \
        return _res;                                                                                                   \
    _assign = TEMP(_expr)
#define ASSIGN_EXPRID_OR_RET(_assign, _expr_stmt, _res)                                                                \
    Expr *TEMP(_expr) = (_expr_stmt);                                                                                  \
    if (!expr_ok(TEMP(_expr)))                                                                                         \
        return _res;                                                                                                   \
    _assign = exprid(TEMP(_expr))
#define ASSIGN_TYPE_OR_RET(_assign, _type_stmt, _res)                                                                  \
    TypeInfo *TEMP(_type) = (_type_stmt);                                                                              \
    if (!type_info_ok(TEMP(_type)))                                                                                    \
        return _res;                                                                                                   \
    _assign = TEMP(_type)
#define ASSIGN_DECL_OR_RET(_assign, _decl_stmt, _res)                                                                  \
    Decl *TEMP(_decl) = (_decl_stmt);                                                                                  \
    if (!decl_ok(TEMP(_decl)))                                                                                         \
        return _res;                                                                                                   \
    _assign = TEMP(_decl)
