...
static inline bool llvm_value_is_addr(BEValue *value)
{
    return value->kind == BE_ADDRESS || value->kind == BE_ADDRESS_FAILABLE;
}
static inline bool llvm_value_is_bool(BEValue *value)
{
...
void llvm_attribute_add_range(GenContext *c, LLVMValueRef value_to_add_attribute_to, unsigned attribute,
...
void llvm_attribute_add_string(GenContext *c, LLVMValueRef value_to_add_attribute_to, const char *attribute,
                               const char *value, int index);
void llvm_attribute_add_type(GenContext *c, LLVMValueRef value_to_add_attribute_to, unsigned attribute,
                             LLVMTypeRef type, int index);
void llvm_attribute_add_int(GenContext *c, LLVMValueRef value_to_add_attribute_to, unsigned attribute, uint64_t val,
                            int index);
...
LLVMValueRef llvm_emit_memclear_size_align(GenContext *c, LLVMValueRef ref, uint64_t size, AlignSize align,
                                           bool bitcast);
void llvm_store_zero(GenContext *c, BEValue *ref);
void llvm_emit_memcpy(GenContext *c, LLVMValueRef dest, unsigned dest_align, LLVMValueRef source, unsigned src_align,
                      uint64_t len);
...
static inline LLVMValueRef decl_failable_ref(Decl *decl)
{
    ...
    if (decl->var.kind == VARDECL_UNWRAPPED)
        return decl_failable_ref(decl->var.alias);
...
static inline LLVMValueRef decl_ref(Decl *decl)
{
    if (decl->decl_kind == DECL_VAR && decl->var.kind == VARDECL_UNWRAPPED)
        return decl_ref(decl->var.alias);
...
static inline LLVMValueRef llvm_emit_bitcast(GenContext *context, LLVMValueRef value, Type *type)
{
    ...
    if (result_type == LLVMTypeOf(value))
        return value;
...
static inline bool llvm_use_debug(GenContext *context)
{
...
#define EMIT_LOC(c, x)                                                                                                 \
    ...
    {                                                                                                                  \
        if (c->debug.builder)                                                                                          \
            llvm_emit_debug_location(c, x->span);                                                                      \
    } while (0);
