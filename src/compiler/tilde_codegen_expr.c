...
static void tilde_emit_parameter(TbContext *c, TB_Reg **args, ABIArgInfo *info, TBEValue *be_value, Type *type)
{
    ...
    case ABI_ARG_DIRECT_COERCE: {
        ...
        TODO vec_add(*args, tilde_load_value(c, be_value));
        return;
        /*
...
void tilde_emit_call_expr(TbContext *c, TBEValue *result_value, Expr *expr)
{
    ...
    *result_value = (TBEValue){.kind = TBE_VALUE, .reg = 0};
    ...
    if (prototype->ret_by_ref)
    {
        ...
        tilde_emit_parameter(c, &values, prototype->ret_by_ref_abi_info, &synthetic_return_param,
    ...
    if (prototype->variadic == VARIADIC_TYPED)
    {
        ...
        else
        {
            ...
            for (unsigned i = non_variadic_params; i < arguments; i++)
            {
                ...
                alignment, &store_alignment); llvm_store_value_aligned(c, slot, &temp_value, store_alignment);*/
            }
            TODO
            /*
            BEValue len_addr;
    ...
    if (prototype->call_abi)
    {
        ...
        // platform_target.arch, platform_target.os));
    }
    ...
    for (unsigned i = 0; i < non_variadic_params; i++)
    {
        ...
        switch (info->kind)
        {
            case ABI_ARG_INDIRECT:
                ...
                {
                    // llvm_attribute_add_call_type(c, call_value, attribute_id.byval, (int)i + 1, llvm_get_type(c,
    info->indirect.type));
                }
        ...
        case ABI_ARG_EXPAND:
            ...
            case ABI_ARG_IGNORE:
                ...
                *result_value = (BEValue) { .type = type_void, .kind = BE_VALUE };
                return;
                case ABI_ARG_INDIRECT:
                    llvm_attribute_add_call_type(c, call_value, attribute_id.sret, 1, llvm_get_type(c,
    ret_info->indirect.type)); llvm_attribute_add_call(c, call_value, attribute_id.align, 1,
    ret_info->indirect.alignment);
                    ...
                    case ABI_ARG_DIRECT_PAIR:
                    {
                        ...
                        llvm_emit_convert_value_from_coerced(c, result_value, struct_type, call_value,
    call_return_type); break;
                    }
                    case ABI_ARG_EXPAND_COERCE:
                    {
                        ...
                        // 15b. "Convert" this return type pointer in memory to our coerce type which is { pad, lo, pad,
    hi } LLVMTypeRef coerce_type = llvm_get_coerce_type(c, ret_info); LLVMValueRef coerce = LLVMBuildBitCast(c->builder,
    ret, coerce_type, "");

                        // 15d. Find the address to the low value
                        AlignSize alignment;
                        LLVMValueRef lo = llvm_emit_struct_gep_raw(c, coerce, coerce_type,
    ret_info->coerce_expand.lo_index, type_abi_alignment(call_return_type), &alignment);
                        ...
                        LLVMValueRef hi = llvm_emit_struct_gep_raw(c, coerce, coerce_type,
    ret_info->coerce_expand.hi_index, type_abi_alignment(call_return_type), &alignment);
