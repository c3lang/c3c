...
LLVMValueRef llvm_emit_const_initializer(GenContext *c, ConstInitializer *const_init)
{
    ...
    case CONST_INIT_ARRAY_FULL: {
        ...
        for (MemberIndex i = 0; i < size; i++)
        {
            ...
            if (element_type_llvm != LLVMTypeOf(element))
                was_modified = true;
    ...
    case CONST_INIT_ARRAY: {
        ...
        VECEACH(elements, i)
        {
            ...
            if (LLVMTypeOf(value) == element_type_llvm)
                was_modified = true;
            vec_add(parts, value);
...
void llvm_emit_ptr_from_array(GenContext *c, BEValue *value)
{
    ...
    case TYPE_SUBARRAY: {
        ...
        llvm_value_set_address_align(value, member.value, type_get_ptr(value->type->array.base),
...
void llvm_emit_global_variable_init(GenContext *c, Decl *decl)
{
    ...
    if (init_expr)
    {
        ...
        else
        {
            ...
            {
                init_value = LLVMConstStringInContext(c->context, init_expr->const_expr.bytes.ptr,
                                                      init_expr->const_expr.bytes.len, 1);
            }
            else
            {
    ...
    decl->backend_ref =
        LLVMAddGlobal(c->module, LLVMTypeOf(init_value), decl->extname ? decl->extname : decl->external_name);
    ...
    case VISIBLE_MODULE:
        ...
        if (failable_ref)
            LLVMSetVisibility(failable_ref, LLVMProtectedVisibility);
        break;
    case VISIBLE_PUBLIC:
        LLVMSetVisibility(decl->backend_ref, LLVMDefaultVisibility);
        if (failable_ref)
            LLVMSetVisibility(failable_ref, LLVMDefaultVisibility);
        break;
    case VISIBLE_EXTERN:
        LLVMSetLinkage(decl->backend_ref, LLVMExternalLinkage);
        if (failable_ref)
            LLVMSetLinkage(failable_ref, LLVMExternalLinkage);
        // LLVMSetVisibility(decl->backend_ref, LLVMDefaultVisibility);
        break;
    case VISIBLE_LOCAL:
        LLVMSetVisibility(decl->backend_ref, LLVMHiddenVisibility);
        if (failable_ref)
            LLVMSetVisibility(failable_ref, LLVMHiddenVisibility);
        break;
    }
...
void llvm_emit_introspection_type_from_decl(GenContext *c, Decl *decl)
{
    ...
    if (decl_is_enum_kind(decl))
    {
        ...
        for (unsigned i = 0; i < elements; i++)
        {
            ...
            decl->enums.values[i]->backend_ref =
                llvm_emit_array_gep_raw(c, enum_elements, elements_type, i, alignment, &store_align);
        }
    }
...
void llvm_value_rvalue(GenContext *c, BEValue *value)
{
    ...
    llvm_value_fold_failable(c, value);
    value->value = llvm_emit_load_aligned(c, llvm_get_type(c, value->type), value->value,
                                          value->alignment ? value->alignment : type_abi_alignment(value->type), "");
...
const char *llvm_codegen(void *context)
{
    ...
    LLVMPassManagerBuilderSetDisableUnrollLoops(pass_manager_builder,
...
void llvm_attribute_add_int(GenContext *context, LLVMValueRef value_to_add_attribute_to, unsigned attribute,
                            uint64_t val, int index)
{
...
void llvm_attribute_add_range(GenContext *context, LLVMValueRef value_to_add_attribute_to, unsigned attribute,
...
void llvm_attribute_add_string(GenContext *context, LLVMValueRef value_to_add_attribute_to, const char *attribute,
                               const char *value, int index)
{
    LLVMAttributeRef llvm_attr = LLVMCreateStringAttribute(context->context, attribute, (unsigned)strlen(attribute),
                                                           value, (unsigned)strlen(value));
    LLVMAddAttributeAtIndex(value_to_add_attribute_to, (LLVMAttributeIndex)index, llvm_attr);
}
...
LLVMValueRef llvm_store_bevalue_aligned(GenContext *c, LLVMValueRef destination, BEValue *value, AlignSize alignment)
{
    ...
    case BE_BOOLEAN:
        ...
        FALLTHROUGH;
    case BE_VALUE:
        return llvm_store_aligned(c, destination, value->value,
...
void llvm_emit_memcpy_to_decl(GenContext *c, Decl *decl, LLVMValueRef source, unsigned source_alignment)
{
    if (source_alignment == 0)
        source_alignment = type_abi_alignment(decl->type);
    assert(!decl->is_value);
    llvm_emit_memcpy(c, decl->backend_ref, decl->alignment, source, source_alignment, type_size(decl->type));
}

LLVMValueRef llvm_emit_load_aligned(GenContext *c, LLVMTypeRef type, LLVMValueRef pointer, AlignSize alignment,
                                    const char *name)
{
    assert(c->builder);
