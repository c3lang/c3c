                                           ...
                                           bool bitcast)
{
    ...
    LLVMValueRef target =
        bitcast ? LLVMBuildBitCast(c->builder, ref, llvm_get_type(c, type_get_ptr(type_char)), "") : ref;
...
void llvm_emit_global_variable_init(GenContext *c, Decl *decl)
{
    ...
    else
    {
        ...
        if (decl->visibility == VISIBLE_LOCAL || decl->visibility == VISIBLE_MODULE)
            addr = LLVMGlobalUnnamedAddr;
    ...
    case VISIBLE_LOCAL:
        ...
        if (failable_ref)
            LLVMSetLinkage(failable_ref, LLVMInternalLinkage);
        break;
    }
