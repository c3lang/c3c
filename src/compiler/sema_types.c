...
bool sema_resolve_array_like_len(SemaContext *context, TypeInfo *type_info, ArraySize *len_ref)
{
    ...
    if (!sema_analyse_expr(context, len_expr))
        return type_info_poison(type_info);
...
static inline bool sema_resolve_array_type(SemaContext *context, TypeInfo *type, bool shallow)
{
    ...
    case TYPE_INFO_VECTOR: {
        ArraySize width;
        if (!sema_resolve_array_like_len(context, type, &width))
            return type_info_poison(type);
    ...
    case TYPE_INFO_ARRAY: {
        ArraySize size;
        if (!sema_resolve_array_like_len(context, type, &size))
            return type_info_poison(type);
...
bool sema_resolve_type_shallow(SemaContext *context, TypeInfo *type_info, bool allow_inferred_type, bool in_shallow)
{
    if (type_info->resolve_status == RESOLVE_DONE)
        return type_info_ok(type_info);

    if (type_info->resolve_status == RESOLVE_RUNNING)
    {
        // TODO this is incorrect for unresolved expressions
        SEMA_ERROR(type_info, "Circular dependency resolving type '%s'.", type_info->unresolved.name);
        return type_info_poison(type_info);
    }
...
RETRY:
    ...
    case TYPE_INFO_IDENTIFIER:
        if (!sema_resolve_type_identifier(context, type_info))
            return false;
        break;
    ...
    case TYPE_INFO_ARRAY:
    case TYPE_INFO_VECTOR:
        if (!sema_resolve_array_type(context, type_info, in_shallow))
            return false;
        break;
    case TYPE_INFO_POINTER:
        if (!sema_resolve_ptr_type(context, type_info))
            return false;
        break;
    }
APPEND_QUALIFIERS:
    ...
    if (type_info->failable)
    {
        ...
        if (!type_is_failable(type))
            type_info->type = type_get_failable(type);
    }
    return true;
}
...
{
    if (type->type_kind != TYPE_INFERRED_ARRAY)
        return type;
