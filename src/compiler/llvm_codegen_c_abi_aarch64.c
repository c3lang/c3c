...
ABIArgInfo *aarch64_coerce_illegal_vector(Type *type){TODO}
...
ABIArgInfo *aarch64_classify_argument_type(Type *type)
{
    ...
    if (size <= 16)
    {
        ...
        if (alignment == 16)
            return abi_arg_new_direct_coerce_type(type_u128);
        ArraySize m = size / alignment;
        if (m > 1)
            return abi_arg_new_direct_coerce_type(type_get_array(type_ulong, m));
...
void c_abi_func_create_aarch64(FunctionPrototype *prototype)
{
    ...
    prototype->ret_abi_info =
        aarch64_classify_return_type(prototype->abi_ret_type, prototype->variadic == VARIADIC_RAW);
    if (prototype->ret_by_ref)
    {
        prototype->ret_by_ref_abi_info =
            aarch64_classify_argument_type(type_get_ptr(type_flatten(prototype->ret_by_ref_type)));
    }
