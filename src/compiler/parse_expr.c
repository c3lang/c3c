...
static Expr *parse_grouping_expr(ParseContext *c, Expr *left)
{
    ...
    if (expr->inner_expr->expr_kind == EXPR_TYPEINFO)
    {
        ...
        {
            SEMA_ERROR_HERE("Unexpected start of a block '{' here. If you intended a compound literal, remove the () "
                            "around the type.");
            return poisoned_expr;
        }
...
Expr *parse_initializer_list(ParseContext *c, Expr *left)
{
    ...
    {
        Expr **exprs = NULL;
        if (!parse_arg_list(c, &exprs, TOKEN_RBRACE, NULL))
            return poisoned_expr;
        int designated = -1;
        VECEACH(exprs, i)
        {
            ...
            if (expr->expr_kind == EXPR_DESIGNATOR)
            {
                if (designated == 0)
                {
                    SEMA_ERROR(expr, "Designated initialization with '[] = ...' and '.param = ...' cannot be mixed "
...
static Expr *parse_call_expr(ParseContext *c, Expr *left)
{
    ...
    if (!tok_is(c, TOKEN_RPAREN))
    {
        ...
        if (!parse_arg_list(c, &params, TOKEN_RPAREN, &unsplat))
            return poisoned_expr;
    }
    if (try_consume(c, TOKEN_EOS) && !tok_is(c, TOKEN_RPAREN))
    {
        ...
        if (!parse_parameters(c, VISIBLE_LOCAL, &body_args))
            return poisoned_expr;
    }
    if (!tok_is(c, TOKEN_RPAREN))
    {
...
static Expr *parse_try_expr(ParseContext *c, Expr *left)
{
    ...
    if (!try_consume(c, TOKEN_LPAREN))
    {
        if (is_try)
        {
            SEMA_ERROR(
                try_expr,
                "An unwrapping 'try' can only occur as the last element of a conditional, did you want 'try(expr)'?");
            return poisoned_expr;
        }
        else
        {
            SEMA_ERROR(try_expr, "An unwrapping 'catch' can only occur as the last element of a conditional, did you "
                                 "want 'catch(expr)'?");
            return poisoned_expr;
        }
    }
...
static Expr *parse_integer(ParseContext *c, Expr *left)
{
    ...
    else
    {
        if (hex_characters)
        {
            ...
            if (type_bits > 128)
            {
                SEMA_ERROR_HERE("%d hex digits indicates a bit width over 128, which is not supported.",
                                hex_characters);
                return poisoned_expr;
            }
        }
        if (oct_characters)
        {
            ...
            if (type_bits > 128)
            {
                SEMA_ERROR_HERE("%d octal digits indicates a bit width over 128, which is not supported.",
                                oct_characters);
                return poisoned_expr;
            }
        }
        if (binary_characters)
        {
            ...
            if (type_bits > 128)
            {
                SEMA_ERROR_HERE("%d binary digits indicates a bit width over 128, which is not supported.",
                                binary_characters);
                return poisoned_expr;
            }
        }
        if (type_bits && type_bits < 8)
            type_bits = 8;
        if (type_bits && !is_power_of_two((uint64_t)type_bits))
            type_bits = (int)next_highest_power_of_2((uint32_t)type_bits);
    }
    if (type_bits)
        expr_int->const_expr.is_hex = false;
    if (!type_bits)
    {
    ...
    if (type_bits)
    {
        type = is_unsigned ? type_int_unsigned_by_bitsize((unsigned)type_bits)
    ...
    if (!int_fits(expr_int->const_expr.ixx, type->type_kind))
    {
        ...
        if (type_bits)
        {
            SEMA_ERROR_HERE("'%s' does not fit in a '%c%d' literal.", i128_to_string(i, radix, true),
                            is_unsigned ? 'u' : 'i', type_bits);
        }
        else
        {
            SEMA_ERROR_HERE("'%s' does not fit in an %s literal.", i128_to_string(i, radix, true),
                            is_unsigned ? "unsigned int" : "int");
        }
        return poisoned_expr;
    }
    expr_int->type = type;
...
static void parse_hex(char *result_pointer, const char *data, const char *end)
{
    ...
    while (data < end)
    {
        int val;
        int val2;
        while ((val = char_to_nibble(*(data++))) < 0)
            if (data == end)
                goto DONE;
...
static void parse_base64(char *result_pointer, char *result_pointer_end, const char *data, const char *end)
{
    ...
    while (data < end)
    {
        ...
        int val3;
        int val4;
        while ((val = base64_to_sextet(*(data++))) < 0)
            if (data == end)
                goto DONE;
        ...
        if (data_current < result_pointer_end)
            *(data_current++) = (char)((triplet >> 16) & 0xFF);
        if (data_current < result_pointer_end)
            *(data_current++) = (char)((triplet >> 8) & 0xFF);
        if (data_current < result_pointer_end)
            *(data_current++) = (char)(triplet & 0xFF);
    }
DONE:;
}
