...
static void sema_unwrappable_from_catch_in_else(SemaContext *c, Expr *cond)
{
    ...
    if (!last || last->expr_kind != EXPR_CATCH_UNWRAP)
        return;
...
static inline bool assert_create_from_contract(SemaContext *context, Ast *directive, AstId **asserts,
                                               SourceSpan evaluation_location)
{
    ...
    VECEACH(exprs, j)
    {
        ...
        if (!comment)
            comment = directive->doc_stmt.contract.expr_string;
...
static inline bool sema_analyse_block_exit_stmt(SemaContext *context, Ast *statement)
{
    ...
    if (statement->return_stmt.expr)
    {
        if (block_type)
        {
            if (!sema_analyse_expr_rhs(context, block_type, statement->return_stmt.expr, type_is_failable(block_type)))
                return false;
        }
        else
        {
    ...
    statement->return_stmt.cleanup =
        context_get_defers(context, context->active_scope.defer_last, context->block_return_defer);
...
static inline bool sema_analyse_return_stmt(SemaContext *context, Ast *statement)
{
    ...
    if (return_expr)
    {
        if (!sema_analyse_expr_rhs(context, expected_rtype, return_expr, type_is_failable(expected_rtype)))
            return false;
    }
    else
    {
    ...
    if (context->ensures)
    {
        ...
        while (doc_directive)
        {
            ...
            {
                if (!assert_create_from_contract(context, directive, &append_id, statement->span))
                    return false;
            }
...
static inline bool sema_analyse_try_unwrap(SemaContext *context, Expr *expr)
{
    ...
    if (!failable)
    {
        if (!sema_analyse_expr(context, ident))
            return false;
        if (ident->expr_kind != EXPR_IDENTIFIER)
        {
            SEMA_ERROR(
                ident,
                "Only single identifiers may be unwrapped using 'try var', maybe you wanted 'try (expr)' instead?");
            return false;
        }
        ...
        if (!IS_FAILABLE(decl))
        {
            ...
            {
                SEMA_ERROR(ident, "This variable is already unwrapped, so you cannot use 'try' on it again, please "
                                  "remove the 'try'.");
                return false;
            }
            SEMA_ERROR(ident, "Expected this variable to be a failable, otherwise it can't be used for unwrap, maybe "
                              "you didn't intend to use 'try'?");
            return false;
        }
    ...
    if (var_type)
    {
        if (!sema_resolve_type_info(context, var_type))
            return false;
    ...
    if (!var_type && !implicit_declaration)
    {
        ...
        if (!sema_analyse_expr_lvalue(context, ident))
            return false;

        // 3b. Make sure it's assignable
        if (!sema_expr_check_assign(context, ident))
            return false;
        ...
        if (IS_FAILABLE(ident))
        {
            if (ident->expr_kind == EXPR_IDENTIFIER)
            {
                SEMA_ERROR(ident, "This is a failable variable, you should only have non-failable variables on the "
                                  "left side unless you use 'try' without '='.");
            }
            else
            {
        ...
        if (!sema_analyse_expr(context, failable))
            return false;
        ...
        if (!cast_implicit(failable, ident->type))
            return false;
    ...
    else
    {
        ...
        if (var_type)
        {
            if (!cast_implicit(failable, var_type->type))
                return false;
        }
...
static inline bool sema_analyse_try_unwrap_chain(SemaContext *context, Expr *expr, CondType cond_type)
{
    ...
    VECEACH(expr->try_unwrap_chain_expr, i)
    {
        ...
        {
            if (!sema_analyse_try_unwrap(context, chain_element))
                return false;
            continue;
        }
        if (!sema_analyse_cond_expr(context, chain_element))
            return false;
    }
    expr->type = type_bool;
    expr->resolve_status = RESOLVE_DONE;
...
static inline bool sema_analyse_catch_unwrap(SemaContext *context, Expr *expr)
{
    ...
    if (!type && !implicit_declaration)
    {
        ...
        {
            SEMA_ERROR(ident, "Expected the variable to have the type %s, not %s.",
                       type_quoted_error_string(type_anyerr), type_quoted_error_string(type->type));
            return false;
        }
    ...
    else
    {
        ...
        if (!sema_resolve_type_info(context, type))
            return false;
...
RESOLVE_EXPRS:;
    ...
    {
        Expr *fail = exprs[i];
        if (!sema_analyse_expr(context, fail))
            return false;
...
static void sema_remove_unwraps_from_try(SemaContext *c, Expr *cond)
{
    assert(cond->expr_kind == EXPR_COND);
    Expr *last = VECLAST(cond->cond_expr);
    if (!last || last->expr_kind != EXPR_TRY_UNWRAP_CHAIN)
        return;
    Expr **chain = last->try_unwrap_chain_expr;
    VECEACH(chain, i)
    {
        Expr *expr = chain[i];
        if (expr->expr_kind != EXPR_TRY_UNWRAP)
            continue;
...
static inline bool sema_analyse_last_cond(SemaContext *context, Expr *expr, CondType cond_type)
{
    ...
    case EXPR_CATCH_UNWRAP:
        if (cond_type != COND_TYPE_UNWRAP_BOOL && cond_type != COND_TYPE_UNWRAP)
        {
            SEMA_ERROR(expr, "Catch unwrapping is only allowed inside of a 'while' or 'if' conditional, maybe "
                             "catch(...) will do what you need?");
            return false;
        }
    ...
    if (cond_type != COND_TYPE_EVALTYPE_VALUE)
        goto NORMAL_EXPR;
    ...
    if (expr->expr_kind == EXPR_BINARY && expr->binary_expr.operator== BINARYOP_ASSIGN)
    {
        ...
        if (left->resolve_status == RESOLVE_DONE || left->expr_kind != EXPR_IDENTIFIER || left->identifier_expr.path)
            goto NORMAL_EXPR;
        ...
        if (is_deref)
            right = right->unary_expr.expr;
        if (!sema_analyse_expr_rhs(context, NULL, right, false))
            return false;
...
static inline bool sema_analyse_cond_list(SemaContext *context, Expr *expr, CondType cond_type)
{
    ...
    {
        if (!sema_analyse_expr(context, dexprs[i]))
            return false;
    }

    if (!sema_analyse_last_cond(context, dexprs[entries - 1], cond_type))
        return false;
...
static inline bool sema_analyse_cond(SemaContext *context, Expr *expr, CondType cond_type)
{
    ...
    if (!sema_analyse_cond_list(context, expr, cond_type))
        return false;
    ...
    {
        SEMA_ERROR(expr,
                   cast_to_bool ? "Expected a boolean expression." : "Expected an expression resulting in a value.");
        return false;
    }
    ...
    if (cast_to_bool)
    {
        if (!cast_implicit(last, type_bool))
            return false;
    }
    return true;
}
...
static inline bool sema_analyse_declare_stmt(SemaContext *context, Ast *statement)
{
    ...
    {
        if (!sema_analyse_var_decl_ct(context, statement->declare_stmt))
            return false;
...
{
    if (!sema_analyse_expr(context, statement->expr_stmt))
        return false;
    return true;
}
...
{
    // TODO special parsing of "catch"
    if (!sema_analyse_defer_stmt_body(context, statement, astptr(statement->defer_stmt.body)))
        return false;
...
static Expr *sema_insert_method_macro_call(SemaContext *context, SourceSpan span, Decl *method_decl, Expr *parent,
                                           Expr **arguments)
{
    ...
    if (!is_macro)
    {
        if (parent->type->type_kind != TYPE_POINTER)
            expr_insert_addr(parent);
    }
    if (!sema_expr_analyse_general_call(context, len_call, method_decl, parent, is_macro, false))
...
static inline bool sema_analyse_foreach_stmt(SemaContext *context, Ast *statement)
{
    ...
    while (enumerator->expr_kind == EXPR_GROUP)
        enumerator = enumerator->inner_expr;
    ...
    if (expr_is_init_list(enumerator) ||
        (enumerator->expr_kind == EXPR_CONST && enumerator->const_expr.const_kind == CONST_LIST))
    {
        bool may_be_array;
        bool is_const_size;
        ...
        if (!sema_resolve_type_info(context, variable_type_info))
            return false;
    ...
    if (flattened_type->type_kind == TYPE_POINTER)
    {
        ...
        {
            SEMA_ERROR(enumerator, "It is not possible to enumerate an expression of type %s.",
    ...
    if (value_type && value_by_ref)
        value_type = type_get_ptr(value_type);
    ...
    if (!value_type)
    {
        ...
        {
            SEMA_ERROR(enumerator, "It's not possible to enumerate an expression of type %s.",
                       type_quoted_error_string(enumerator->type));
            return false;
        }
        if (!by_ref && value_by_ref)
        {
            SEMA_ERROR(enumerator, "%s does not support 'foreach' with the value by reference.",
                       type_quoted_error_string(enumerator->type));
            return false;
        }
    ...
    if (!sema_resolve_type_info(context, var->var.type_info))
        return false;
    ...
    if (index)
    {
        if (!index->var.type_info)
            index->var.type_info = type_info_new_base(index_type, enumerator->span);
        if (!sema_resolve_type_info(context, index->var.type_info))
            return false;
        ...
        {
            SEMA_ERROR(index->var.type_info, "Index must be an integer type, '%s' is not valid.",
    ...
    if (is_addr)
        expr_insert_deref(enum_val);
    Type *enumerator_type = type_flatten(enum_val->type);
    Expr *len_call;
    ArraySize array_len = 0;
    if (len)
    {
        ASSIGN_EXPR_OR_RET(len_call, sema_insert_method_macro_call(context, enumerator->span, len, enum_val, NULL),
                           false);
    }
    else
    {
        ...
        else
        {
            ...
            if (!sema_analyse_expr(context, enum_val))
                return false;
    ...
    if (len_call)
    {
        ...
        if (!cast_implicit(len_call, idx_init->type))
            return false;
    ...
    if (index)
    {
        ...
        if (!cast(load_idx, index_var_type))
            return false;
...
static inline bool sema_analyse_if_stmt(SemaContext *context, Ast *statement)
{
    ...
    SCOPE_OUTER_START
    CondType cond_type = then->ast_kind == AST_IF_CATCH_SWITCH_STMT ? COND_TYPE_UNWRAP : COND_TYPE_UNWRAP_BOOL;
    success = sema_analyse_cond(context, cond, cond_type);

    if (success && !ast_ok(then))
    {
        SEMA_ERROR(
            then, "The 'then' part of a single line if-statement must start on the same line as the 'if' or use '{ }'");
        success = false;
    }
    ...
    if (success && else_body)
    {
        ...
        if (success && else_body->ast_kind != AST_COMPOUND_STMT && else_body->ast_kind != AST_IF_STMT)
        {
            SEMA_ERROR(else_body, "An 'else' must use '{ }' even around a single statement.");
            success = false;
        }
    }
...
{
    if (!sema_analyse_expr(context, stmt->asm_stmt.body))
        return false;
    if (stmt->asm_stmt.body->expr_kind != EXPR_CONST || stmt->asm_stmt.body->const_expr.const_kind != CONST_STRING)
    {
...
static inline Decl *sema_analyse_label(SemaContext *context, Ast *stmt)
{
    ...
    if (context->active_scope.in_defer)
    {
        ...
        {
            SEMA_ERROR(stmt, stmt->ast_kind == AST_BREAK_STMT ? "You cannot break out of a defer."
                                                              : "You cannot use continue out of a defer.");
            return poisoned_decl;
        }
    }
    return target;
...
static bool context_labels_exist_in_scope(SemaContext *context)
{
    ...
    {
        if (locals[local - 1]->decl_kind == DECL_LABEL)
            return true;
    }
    return false;
}
...
static bool sema_analyse_nextcase_stmt(SemaContext *context, Ast *statement)
{
    ...
    if (statement->nextcase_stmt.label.name)
    {
        ...
        if (!target)
        {
            SEMA_ERROR(statement, "Cannot find a switch statement with the name '%s'.",
                       statement->nextcase_stmt.label.name);
            return false;
        }
        if (target->decl_kind != DECL_LABEL)
        {
        ...
        {
            SEMA_ERROR(&statement->nextcase_stmt.label,
                       "Expected the label to match a 'switch' or 'if-catch' statement.");
            return false;
        }
    ...
    if (!statement->nextcase_stmt.expr)
    {
        ...
        statement->nextcase_stmt.defer_id =
            context_get_defers(context, context->active_scope.defer_last, parent->switch_stmt.defer);
    ...
    if (target->expr_kind == EXPR_CONST)
    {
        ...
        VECEACH(parent->switch_stmt.cases, i)
        {
            ...
            ExprConst *to_const_expr =
                case_stmt->case_stmt.to_expr ? &case_stmt->case_stmt.to_expr->const_expr : const_expr;
...
static bool sema_analyse_continue_stmt(SemaContext *context, Ast *statement)
{
    ...
    if (statement->contbreak_stmt.label.name)
    {
        ...
        case AST_FOR_STMT:
            // Is this plain "do"?
            if (parent->for_stmt.cond || parent->flow.skip_first)
                break;
            FALLTHROUGH;
        default:
...
static inline bool sema_analyse_then_overwrite(SemaContext *context, Ast *statement, AstId replacement)
{
    ...
    while (1)
    {
        ...
        if (!sema_analyse_statement(context, curr_ast))
            return false;
...
static inline bool sema_check_type_case(SemaContext *context, Type *switch_type, Ast *case_stmt, Ast **cases,
                                        unsigned index)
{
    Expr *expr = case_stmt->case_stmt.expr;
    if (!sema_analyse_expr_rhs(context, type_typeid, expr, false))
        return false;
    ...
    if (expr->expr_kind == EXPR_CONST)
    {
        ...
        {
            Ast *other = cases[i];
            if (other->ast_kind != AST_CASE_STMT)
                continue;
...
static inline bool sema_check_value_case(SemaContext *context, Type *switch_type, Ast *case_stmt, Ast **cases,
                                         unsigned index, bool *if_chained, bool *max_ranged)
{
    assert(switch_type);
    Expr *expr = case_stmt->case_stmt.expr;
    ...
    if (!sema_analyse_expr_rhs(context, switch_type, expr, false))
        return false;
    if (to_expr && !sema_analyse_expr_rhs(context, switch_type, to_expr, false))
        return false;
    ...
    if (!*max_ranged && type_is_integer(expr->type) && to_const_expr != const_expr)
    {
        if (int_comp(const_expr->ixx, to_const_expr->ixx, BINARYOP_GT))
        {
            sema_error_at(extend_span_with_token(expr->span, to_expr->span),
                          ...
                          int_to_str(const_expr->ixx, 10), int_to_str(to_const_expr->ixx, 10));
            return false;
        }
    ...
    for (unsigned i = 0; i < index; i++)
    {
        ...
        if (expr_const_compare(const_expr, other_to_const, BINARYOP_LE) &&
            expr_const_compare(to_const_expr, other_const, BINARYOP_GE))
        {
...
static bool sema_analyse_switch_body(SemaContext *context, Ast *statement, SourceSpan expr_span, Type *switch_type,
                                     Ast **cases, ExprVariantSwitch *variant, Decl *var_holder)
{
    bool use_type_id = false;
    if (!type_is_comparable(switch_type))
    {
        sema_error_at(expr_span,
                      "You cannot test '%s' for equality, and only values that supports '==' for comparison can be "
                      "used in a switch.",
                      type_to_error_string(switch_type));
        return false;
    }
    ...
    for (unsigned i = 0; i < case_count; i++)
    {
        ...
        if (stmt->ast_kind == AST_CASE_STMT && body && type_switch && var_holder &&
            stmt->case_stmt.expr->expr_kind == EXPR_CONST)
        {
            if (variant->is_assign)
            {
                Type *real_type = type_get_ptr(stmt->case_stmt.expr->const_expr.typeid);
                Decl *new_var =
                    decl_new_var(variant->new_ident, variant->span,
                                 type_info_new_base(variant->is_deref ? real_type->pointer : real_type, variant->span),
                                 VARDECL_LOCAL, VISIBLE_LOCAL);
                Expr *var_result = expr_variable(var_holder);
                if (!cast(var_result, real_type))
                    return false;
                if (variant->is_deref)
                {
            ...
            else
            {
                ...
                Decl *alias =
                    decl_new_var(var_holder->name, var_holder->span,
                                 type_info_new_base(type, stmt->case_stmt.expr->span), VARDECL_LOCAL, VISIBLE_LOCAL);
                Expr *ident_converted = expr_variable(var_holder);
                if (!cast(ident_converted, type))
                    return false;
...
static bool sema_analyse_ct_switch_body(SemaContext *context, Ast *statement)
{
        ...
        case AST_CASE_STMT: {
            ...
            else
            {
                if (!sema_analyse_expr_rhs(context, type, expr, false))
                    return false;
                if (to_expr && !sema_analyse_expr_rhs(context, type, to_expr, false))
                    return false;
            }
            if (expr->expr_kind != EXPR_CONST)
            {
            ...
            for (unsigned j = 0; j < i; j++)
            {
                ...
                if (other_stmt->ast_kind == AST_DEFAULT_STMT)
                    continue;
                ExprConst *other_const = &other_stmt->case_stmt.expr->const_expr;
                ExprConst *other_const_to =
                    other_stmt->case_stmt.to_expr ? &other_stmt->case_stmt.to_expr->const_expr : other_const;
    ...
    if (!sema_analyse_statement(context, body))
        return false;
...
{
    Expr *cond = statement->ct_switch_stmt.cond;
    if (!sema_analyse_ct_expr(context, cond))
        return false;
...
static bool sema_analyse_ct_foreach_stmt(SemaContext *context, Ast *statement)
{
    ...
    if (!sema_analyse_ct_expr(context, collection))
        return false;
    ...
    if (index_name)
    {
        ...
        index->type = type_int;
        if (!sema_add_local(context, index))
            return SCOPE_POP_ERROR();
    }
    Decl *value = decl_new_var(statement->ct_foreach_stmt.value_name, statement->ct_foreach_stmt.value_span, NULL,
                               VARDECL_LOCAL_CT, VISIBLE_LOCAL);
    if (!sema_add_local(context, value))
        return SCOPE_POP_ERROR();
    // Get the body
    ...
    VECEACH(expression, i)
    {
        ...
        if (!sema_analyse_compound_stmt(context, compound_stmt))
            return SCOPE_POP_ERROR();
...
static bool sema_analyse_switch_stmt(SemaContext *context, Ast *statement)
{
    ...
    {
        if (!sema_analyse_cond(context, cond, COND_TYPE_EVALTYPE_VALUE))
            return false;
        Expr *last = VECLAST(cond->cond_expr);
    ...
    if (!sema_analyse_switch_body(context, statement, cond ? cond->span : statement->span, switch_type->canonical,
...
bool sema_analyse_assert_stmt(SemaContext *context, Ast *statement)
{
    ...
    {
        if (!sema_analyse_try_unwrap_chain(context, expr, COND_TYPE_UNWRAP_BOOL))
            return false;
    }
    else
    {
...
static inline bool sema_analyse_ct_for_stmt(SemaContext *context, Ast *statement)
{
    ...
    if ((init = statement->for_stmt.init))
    {
        ...
        VECEACH(expressions, i)
        {
            ...
            if (expr->expr_kind == EXPR_DECL)
            {
                ...
                if (!sema_analyse_var_decl_ct(context, decl))
                    goto EXIT_ERROR;
                continue;
            }
            if (!sema_analyse_expr(context, expr))
    ...
    for (int i = 0; i < MAX_MACRO_ITERATIONS; i++)
    {
        ...
        if (!sema_analyse_cond_expr(context, copy))
            goto EXIT_ERROR;
...
{
    if (sema_analyse_statement_inner(context, statement))
        return true;
...
static bool sema_analyse_contracts(SemaContext *context, AstId doc, AstId **asserts)
{
    while (doc)
    {
        ...
        case DOC_DIRECTIVE_REQUIRE:
            if (!sema_analyse_require(context, directive, asserts))
                return false;
            break;
        case DOC_DIRECTIVE_CHECKED:
            if (!sema_analyse_checked(context, directive, asserts))
                return false;
            break;
        case DOC_DIRECTIVE_PARAM:
            break;
        case DOC_DIRECTIVE_ERRORS:
            if (!sema_analyse_errors(context, directive))
                return false;
            break;
        case DOC_DIRECTIVE_ENSURE:
            if (!sema_analyse_ensure(context, directive))
                return false;
...
bool sema_analyse_function_body(SemaContext *context, Decl *func)
{
    ...
    context->active_scope = (DynamicScope){.scope_id = 0, .depth = 0, .local_decl_start = 0, .current_local = 0};
    ...
    VECEACH(params, i)
    {
        if (!sema_add_local(context, params[i]))
            return false;
    }
    AstId assert_first = 0;
    AstId *next = &assert_first;
    if (!sema_analyse_contracts(context, func->func_decl.docs, &next))
        return false;
    ...
    else
    {
        ...
        if (canonical_rtype == type_void)
        {
            ...
            if (*next_id)
            {
                ...
                // Cleanup later
                if (last->ast_kind == AST_RETURN_STMT)
                    goto SKIP_NEW_RETURN;
                span = last->span;
                next_id = &last->next;
            }
        ...
        if (!sema_analyse_compound_statement_no_scope(context, body))
            return false;
        assert(context->active_scope.depth == 1);
