...
static inline Ast *parse_if_stmt(ParseContext *c)
{
    ...
    if (tok_is(c, TOKEN_LBRACE) && (peek(c) == TOKEN_CASE || peek(c) == TOKEN_DEFAULT))
    {
        ...
        Ast **cases = NULL;
        if (!parse_switch_body(c, &cases, TOKEN_CASE, TOKEN_DEFAULT))
            return poisoned_ast;
...
static inline Ast *parse_switch_stmt(ParseContext *c)
{
    ...
    if (!parse_switch_body(c, &switch_ast->switch_stmt.cases, TOKEN_CASE, TOKEN_DEFAULT))
        return poisoned_ast;
    return switch_ast;
}
...
static inline bool parse_foreach_var(ParseContext *c, Ast *foreach)
{
    ...
    {
        foreach
            ->foreach_stmt.value_by_ref = true;
    }
...
static inline Ast *parse_continue(ParseContext *c)
{
    ...
    if (ast->contbreak_stmt.label.name)
        ast->contbreak_stmt.is_label = true;
    return ast;
}
...
static inline bool parse_ct_compound_stmt(ParseContext *c, AstId *start)
{
    ...
    {
        TokenType tok = c->tok;
        if (tok == TOKEN_CT_ELSE || tok == TOKEN_CT_ELIF || tok == TOKEN_CT_ENDIF)
            break;
...
static inline Ast *parse_ct_else_stmt(ParseContext *c)
{
    ...
    if (!parse_ct_compound_stmt(c, &ast->ct_else_stmt))
        return poisoned_ast;
    return ast;
}
...
static inline Ast *parse_ct_if_stmt(ParseContext *c, bool is_elif)
{
    ...
    if (!parse_ct_compound_stmt(c, &ast->ct_if_stmt.then))
        return poisoned_ast;
...
Ast *parse_stmt(ParseContext *c)
{
    ...
    case TOKEN_CT_ENDFOREACH:
        SEMA_ERROR_HERE("Unexpected '%s' found when expecting a statement.", token_type_to_string(c->tok));
        advance(c);
        return poisoned_ast;
    case TOKEN_RPAREN:
    case TOKEN_RBRACE:
