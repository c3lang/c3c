...
static inline bool parse_ct_compound_stmt(ParseContext *c, AstId *start, SourceSpan owner_if_span)
{
	AstId *next = start;
	while (1)
	{
		...
		if (tok == TOKEN_RBRACE || tok == TOKEN_EOF) {
			PRINT_ERROR_HERE("Expected '$endif' for '$if' started at line %u.", owner_if_span.row);
...
static inline Ast *parse_ct_if_stmt(ParseContext *c)
{
	...
	CONSUME_OR_RET(TOKEN_COLON, poisoned_ast);
	if (!parse_ct_compound_stmt(c, &ast->ct_if_stmt.then, if_span)) return poisoned_ast;

	if (tok_is(c, TOKEN_CT_ELSE))
	{
		Ast *else_ast = new_ast(AST_CT_ELSE_STMT, c->span);
		advance_and_verify(c, TOKEN_CT_ELSE);
		if (!parse_ct_compound_stmt(c, &else_ast->ct_else_stmt, if_span)) return poisoned_ast;
