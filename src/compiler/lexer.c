...
#define UPPER_CASE                                                                                                     \
    'A' : case 'B' : case 'C' : case 'D' : case 'E' : case 'F' : case 'G' : case 'H' : case 'I' : case 'J' : case 'K'  \
        : case 'L' : case 'M' : case 'N' : case 'O' : case 'P' : case 'Q' : case 'R' : case 'S' : case 'T' : case 'U'  \
        : case 'V' : case 'W' : case 'X' : case 'Y' : case 'Z'
#define NUM_CASE '0' : case '1' : case '2' : case '3' : case '4' : case '5' : case '6' : case '7' : case '8' : case '9'
...
static bool add_error_token_at(Lexer *lexer, const char *loc, uint32_t len, const char *message, ...)
{
    ...
    if (len > MAX_SOURCE_LOCATION_LEN)
        len = 0;
    SourceSpan location = {
        .file_id = lexer->file->file_id,
        .row = current_line,
...
static inline bool scan_ident(Lexer *lexer, TokenType normal, TokenType const_token, TokenType type_token, char prefix)
{
    ...
    while (1)
    {
        ...
        case NUM_CASE:
            if (!type)
                return add_error_token(lexer, "A letter must precede any digit");
        case '_':
            break;
        default:
            goto EXIT;
...
EXIT:;
    ...
    {
        if (!prefix && len == 1)
            return return_token(lexer, TOKEN_UNDERSCORE, "_");
    ...
    if (type == TOKEN_RETURN && lexer->mode == LEX_DOCS)
        type = TOKEN_IDENT;
...
static bool scan_oct(Lexer *lexer)
{
    ...
    bool is_float = false;
    if (!scan_number_suffix(lexer, &is_float))
        return false;
    if (is_float)
    {
...
static bool scan_binary(Lexer *lexer)
{
    ...
    next(lexer);
    while (is_binary_or_(peek(lexer)))
        next(lexer);
    if (is_number(peek((lexer))))
    {
        return add_error_token_at_current(
...
static inline bool scan_exponent(Lexer *lexer)
{
    ...
    if (!is_digit(c))
    {
        ...
        if (c < 31 || c > 127)
            add_error_token(lexer, "An unexpected character was found while parsing the exponent.");
...
static inline bool scan_hex(Lexer *lexer)
{
    ...
    {
        return add_error_token_at_current(
            lexer, "'0x' starts a hexadecimal number, so the next character should be 0-9, a-f or A-F.");
    }
    next(lexer);
    while (is_hex_or_(peek(lexer)))
        next(lexer);
    bool is_float = false;
    if (peek(lexer) == '.' && peek_next(lexer) != '.')
    {
        ...
        if (c == '_')
            return add_error_token_at_current(lexer,
                                              "'_' is not allowed directly after decimal point, try removing it.");
        if (is_hex(c))
            next(lexer);
        while (is_hex_or_(peek(lexer)))
            next(lexer);
 ...
 * 2. If UTF8, accept 1 UTF character, size is ushort normally, ulong on wide UTF characters, no additional characters
 * accepted.
...
static inline bool scan_char(Lexer *lexer)
{
    ...
    while (!match(lexer, '\''))
    {
        ...
        if (width > 15)
            return add_error_token_at_start(lexer, "The character literal exceeds 16 characters.");
        ...
        case 'x': {
            ...
            if (hex < 0)
            {
                return add_error_token_at(lexer, escape_begin, lexer->current - escape_begin,
                                          "Expected a two character hex value after \\x.");
            }
        ...
        case 'U': {
            ...
            if (!match(lexer, '\''))
            {
                ...
                if (peek(lexer) == '\0')
                    continue;
                return add_error_token_at_current(
                    lexer, "Character literals with '\\%c' can only contain one character, please remove this one.",
                    escape);
            }
...
static inline bool scan_hex_array(Lexer *lexer)
{
    ...
    while (1)
    {
        ...
        if (c == 0)
        {
            return add_error_token_at_current(lexer, "The hex string seems to be missing a terminating '%c'",
                                              start_char);
        }
        if (c == start_char)
            break;
        ...
        {
            return add_error_token_at_current(
                lexer, "'%c' isn't a valid hexadecimal digit, all digits should be a-z, A-Z and 0-9.", c);
        }
        return add_error_token_at_current(
    ...
    if (!return_token(lexer, TOKEN_BYTES, lexer->lexing_start))
        return false;
...
static inline bool scan_base64(Lexer *lexer)
{
    ...
    while (1)
    {
        ...
        if (c == 0)
        {
            return add_error_token_at_start(lexer, "The base64 string seems to be missing a terminating '%c'",
                                            start_char);
        }
        next(lexer);
        if (c == start_char)
        ...
        if (c == '=')
        {
            ...
            {
                return add_error_token_at_current(lexer,
                                                  "There cannot be more than 2 '=' at the end of a base64 string.", c);
            }
            end_len++;
            continue;
        }
...
static bool lexer_scan_token_inner(Lexer *lexer)
{
    ...
    case '[':
        if (match(lexer, '<'))
            return return_token(lexer, TOKEN_LVEC, "[<");
    ...
    case '.':
        ...
        {
            if (match(lexer, '.'))
                return return_token(lexer, TOKEN_ELLIPSIS, "...");
    ...
    case '!':
        if (match(lexer, '!'))
            return return_token(lexer, TOKEN_BANGBANG, "!!");
    ...
    case '^':
        return match(lexer, '=') ? return_token(lexer, TOKEN_BIT_XOR_ASSIGN, "^=")
                                 : return_token(lexer, TOKEN_BIT_XOR, "^");
    case '?':
        if (match(lexer, '?'))
            return return_token(lexer, TOKEN_QUESTQUEST, "??");
        return match(lexer, ':') ? return_token(lexer, TOKEN_ELVIS, "?:") : return_token(lexer, TOKEN_QUESTION, "?");
    case '<':
        if (match(lexer, '<'))
        {
            if (match(lexer, '='))
                return return_token(lexer, TOKEN_SHL_ASSIGN, "<<=");
    ...
    case '>':
        ...
        {
            if (match(lexer, '='))
                return return_token(lexer, TOKEN_SHR_ASSIGN, ">>=");
            return return_token(lexer, TOKEN_SHR, ">>");
        }
        if (match(lexer, ']'))
            return return_token(lexer, TOKEN_RVEC, ">]");
        return match(lexer, '=') ? return_token(lexer, TOKEN_GREATER_EQ, ">=")
                                 : return_token(lexer, TOKEN_GREATER, ">");
    case '%':
        return match(lexer, '=') ? return_token(lexer, TOKEN_MOD_ASSIGN, "%=") : return_token(lexer, TOKEN_MOD, "%");
    case '&':
        if (match(lexer, '&'))
            return return_token(lexer, TOKEN_AND, "&&");
        return match(lexer, '=') ? return_token(lexer, TOKEN_BIT_AND_ASSIGN, "&=")
                                 : return_token(lexer, TOKEN_AMP, "&");
    case '|':
        if (match(lexer, '}'))
            return return_token(lexer, TOKEN_RBRAPIPE, "|}");
        if (match(lexer, '|'))
            return return_token(lexer, TOKEN_OR, "||");
        return match(lexer, '=') ? return_token(lexer, TOKEN_BIT_OR_ASSIGN, "|=")
                                 : return_token(lexer, TOKEN_BIT_OR, "|");
    case '+':
        if (match(lexer, '+'))
            return return_token(lexer, TOKEN_PLUSPLUS, "++");
        if (match(lexer, '='))
            return return_token(lexer, TOKEN_PLUS_ASSIGN, "+=");
        return return_token(lexer, TOKEN_PLUS, "+");
    case '-':
        if (match(lexer, '>'))
            return return_token(lexer, TOKEN_ARROW, "->");
        if (match(lexer, '-'))
            return return_token(lexer, TOKEN_MINUSMINUS, "--");
        if (match(lexer, '='))
            return return_token(lexer, TOKEN_MINUS_ASSIGN, "-=");
    ...
    default:
        ...
        if (c < 0)
        {
            return add_error_token(
                lexer,
                "The 0x%x character may not be placed outside of a string or comment, did you forget a \" somewhere?",
                (uint8_t)c);
        }
        return add_error_token(
            lexer, "'%c' may not be placed outside of a string or comment, did you perhaps forget a \" somewhere?", c);
    }
}
