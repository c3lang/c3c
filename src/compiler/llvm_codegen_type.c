...
static inline LLVMTypeRef llvm_type_from_decl(GenContext *c, Decl *decl)
{
    ...
    case DECL_UNION: {
        ...
        if (vec_size(members))
        {
            ...
            LLVMTypeRef type_ref[2] = {llvm_get_type(c, rep_type->type), NULL};
            unsigned elements = 1;
            if (decl->strukt.padding)
            {
...
static inline void add_func_type_param(GenContext *context, Type *param_type, ABIArgInfo *arg_info,
                                       LLVMTypeRef **params)
{
...
LLVMTypeRef llvm_func_type(GenContext *context, FunctionPrototype *prototype)
{
    ...
    {
        add_func_type_param(context, type_get_ptr(type_lowering(prototype->ret_by_ref_type)),
                            prototype->ret_by_ref_abi_info, &params);
    }
