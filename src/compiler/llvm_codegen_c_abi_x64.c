...
{
    if (available->sse_registers < used->sse_registers)
        return false;
    if (available->int_registers < used->int_registers)
        return false;
...
static ABIArgInfo *x64_classify_argument_type(Type *type, unsigned free_int_regs, Registers *needed_registers,
                                              NamedArgument is_named);
...
static bool x64_type_is_illegal_vector(Type *type)
{
    ...
    if (size <= 8 || size > platform_target.x64.native_vector_size_avx)
        return true;
...
void x64_classify_post_merge(ByteSize size, X64Class *lo_class, X64Class *hi_class)
{
    ...
    if (*hi_class == CLASS_MEMORY)
        goto DEFAULT_TO_MEMORY;
    if (size > 16 && (*lo_class != CLASS_SSE || *hi_class != CLASS_SSEUP))
        goto DEFAULT_TO_MEMORY;
...
void x64_classify_struct_union(Type *type, ByteSize offset_base, X64Class *current, X64Class *lo_class,
                               X64Class *hi_class, NamedArgument named_arg)
{
    ByteSize size = type_size(type);
    // 64 byte max.
    if (size > 64)
    ...
    *current = CLASS_NO_CLASS;
    ...
    VECEACH(members, i)
    {
        ...
        if (size > 16 &&
            ((!is_union && size != type_size(member->type)) || size > platform_target.x64.native_vector_size_avx))
        {
            *lo_class = CLASS_MEMORY;
        ...
        *hi_class = x64_merge(*hi_class, field_hi);
        if (*lo_class == CLASS_MEMORY || *hi_class == CLASS_MEMORY)
            break;
    }
...
void x64_classify_array(Type *type, ByteSize offset_base, X64Class *current, X64Class *lo_class, X64Class *hi_class,
...
{
    if (type_size(decl->type) <= offset)
        return NULL;
    Decl **members = decl->strukt.members;
    Decl *last_match = NULL;
    VECEACH(members, i)
    {
        if (members[i]->offset > (MemberIndex)offset)
            break;
...
bool x64_bits_contain_no_user_data(Type *type, unsigned start, unsigned end)
{
    ...
    if (type->type_kind == TYPE_ARRAY)
    {
        ...
        for (unsigned i = 0; i < type->array.len; i++)
        {
            ...
            if (!x64_bits_contain_no_user_data(type->array.base, element_start, end - offset))
                return false;
        }
        // No overlap
        return true;
    ...
    if (type->type_kind == TYPE_STRUCT || type->type_kind == TYPE_UNION)
    {
        ...
        VECEACH(members, i)
        {
            ...
            if (!x64_bits_contain_no_user_data(member->type, field_start, end - offset))
                return false;
        }
        // No overlap
        return true;
...
{
    if (offset == 0 && type->type_kind == TYPE_F32)
        return true;
...
static AbiType x64_get_sse_type_at_offset(Type *type, unsigned ir_offset, Type *source_type, unsigned source_offset)
{
    ...
    if (x64_bits_contain_no_user_data(source_type, source_offset + 4, source_offset + 8))
        return abi_type_get(type_float);

    // We want to pass as <2 x float> if the LLVM IR type contains a float at
    // offset+0 and offset+4.  Walk the LLVM IR type to find out if this is the
    // case.
    if (x64_contains_float_at_offset(type, ir_offset) && x64_contains_float_at_offset(type, ir_offset + 4))
    {
...
AbiType x64_get_int_type_at_offset(Type *type, unsigned offset, Type *source_type, unsigned source_offset)
{
    ...
    case TYPE_I32:
        if (offset)
            break;
        if (x64_bits_contain_no_user_data(source_type, source_offset + type_size(type), source_offset + 8))
        {
            return abi_type_get(type);
        }
        break;
    ...
    case TYPE_ANY:
        if (offset < 8)
            return abi_type_get(type_ulong);
        if (offset < 16)
            return abi_type_get(type_voidptr);
        break;
    case TYPE_SUBARRAY:
        if (offset < 8)
            return abi_type_get(type_voidptr);
...
ABIArgInfo *x64_classify_return(Type *return_type)
{
    ...
    if (abi_type_is_valid(high_part))
        return x64_get_argument_pair_return(result_type, high_part);
...
static ABIArgInfo *x64_classify_return_type(Type *ret_type, Registers *registers, bool is_regcall)
{
    ...
    if (is_regcall && x64_type_is_structure(ret_type))
    {
        ...
        if (try_use_registers(registers, &needed_registers))
            return info;
...
void c_abi_func_create_x64(FunctionPrototype *prototype)
{
    ...
    Registers available_registers = {.int_registers = is_regcall ? 11 : 6, .sse_registers = is_regcall ? 16 : 8};

    prototype->ret_abi_info = x64_classify_return_type(prototype->abi_ret_type, &available_registers, is_regcall);
    if (abi_arg_is_indirect(prototype->ret_abi_info))
    ...
    if (prototype->ret_by_ref)
    {
        prototype->ret_by_ref_abi_info = x64_classify_parameter(type_get_ptr(type_lowering(prototype->ret_by_ref_type)),
                                                                &available_registers, is_regcall, NAMED);
    }
