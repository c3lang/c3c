...
static inline void llvm_emit_inc_dec_change(GenContext *c, bool use_mod, BEValue *addr, BEValue *after, BEValue *before,
                                            Expr *expr, int diff);
static void llvm_emit_post_unary_expr(GenContext *context, BEValue *be_value, Expr *expr);
static inline void llvm_emit_subscript_addr_with_base(GenContext *c, BEValue *result, BEValue *parent, BEValue *index,
                                                      SourceSpan loc);
...
static inline LLVMValueRef llvm_emit_insert_value(GenContext *c, LLVMValueRef agg, LLVMValueRef new_value,
                                                  unsigned index)
{
...
LLVMValueRef llvm_emit_coerce_alignment(GenContext *c, BEValue *be_value, LLVMTypeRef coerce_type,
                                        AlignSize target_alignment, AlignSize *resulting_alignment)
{
...
LLVMTypeRef llvm_const_padding_type(GenContext *c, AlignSize size)
{
    assert(size > 0);
    if (size == 1)
        return llvm_get_type(c, type_char);
...
void llvm_enter_struct_for_coerce(GenContext *c, LLVMValueRef *struct_ptr, LLVMTypeRef *type, ByteSize dest_size)
{
    while (1)
    {
        if (LLVMGetTypeKind(*type) != LLVMStructTypeKind)
            return;
        if (!LLVMCountStructElementTypes(*type))
            return;
...
LLVMValueRef llvm_emit_coerce(GenContext *c, LLVMTypeRef coerced, BEValue *value, Type *original_type)
{
    ...
    if (!llvm_value_is_addr(value) && LLVMGetTypeKind(coerced) == LLVMIntegerTypeKind &&
...
void llvm_emit_coerce_store(GenContext *c, LLVMValueRef addr, AlignSize alignment, LLVMTypeRef coerced,
                            LLVMValueRef value, LLVMTypeRef target_type)
{
...
void llvm_emit_convert_value_from_coerced(GenContext *c, BEValue *result, LLVMTypeRef coerced, LLVMValueRef value,
                                          Type *original_type)
{
...
static void llvm_emit_array_bounds_check(GenContext *c, BEValue *index, LLVMValueRef array_max_index, SourceSpan loc)
{
    ...
    {
        llvm_emit_int_comp(c, &result, index->type, index->type, index->value, llvm_get_zero(c, index->type),
                           BINARYOP_LT);
        llvm_emit_panic_if_true(c, &result, "Negative array indexing", loc);
    }

    llvm_emit_int_comp(c, &result, index->type, index->type, index->value, array_max_index, BINARYOP_GE);
                                                      ...
                                                      SourceSpan loc)
{
    ...
    case TYPE_FLEXIBLE_ARRAY: {
        AlignSize alignment;
        LLVMValueRef ptr = llvm_emit_array_gep_raw_index(c, parent->value, llvm_get_type(c, type), index->value,
    ...
    case TYPE_SUBARRAY: {
        LLVMValueRef ptr =
            llvm_emit_pointer_inbounds_gep_raw(c, llvm_get_type(c, type->array.base), parent->value, index->value);
...
static inline void gencontext_emit_subscript(GenContext *c, BEValue *value, Expr *expr)
{
    ...
    else if (parent_type_kind == TYPE_ARRAY)
    {
        ...
        needs_len =
            (active_target.feature.safe_mode && expr->expr_kind != EXPR_CONST) || expr->subscript_expr.from_back;
    }
    if (needs_len)
    {
    ...
    if (expr->subscript_expr.from_back)
    {
        assert(needs_len);
        index.value =
            LLVMBuildNUWSub(c->builder, llvm_zext_trunc(c, len.value, llvm_get_type(c, index.type)), index.value, "");
    }
...
static MemberIndex find_member_index(Decl *parent, Decl *member)
{
    VECEACH(parent->strukt.members, i)
    {
        ...
        {
            if (find_member_index(maybe_member, member) != -1)
                return (int)i;
        }
    }
    return -1;
...
static void gencontext_emit_member_addr(GenContext *c, BEValue *value, Decl *parent, Decl *member)
{
    ...
    do
    {
        ...
        case TYPE_UNION:
            llvm_value_addr(c, value);
            llvm_value_set_address(value, llvm_emit_bitcast(c, value->value, type_get_ptr(found->type)), found->type,
                                   value->alignment);
            break;
        case TYPE_STRUCT:
...
static inline void llvm_extract_bool_bit_from_array(GenContext *c, BEValue *be_value, Decl *member)
{
    ...
    // Grab the byte
    LLVMValueRef byte_ptr = llvm_emit_array_gep_raw(c, array_ptr, llvm_get_type(c, be_value->type), start_bit / 8,
                                                    be_value->alignment, &alignment);
...
static inline void llvm_extract_bitvalue_from_array(GenContext *c, BEValue *be_value, Decl *member, Decl *parent_decl)
{
    ...
    if (parent_decl->bitstruct.big_endian)
        big_endian = true;
    if (parent_decl->bitstruct.little_endian)
        big_endian = false;
    ...
    {
        AlignSize alignment;
        LLVMValueRef byte_ptr = llvm_emit_array_gep_raw(c, array_ptr, llvm_get_type(c, be_value->type), (unsigned)i,
                                                        be_value->alignment, &alignment);
        LLVMValueRef element = llvm_load(c, array_type, byte_ptr, alignment, "");
...
static inline void llvm_extract_bitvalue(GenContext *c, BEValue *be_value, Expr *parent, Decl *member)
{
    ...
    bool bswap = false;
    if (parent_decl->bitstruct.big_endian && !platform_target.big_endian)
        bswap = true;
    if (parent_decl->bitstruct.little_endian && platform_target.big_endian)
        bswap = true;
    LLVMValueRef value = llvm_load_value_store(c, be_value);
    if (bswap)
        value = llvm_emit_bswap(c, value);
...
static inline void llvm_emit_bitassign_array(GenContext *c, BEValue *result, BEValue parent, Decl *parent_decl,
                                             Decl *member)
{
    ...
    if (member_type == type_bool)
    {
        ...
        LLVMValueRef byte_ptr =
            llvm_emit_array_gep_raw(c, array_ptr, array_type, start_bit / 8, parent.alignment, &alignment);
    ...
    for (int i = start_byte; i <= end_byte; i++)
    {
        AlignSize alignment;
        LLVMValueRef byte_ptr =
            llvm_emit_array_gep_raw(c, array_ptr, array_type, (unsigned)i, parent.alignment, &alignment);
...
void llvm_emit_cast(GenContext *c, CastKind cast_kind, BEValue *value, Type *to_type, Type *from_type)
{
    ...
    case CAST_EUER:
        TODO // gencontext_emit_value_bitcast(c, value->value, to_type, from_type);
            case CAST_ERBOOL : case CAST_EUBOOL:
        {
            BEValue zero;
    ...
    case CAST_SABOOL:
        ...
        if (llvm_value_is_addr(value))
        {
            value->value = llvm_emit_struct_gep_raw(c, value->value, llvm_get_type(c, value->type), 1, value->alignment,
                                                    &value->alignment);
        }
        else
        {
...
static inline void gencontext_emit_cast_expr(GenContext *context, BEValue *be_value, Expr *expr)
{
    ...
    llvm_emit_cast(context, expr->cast_expr.kind, be_value, expr->type, exprtype(expr->cast_expr.expr));
}

static LLVMValueRef llvm_recursive_set_value(GenContext *c, DesignatorElement **current_element_ptr,
                                             LLVMValueRef parent, DesignatorElement **last_element_ptr, Expr *value)
{
...
static void llvm_emit_inititialize_reference_const(GenContext *c, BEValue *ref, ConstInitializer *const_init)
{
    ...
    case CONST_INIT_ARRAY_FULL: {
        ...
        {
            AlignSize alignment;
            LLVMValueRef array_pointer =
                llvm_emit_array_gep_raw(c, array_ref, array_type_llvm, (unsigned)i, ref->alignment, &alignment);
            BEValue value;
    ...
    case CONST_INIT_ARRAY: {
        ...
        VECEACH(elements, i)
        {
            ...
            AlignSize alignment;
            LLVMValueRef array_pointer = llvm_emit_array_gep_raw(c, array_ref, array_type_llvm, (unsigned)element_index,
...
static void llvm_emit_initialize_designated_const_range(GenContext *c, BEValue *ref, AlignSize offset,
                                                        DesignatorElement **current, DesignatorElement **last,
                                                        Expr *expr, BEValue *emitted_value)
{
                                            ...
                                            DesignatorElement **last, Expr *expr, BEValue *emitted_value)
{
    ...
    case DESIGNATOR_ARRAY: {
        ...
        AlignSize alignment;
        LLVMValueRef ptr = llvm_emit_array_gep_raw(c, ref->value, llvm_get_type(c, ref->type), (unsigned)curr->index,
                                                   ref->alignment, &alignment);
...
static inline void llvm_emit_initialize_reference_designated(GenContext *c, BEValue *ref, Expr *expr)
{
    ...
    VECEACH(elements, i)
    {
        ...
        DesignatorElement **last_element =
            designator->designator_expr.path + vec_size(designator->designator_expr.path) - 1;
        llvm_emit_initialize_designated(c, ref, 0, designator->designator_expr.path, last_element,
...
LLVMValueRef llvm_emit_const_bitstruct_array(GenContext *c, ConstInitializer *initializer)
{
    ...
    for (MemberIndex i = 0; i < count; i++)
    {
        ...
        for (int j = start_byte; j <= end_byte; j++)
        {
            ...
            if (member_type_bitsize > 8)
                to_or = LLVMConstTrunc(to_or, c->byte_type);
...
LLVMValueRef llvm_emit_const_bitstruct(GenContext *c, ConstInitializer *initializer)
{
    ...
    if (initializer->kind == CONST_INIT_ZERO)
        return llvm_get_zero(c, base_type);
                                            ...
                                            Expr *expr, int diff)
{
    ...
    case TYPE_POINTER: {
        ...
        LLVMValueRef add = LLVMConstInt(diff < 0 ? llvm_get_type(c, type_ichar) : llvm_get_type(c, type_char),
                                        (unsigned long long)diff, diff < 0);
    ...
    case ALL_INTS: {
        ...
        after_value = diff > 0 ? llvm_emit_add_int(c, type, value.value, diff_value, expr->span)
    ...
    case TYPE_VECTOR: {
        ...
        if (is_integer)
        {
            after_value = diff > 0 ? llvm_emit_add_int(c, type, value.value, val, expr->span)
    ...
    if (after)
        llvm_value_set(after, after_value, addr->type);
}
...
static void gencontext_emit_unary_expr(GenContext *c, BEValue *value, Expr *expr)
{
    ...
    case UNARYOP_NEG:
        ...
        if (active_target.feature.trap_on_wrap)
        {
            ...
            LLVMValueRef call_res = llvm_emit_call_intrinsic(c, intrinsic_id.ssub_overflow, &type_to_use, 1, args, 2);
...
void llvm_emit_len_for_expr(GenContext *c, BEValue *be_value, BEValue *expr_to_len)
{
    ...
    case TYPE_SUBARRAY:
        ...
        else
        {
            ...
            AlignSize alignment;
            LLVMValueRef len_addr =
                llvm_emit_struct_gep_raw(c, expr_to_len->value, subarray_type, 1, expr_to_len->alignment, &alignment);
...
static void llvm_emit_trap_negative(GenContext *c, Expr *expr, LLVMValueRef value, const char *error)
{
    if (!active_target.feature.safe_mode)
        return;
    if (type_is_integer_unsigned(expr->type->canonical))
        return;
...
static void llvm_emit_trap_zero(GenContext *c, Type *type, LLVMValueRef value, const char *error, SourceSpan loc)
{
    ...
    LLVMValueRef ok = type_is_integer(type) ? LLVMBuildICmp(c->builder, LLVMIntEQ, value, zero, "zero")
                                            : LLVMBuildFCmp(c->builder, LLVMRealUEQ, value, zero, "zero");
...
static void llvm_emit_slice_values(GenContext *c, Expr *slice, BEValue *parent_ref, BEValue *start_ref,
                                   BEValue *end_ref)
{
...
static void gencontext_emit_slice(GenContext *c, BEValue *be_value, Expr *expr)
{
    ...
    // Calculate the size
    LLVMValueRef size = LLVMBuildSub(
        c->builder, LLVMBuildAdd(c->builder, end.value, llvm_const_int(c, start.type, 1), ""), start.value, "size");
    ...
    case TYPE_ARRAY: {
        ...
        // Move pointer
        AlignSize alignment;
        start_pointer = llvm_emit_array_gep_raw_index(c, parent.value, llvm_get_type(c, parent.type), start.value,
                                                      type_abi_alignment(parent.type), &alignment);
        start_pointer = llvm_emit_bitcast(c, start_pointer, pointer_type);
        break;
    }
    case TYPE_SUBARRAY:
        start_pointer =
            llvm_emit_pointer_inbounds_gep_raw(c, llvm_get_type(c, parent.type->array.base), parent.value, start.value);
        break;
    case TYPE_POINTER:
        start_pointer =
            llvm_emit_pointer_inbounds_gep_raw(c, llvm_get_pointee_type(c, parent.type), parent.value, start.value);
        break;
    case TYPE_FLEXIBLE_ARRAY:
    case TYPE_VECTOR:
...
static void llvm_emit_slice_assign(GenContext *c, BEValue *be_value, Expr *expr)
{
    ...
    if (LLVMIsConstant(start.value) && LLVMIsConstant(end.value))
    {
        ...
        uint64_t end_val =
            signed_end ? (uint64_t)LLVMConstIntGetSExtValue(end.value) : (uint64_t)LLVMConstIntGetZExtValue(end.value);
...
void llvm_emit_int_comp(GenContext *c, BEValue *result, Type *lhs_type, Type *rhs_type, LLVMValueRef lhs_value,
                        LLVMValueRef rhs_value, BinaryOp binary_op)
{
                                              ...
                                              SourceSpan loc)
{
    if (active_target.feature.trap_on_wrap)
    {
        ...
        unsigned operation = type_is_integer_unsigned(type) ? intrinsic_id.umul_overflow : intrinsic_id.smul_overflow;
        LLVMValueRef call_res = llvm_emit_call_intrinsic(c, operation, types, 1, args, 2);
...
static void llvm_emit_subarray_comp(GenContext *c, BEValue *be_value, BEValue *lhs, BEValue *rhs, BinaryOp binary_op)
{
    ...
    llvm_value_set_address_abi_aligned(
        &lhs_to_compare, llvm_emit_pointer_inbounds_gep_raw(c, llvm_base_type, lhs_value.value, current_index.value),
        array_base_type);
    llvm_value_set_address_abi_aligned(
        &rhs_to_compare, llvm_emit_pointer_inbounds_gep_raw(c, llvm_base_type, rhs_value.value, current_index.value),
        array_base_type);
...
void gencontext_emit_binary(GenContext *c, BEValue *be_value, Expr *expr, BEValue *lhs_loaded, BinaryOp binary_op)
{
    ...
    case BINARYOP_SUB:
        if (lhs_type->type_kind == TYPE_POINTER)
        {
            ...
            if (lhs_type == rhs_type)
            {
                ...
                val = LLVMBuildExactSDiv(c->builder, val,
                                         llvm_const_int(c, type_iptrdiff, type_abi_alignment(lhs_type->pointer)), "");
                break;
            }
    ...
    case BINARYOP_DIV:
        ...
        val = type_is_unsigned(lhs_type) ? LLVMBuildUDiv(c->builder, lhs_value, rhs_value, "udiv")
                                         : LLVMBuildSDiv(c->builder, lhs_value, rhs_value, "sdiv");
        break;
    case BINARYOP_MOD:
        llvm_emit_trap_zero(c, rhs_type, rhs_value, "% by zero", expr->span);
        val = type_is_unsigned(lhs_type) ? LLVMBuildURem(c->builder, lhs_value, rhs_value, "umod")
    ...
    case BINARYOP_SHR:
        ...
        val = type_is_unsigned(lhs_type) ? LLVMBuildLShr(c->builder, lhs_value, rhs_value, "lshr")
...
static void llvm_emit_post_unary_expr(GenContext *context, BEValue *be_value, Expr *expr)
{
    ...
    llvm_emit_post_inc_dec(context, be_value, expr->unary_expr.expr, expr->unary_expr.operator== UNARYOP_INC ? 1 : - 1,
                           false);
}
...
void llvm_emit_try_assign_try_catch(GenContext *c, bool is_try, BEValue *be_value, BEValue *var_addr,
                                    BEValue *catch_addr, Expr *rhs)
{
...
static inline void llvm_emit_typeofany(GenContext *c, BEValue *be_value, Expr *expr)
{
    ...
    if (llvm_value_is_addr(be_value))
    {
        ...
        LLVMValueRef pointer_addr = llvm_emit_struct_gep_raw(c, be_value->value, llvm_get_type(c, type_any), 1,
                                                             be_value->alignment, &alignment);
...
static inline void llvm_emit_force_unwrap_expr(GenContext *c, BEValue *be_value, Expr *expr)
{
    ...
    if (llvm_emit_check_block_branch(c))
    {
        ...
        llvm_emit_debug_output(c, "Runtime error force unwrap!", file->name, c->cur_func_decl->extname,
                               loc.row ? loc.row : 1);
...
static void llvm_emit_vector_assign_expr(GenContext *c, BEValue *be_value, Expr *expr)
{
    ...
    LLVMValueRef new_value =
        LLVMBuildInsertElement(c->builder, vector_value, llvm_load_value_store(c, be_value), index_val, "elemset");
...
void gencontext_emit_ternary_expr(GenContext *c, BEValue *value, Expr *expr)
{
    if (!expr->ternary_expr.then_expr)
        return gencontext_emit_elvis_expr(c, value, expr);
...
static void llvm_emit_const_expr(GenContext *c, BEValue *be_value, Expr *expr)
{
    ...
    case CONST_BYTES:
        ...
        {
            LLVMValueRef global_name = LLVMAddGlobal(
                c->module, LLVMArrayType(llvm_get_type(c, type_char), expr->const_expr.bytes.len), ".bytes");
            llvm_set_private_linkage(global_name);
            LLVMSetGlobalConstant(global_name, 1);

            LLVMSetInitializer(global_name, LLVMConstStringInContext(c->context, expr->const_expr.bytes.ptr,
                                                                     expr->const_expr.bytes.len, 1));
    ...
    case CONST_STRING: {
        ...
        if (c->builder || !is_array)
        {
            ...
            if (type_is_char_array(expr->type) && type->array.len > size)
                size = type->array.len;
            LLVMValueRef global_name =
                LLVMAddGlobal(c->module, LLVMArrayType(llvm_get_type(c, type_char), size), ".str");
            llvm_set_private_linkage(global_name);
            LLVMSetUnnamedAddress(global_name, LLVMGlobalUnnamedAddr);
            LLVMSetGlobalConstant(global_name, 1);
            LLVMValueRef string =
                LLVMConstStringInContext(c->context, expr->const_expr.string.chars, expr->const_expr.string.len, 0);
            if (size > strlen + 1)
            {
        ...
        if (array_len <= size)
        {
            string = LLVMConstStringInContext(c->context, expr->const_expr.string.chars, array_len, array_len < size);
        }
        else
        {
            char *buffer = ccalloc(1, array_len);
            memcpy(buffer, expr->const_expr.string.chars, expr->const_expr.string.len);
            string = LLVMConstStringInContext(c->context, buffer, array_len, true);
        }
    ...
    case CONST_ENUM: {
        ...
        llvm_value_set(be_value, LLVMBuildPtrToInt(c->builder, decl->backend_ref, llvm_get_type(c, type_anyerr), ""),
                       type_anyerr);
        return;
    }
    default:
...
static void llvm_expand_type_to_args(GenContext *context, Type *param_type, LLVMValueRef expand_ptr,
                                     LLVMValueRef **values, AlignSize alignment);

static void llvm_expand_array_to_args(GenContext *c, Type *param_type, LLVMValueRef expand_ptr, LLVMValueRef **values,
                                      AlignSize alignment)
{
    ...
    for (ByteSize i = 0; i < param_type->array.len; i++)
    {
        ...
        LLVMValueRef element_ptr =
            llvm_emit_array_gep_raw(c, expand_ptr, array_type, (unsigned)i, alignment, &load_align);
        llvm_expand_type_to_args(c, param_type->array.base, element_ptr, values, load_align);
    }
}

static void llvm_expand_struct_to_args(GenContext *context, Type *param_type, LLVMValueRef expand_ptr,
                                       LLVMValueRef **values, AlignSize alignment)
{
    ...
    VECEACH(members, i)
    {
        ...
        AlignSize load_align;
        LLVMValueRef member_ptr = llvm_emit_struct_gep_raw(context, expand_ptr, llvm_get_type(context, param_type), i,
                                                           alignment, &load_align);
...
REDO:
    ...
    case ALL_FLOATS:
    case TYPE_POINTER:
        vec_add(*values, llvm_load(context, llvm_get_type(context, param_type), expand_ptr, alignment, "loadexpanded"));
        return;
    case TYPE_TYPEDEF:
...
void llvm_emit_struct_member_ref(GenContext *c, BEValue *struct_ref, BEValue *member_ref, unsigned member_id)
{
    ...
    AlignSize align;
    LLVMValueRef ptr = llvm_emit_struct_gep_raw(c, struct_ref->value, llvm_get_type(c, struct_ref->type), member_id,
                                                struct_ref->alignment, &align);
...
{
    *alignment =
        type_min_alignment((AlignSize)LLVMOffsetOfElement(context->target_data, struct_type, index), struct_alignment);
...
LLVMValueRef llvm_emit_array_gep_raw_index(GenContext *c, LLVMValueRef ptr, LLVMTypeRef array_type, LLVMValueRef index,
                                           AlignSize array_alignment, AlignSize *alignment)
{
...
LLVMValueRef llvm_emit_array_gep_raw(GenContext *c, LLVMValueRef ptr, LLVMTypeRef array_type, unsigned index,
                                     AlignSize array_alignment, AlignSize *alignment)
{
    return llvm_emit_array_gep_raw_index(c, ptr, array_type, llvm_const_int(c, type_usize, index), array_alignment,
                                         alignment);
}

LLVMValueRef llvm_emit_array_load(GenContext *c, LLVMValueRef ptr, LLVMTypeRef array_type, unsigned index,
                                  AlignSize array_alignment)
{
    AlignSize alignment;
    LLVMValueRef element_ptr = llvm_emit_array_gep_raw_index(c, ptr, array_type, llvm_const_int(c, type_usize, index),
...
LLVMValueRef llvm_emit_pointer_inbounds_gep_raw(GenContext *c, LLVMTypeRef pointee_type, LLVMValueRef ptr,
                                                LLVMValueRef offset)
{
    if (LLVMIsConstant(ptr) && LLVMIsConstant(offset))
    {
...
void llvm_emit_subarray_len(GenContext *c, BEValue *subarray, BEValue *len)
{
    ...
    AlignSize alignment = 0;
    LLVMValueRef len_addr = llvm_emit_struct_gep_raw(c, subarray->value, llvm_get_type(c, subarray->type), 1,
                                                     subarray->alignment, &alignment);
...
void llvm_emit_subarray_pointer(GenContext *c, BEValue *value, BEValue *pointer)
{
    ...
    if (value->kind == BE_ADDRESS)
    {
        AlignSize alignment;
        LLVMValueRef ptr =
            llvm_emit_struct_gep_raw(c, value->value, llvm_get_type(c, value->type), 0, value->alignment, &alignment);
...
void llvm_value_struct_gep(GenContext *c, BEValue *element, BEValue *struct_pointer, unsigned index)
{
    ...
    AlignSize alignment;
    LLVMValueRef ref = llvm_emit_struct_gep_raw(c, struct_pointer->value, llvm_get_type(c, struct_pointer->type),
                                                (unsigned)actual_index, struct_pointer->alignment, &alignment);
...
void llvm_emit_parameter(GenContext *c, LLVMValueRef **args, ABIArgInfo *info, BEValue *be_value, Type *type)
{
    ...
    case ABI_ARG_DIRECT_PAIR: {
        ...
        LLVMValueRef cast =
            llvm_emit_coerce_alignment(c, be_value, struct_type, llvm_abi_alignment(c, struct_type), &struct_align);
        // Get the lo value.
    ...
    case ABI_ARG_EXPAND_COERCE: {
        ...
        AlignSize alignment;
        LLVMValueRef temp =
            llvm_emit_coerce_alignment(c, be_value, coerce_type, llvm_abi_alignment(c, coerce_type), &alignment);

        AlignSize align;
        LLVMValueRef gep_first =
            llvm_emit_struct_gep_raw(c, temp, coerce_type, info->coerce_expand.lo_index, alignment, &align);
        vec_add(*args, llvm_load(c, llvm_abi_type(c, info->coerce_expand.lo), gep_first, align, ""));
        if (abi_type_is_valid(info->coerce_expand.hi))
        {
            LLVMValueRef gep_second =
                llvm_emit_struct_gep_raw(c, temp, coerce_type, info->coerce_expand.hi_index, alignment, &align);
...
static void llvm_emit_unpacked_variadic_arg(GenContext *c, Expr *expr, BEValue *subarray)
{
    ...
    case TYPE_POINTER:
        ...
        llvm_store_value_raw(c, &pointer_addr,
...
void llvm_add_call_attributes(GenContext *c, LLVMValueRef call_value, int start_index, int count, Type **types,
                              ABIArgInfo **infos)
{
    for (unsigned i = 0; i < count; i++)
    {
        ...
        case ABI_ARG_INDIRECT:
            ...
            {
                llvm_attribute_add_call_type(c, call_value, attribute_id.byval, (int)i + start_index,
...
void llvm_emit_call_expr(GenContext *c, BEValue *result_value, Expr *expr)
{
    ...
    *result_value = (BEValue){.kind = BE_VALUE, .value = NULL};
    ...
    case ABI_ARG_INDIRECT:
        ...
        // 6b. Return true is indirect, in this case we allocate a local, using the desired alignment on the caller
        // side.
    ...
    if (prototype->ret_by_ref)
    {
        ...
        llvm_value_set(&synthetic_return_param, llvm_emit_alloca_aligned(c, actual_return_type, "retparam"),
                       type_get_ptr(actual_return_type));
        // 7c. Emit it as a parameter as a pointer (will implicitly add it to the value list)
        llvm_emit_parameter(c, &values, prototype->ret_by_ref_abi_info, &synthetic_return_param,
    ...
    if (prototype->variadic == VARIADIC_TYPED || prototype->variadic == VARIADIC_ANY)
    {
        ...
        llvm_value_set_address_abi_aligned(&subarray, llvm_emit_alloca_aligned(c, vararg_param, "vararg"),
                                           vararg_param);
        ...
        else
        {
            ...
            for (unsigned i = non_variadic_params; i < arguments; i++)
            {
                ...
                LLVMValueRef slot = llvm_emit_array_gep_raw(c, array_ref, llvm_array_type, i - non_variadic_params,
                                                            alignment, &store_alignment);
    ...
    if (prototype->abi_varargs)
    {
        llvm_add_call_attributes(c, call_value, 1 + non_variadic_params, vec_size(prototype->varargs),
                                 prototype->varargs, prototype->abi_varargs);
    }
...
void llvm_emit_try_unwrap(GenContext *c, BEValue *value, Expr *expr)
{
    if (!expr->try_unwrap_expr.failable)
    {
        ...
        LLVMValueRef errv =
            llvm_load(c, llvm_get_type(c, type_anyerr), fail_ref, type_abi_alignment(type_anyerr), "load.err");
...
static inline void llvm_emit_argv_to_subarray(GenContext *c, BEValue *value, Expr *expr)
{
    ...
    LLVMValueRef pointer_value =
        llvm_load(c, llvm_get_ptr_type(c, type_char), pointer_to_arg, type_abi_alignment(type_get_ptr(type_char)), "");
