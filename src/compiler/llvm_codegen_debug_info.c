...
static inline LLVMMetadataRef llvm_get_debug_member(GenContext *c, Type *type, const char *name, unsigned offset,
                                                    SourceSpan *loc, LLVMMetadataRef scope, LLVMDIFlags flags);
static inline LLVMMetadataRef llvm_get_debug_struct(GenContext *c, Type *type, const char *external_name,
                                                    LLVMMetadataRef *elements, unsigned element_count, SourceSpan *loc,
                                                    LLVMMetadataRef scope, LLVMDIFlags flags);
static LLVMMetadataRef llvm_debug_forward_comp(GenContext *c, Type *type, const char *external_name, SourceSpan *loc,
                                               LLVMMetadataRef scope, LLVMDIFlags flags);
                                                    ...
                                                    LLVMMetadataRef scope, LLVMDIFlags flags)
{
    ...
    LLVMMetadataRef real = LLVMDIBuilderCreateStructType(c->debug.builder, scope, external_name_len ? type->name : "",
                                                         external_name_len ? strlen(type->name) : 0, file, row,
                                                         type_size(type) * 8, (uint32_t)(type_abi_alignment(type) * 8),
                                                         flags, NULL, elements, element_count, c->debug.runtime_version,
                                                         NULL, // VTable
...
static inline LLVMMetadataRef llvm_get_debug_member(GenContext *c, Type *type, const char *name, unsigned offset,
                                                    SourceSpan *loc, LLVMMetadataRef scope, LLVMDIFlags flags)
{
    return LLVMDIBuilderCreateMemberType(c->debug.builder, scope, name, strlen(name), loc ? c->debug.file : NULL,
                                         loc ? loc->row : 0, type_size(type) * 8,
                                         (uint32_t)(type_abi_alignment(type) * 8), offset * 8, flags,
                                         llvm_get_debug_type_internal(c, type, scope));
}
...
void llvm_emit_debug_global_var(GenContext *c, Decl *global)
{
    ...
    global->var.backend_debug_ref = LLVMDIBuilderCreateGlobalVariableExpression(
        c->debug.builder, c->debug.file, global->name, strlen(global->name), global->extname, strlen(global->extname),
        c->debug.file, loc.row ? loc.row : 1, llvm_get_debug_type(c, global->type), global->visibility == VISIBLE_LOCAL,
        LLVMDIBuilderCreateExpression(c->debug.builder, NULL, 0), NULL, global->alignment);
}
...
void llvm_emit_debug_function(GenContext *c, Decl *decl)
{
    ...
    if (!row)
        row = 1;
    c->debug.function = LLVMDIBuilderCreateFunction(
        c->debug.builder, c->debug.file, decl->name, strlen(decl->name), decl->extname, strlen(decl->extname),
        c->debug.file, row, llvm_get_debug_type(c, decl->type), decl->visibility == VISIBLE_LOCAL, true, row, flags,
...
void llvm_emit_debug_local_var(GenContext *c, Decl *decl)
{
    ...
    LLVMMetadataRef var = LLVMDIBuilderCreateAutoVariable(
        c->debug.builder, c->debug.function, name, strlen(name), c->debug.file, row, llvm_get_debug_type(c, decl->type),
        active_target.optimization_level != OPTIMIZATION_NONE, LLVMDIFlagZero, decl->alignment);
    decl->var.backend_debug_ref = var;

    LLVMMetadataRef inline_at = NULL;
    LLVMDIBuilderInsertDeclareAtEnd(
        c->debug.builder, decl->backend_ref, var, LLVMDIBuilderCreateExpression(c->debug.builder, NULL, 0),
        LLVMDIBuilderCreateDebugLocation(c->context, row, col, c->debug.function, inline_at),
...
void llvm_emit_debug_parameter(GenContext *c, Decl *parameter, unsigned index)
{
    ...
    parameter->var.backend_debug_ref = LLVMDIBuilderCreateParameterVariable(
        c->debug.builder, c->debug.function, name, strlen(name), index + 1, c->debug.file, row,
        llvm_get_debug_type(c, parameter->type), always_preserve, LLVMDIFlagZero);
    LLVMMetadataRef inline_at = NULL;

    LLVMDIBuilderInsertDeclareAtEnd(
        c->debug.builder, parameter->backend_ref, parameter->var.backend_debug_ref,
...
void llvm_emit_debug_location(GenContext *context, SourceSpan location)
{
    ...
    if (oldloc && context->last_emitted_loc.a == location.a)
        return;
    ...
    LLVMMetadataRef loc =
        LLVMDIBuilderCreateDebugLocation(context->context, row ? row : 1, col ? col : 1, scope, /* inlined at */ 0);
                                               ...
                                               LLVMMetadataRef scope, LLVMDIFlags flags)
{
    ...
    return LLVMDIBuilderCreateReplaceableCompositeType(
        c->debug.builder, id_counter++, type->name, strlen(type->name), scope, c->debug.file, row, 1 /* version TODO */,
        type_size(type) * 8, type_abi_alignment(type) * 8, flags, external_name, strlen(external_name));
}
void llvm_debug_push_lexical_scope(GenContext *context, SourceSpan location)
{
    ...
    unsigned row = location.row;
    unsigned col = location.col;
    LLVMMetadataRef block = LLVMDIBuilderCreateLexicalBlock(context->debug.builder, scope, context->debug.file,
                                                            row ? row : 1, col ? col : 1);
...
static LLVMMetadataRef llvm_debug_simple_type(GenContext *context, Type *type, int dwarf_code)
{
    return type->backend_debug_type =
               LLVMDIBuilderCreateBasicType(context->debug.builder, type->name, strlen(type->name),
                                            type->builtin.bitsize, (LLVMDWARFTypeEncoding)dwarf_code, 0);
}
...
static LLVMMetadataRef llvm_debug_pointer_type(GenContext *c, Type *type)
{
    ...
    LLVMMetadataRef real =
        LLVMDIBuilderCreatePointerType(c->debug.builder, llvm_get_debug_type(c, type->pointer), type_size(type) * 8,
                                       type_abi_alignment(type) * 8, 0, type->name, strlen(type->name));
    if (type->backend_debug_type)
    {
        LLVMMetadataReplaceAllUsesWith(type->backend_debug_type, real);
    }
...
static LLVMMetadataRef llvm_debug_enum_type(GenContext *c, Type *type, LLVMMetadataRef scope)
{
    ...
    VECEACH(enums, i)
    {
        ...
        LLVMMetadataRef debug_info = LLVMDIBuilderCreateEnumerator(
            c->debug.builder, enum_constant->name, strlen(enum_constant->name), (int64_t)val, is_unsigned);
        vec_add(elements, debug_info);
    }

    unsigned row = decl->span.row;
    LLVMMetadataRef real = LLVMDIBuilderCreateEnumerationType(
        c->debug.builder, scope, type->decl->name, strlen(type->decl->name), c->debug.file, row ? row : 1,
        type_size(type) * 8, type_abi_alignment(type) * 8, elements, vec_size(elements),
...
static LLVMMetadataRef llvm_debug_structlike_type(GenContext *c, Type *type, LLVMMetadataRef scope)
{
    ...
    VECEACH(members, i)
    {
        ...
        LLVMMetadataRef debug_info = llvm_get_debug_member(c, member->type, member->name ? member->name : "",
                                                           member->offset, &member->span, forward, LLVMDIFlagZero);
        vec_add(elements, debug_info);
    }
    ...
    {
        unsigned row = decl->span.row;
        real = LLVMDIBuilderCreateUnionType(
            c->debug.builder, scope, type->decl->name ? type->decl->name : "",
            type->decl->name ? strlen(type->decl->name) : 0, c->debug.file, row ? row : 1, type_size(type) * 8,
            type_abi_alignment(type) * 8, LLVMDIFlagZero, elements, vec_size(members), c->debug.runtime_version,
            type->decl->name ? decl->extname : "", type->decl->name ? strlen(decl->extname) : 0);
        LLVMMetadataReplaceAllUsesWith(forward, real);
        return real;
    }
    return llvm_get_debug_struct(c, type, decl->name ? decl->extname : "", elements, vec_size(elements), &decl->span,
                                 scope, LLVMDIFlagZero);
}
...
static LLVMMetadataRef llvm_debug_any_type(GenContext *c, Type *type)
{
    ...
    LLVMMetadataRef elements[2] = {llvm_get_debug_member(c, type_void, "ptr", 0, NULL, forward, LLVMDIFlagZero),
...
{
    return LLVMDIBuilderCreateTypedef(c->debug.builder, llvm_get_debug_type(c, type_iptr->canonical), type->name,
                                      strlen(type->name), NULL, 0, NULL, 0);
}
...
static LLVMMetadataRef llvm_debug_array_type(GenContext *c, Type *type)
{
    ...
    LLVMMetadataRef real =
        LLVMDIBuilderCreateArrayType(c->debug.builder, type_size(type) * 8, type_abi_alignment(current_type) * 8,
                                     llvm_get_debug_type(c, current_type), ranges, vec_size(ranges));
...
static LLVMMetadataRef llvm_debug_typedef_type(GenContext *c, Type *type)
{
    ...
    if (!decl)
    {
        return LLVMDIBuilderCreateTypedef(c->debug.builder, llvm_get_debug_type(c, type->canonical), type->name,
                                          strlen(type->name), NULL, 0, NULL, 0);
    }
    ...
    unsigned row = decl->span.row;
    LLVMMetadataRef real = LLVMDIBuilderCreateTypedef(c->debug.builder, llvm_get_debug_type(c, original_type),
                                                      decl->name, strlen(decl->name), c->debug.file, row ? row : 1,
...
static LLVMMetadataRef llvm_debug_vector_type(GenContext *c, Type *type)
{
    ...
    return LLVMDIBuilderCreateVectorType(c->debug.builder, type_size(type) * 8, type_abi_alignment(current_type) * 8,
                                         llvm_get_debug_type(c, current_type), ranges, vec_size(ranges));
}
...
static LLVMMetadataRef llvm_debug_func_type(GenContext *c, Type *type)
{
    ...
    return LLVMDIBuilderCreateSubroutineType(c->debug.builder, c->debug.file, buffer, vec_size(buffer), 0);
}
