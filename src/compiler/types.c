...
Type *type_int_signed_by_bitsize(unsigned bitsize)
{
    ...
    case 128:
        return type_i128;
    default:
        FATAL_ERROR("Illegal bitsize %d", bitsize);
    }
}
Type *type_int_unsigned_by_bitsize(unsigned bytesize)
{
    ...
    case 128:
        return type_u128;
    default:
        FATAL_ERROR("Illegal bitsize %d", bytesize);
    }
}
...
RETRY:
    ...
    case TYPE_VECTOR: {
        ...
        if (max_alignment_vector && alignment > max_alignment_vector)
            alignment = max_alignment_vector;
        return alignment;
    }
    case TYPE_VOID:
    case TYPE_FAILABLE_ANY:
...
{
    if (canonical)
        failable_type = failable_type->canonical;
...
static inline bool array_structurally_equivalent_to_struct(Type *array, Type *type)
{
    ...
    if (len == 1 && type_is_structurally_equivalent(base, type))
        return true;
    ...
    VECEACH(members, i)
    {
        if (!type_is_structurally_equivalent(array_base, members[i]->type))
            return false;
...
bool type_is_structurally_equivalent(Type *type1, Type *type2)
{
    ...
    if (type2->type_kind == TYPE_UNION)
    {
        ...
        VECEACH(members, i)
        {
            if (type_is_structurally_equivalent(type1, members[i]->type))
                return true;
        }
    ...
    if (type1->type_kind == TYPE_UNION)
    {
        ...
        VECEACH(members, i)
        {
            if (type_is_structurally_equivalent(members[i]->type, type2))
                return true;
        }
        return false;
    }
    ...
    VECEACH(members, i)
    {
        if (!type_is_structurally_equivalent(members[i]->type, other_members[i]->type))
            return false;
        if (members[i]->offset != other_members[i]->offset)
            return false;
    }
    return true;
}
...
{
    if (canonical)
        element_type = element_type->canonical;
    ...
    for (int i = ARRAY_OFFSET; i < entries; i++)
    {
        ...
        if (vector)
        {
            if (ptr_vec->type_kind != TYPE_VECTOR)
                continue;
            if (ptr_vec->array.len == len)
                return ptr_vec;
        }
        else
        {
...
static void type_create(const char *name, Type *location, TypeKind kind, unsigned bitsize, unsigned align,
                        unsigned pref_align)
{
    assert(align);
...
{
    *location = (Type){.type_kind = TYPE_TYPEDEF, .name = name, .canonical = canonical};
    global_context_add_type(location);
}
...
static int compare_function(FunctionSignature *sig, FunctionPrototype *proto)
{
    ...
    if (param_count != vec_size(other_params))
        return -1;
    if (type_infoptr(sig->returntype)->type->canonical != proto->rtype->canonical)
        return -1;
    VECEACH(params, i)
    {
        Decl *param = params[i];
        Type *other_param = other_params[i];
        if (param->type->canonical != other_param->canonical)
            return -1;
    }
    return 0;
}
...
bool type_is_subtype(Type *type, Type *possible_subtype)
{
    ...
    while (1)
    {
        if (type == possible_subtype)
            return true;
        if (type->type_kind != possible_subtype->type_kind)
            return false;
        if (type->decl->decl_kind != DECL_STRUCT)
            return false;

        if (!possible_subtype->decl->strukt.members)
            return false;

        Decl *first_element = possible_subtype->decl->strukt.members[0];

        if (first_element->decl_kind != DECL_VAR)
            return false;
...
Type *type_find_max_num_type(Type *num_type, Type *other_num)
{
    ...
    if (other_kind < TYPE_INTEGER_FIRST || other_kind > TYPE_FLOAT_LAST)
        return NULL;
...
Type *type_find_max_type(Type *type, Type *other)
{
    ...
    case TYPE_FLEXIBLE_ARRAY:
        return NULL;
    case ALL_INTS:
        if (other->type_kind == TYPE_DISTINCT && type_underlying_is_numeric(other))
            return other;
        if (other->type_kind == TYPE_ENUM)
            return type_find_max_type(type, other->decl->enums.type_info->type->canonical);
        return type_find_max_num_type(type, other);
    case ALL_FLOATS:
        if (other->type_kind == TYPE_DISTINCT && type_is_float(other->decl->distinct_decl.base_type))
            return other;
        return type_find_max_num_type(type, other);
    case TYPE_POINTER:
        if (type->pointer->type_kind == TYPE_ARRAY)
        {
            ...
            if (other->type_kind == TYPE_SUBARRAY && array_base == other->array.base->canonical)
            {
                return other;
            }
        }
        ...
        if (other->type_kind == TYPE_POINTER)
            other = type_decay_array_pointer(other);
    ...
    case TYPE_ENUM:
        ...
        return NULL;
    case TYPE_ERRTYPE:
        if (other->type_kind == TYPE_ERRTYPE)
            return type_anyerr;
        return NULL;
    case TYPE_ANYERR:
        return type_anyerr;
    ...
    case TYPE_STRUCT:
    case TYPE_UNTYPED_LIST:
        TODO case TYPE_TYPEDEF : UNREACHABLE case TYPE_DISTINCT : return NULL;
    case TYPE_ARRAY:
        return NULL;
    case TYPE_SUBARRAY:
        TODO case TYPE_VECTOR :
...
Type *type_find_common_ancestor(Type *left, Type *right)
{
    ...
    if (left == right)
        return left;
    if (left->type_kind != right->type_kind)
        return NULL;
    ...
    {
        if (!left->decl->strukt.members)
            break;
        Decl *first_element = left->decl->strukt.members[0];
        if (first_element->decl_kind != DECL_VAR)
            break;
        if (first_element->type->canonical == right)
            return right;
