...
Type *type_abi_find_single_struct_element(Type *type)
{
    ...
    if (found && type_size(type) != type_size(found))
        found = NULL;
    return found;
}
...
bool type_homogenous_aggregate_small_enough(Type *type, unsigned members)
{
    ...
    {
    case ABI_PPC64_SVR4:
        if (type->type_kind == TYPE_F128 && platform_target.float128)
            return members <= 8;
...
RETRY:
        ...
        *elements = 0;
        {
            ...
            VECEACH(members, i)
            {
                ...
                if (!type_is_homogenous_aggregate(member_type, base, &member_members))
                    return false;
            ...
            // Ensure no padding
            if (type_size(*base) * *elements != type_size(type))
                return false;
        }
        goto TYPECHECK;
    ...
    case TYPE_ARRAY:
        ...
        if (!type_is_homogenous_aggregate(type->array.base, base, elements))
            return false;
    ...
    *elements = 1;
    ...
    if (!type_is_homogenous_base_type(type))
        return false;
    ...
    if (((*base)->type_kind == TYPE_VECTOR) != (type->type_kind == TYPE_VECTOR))
        return false;

    // Size does not match => failure
    if (type_size(*base) != type_size(type))
        return false;
...
AlignSize type_alloca_alignment(Type *type)
{
    ...
    if (align < 16 && (platform_target.abi == ABI_X64 || platform_target.abi == ABI_WIN64))
    {
        ...
        if (type->type_kind == TYPE_ARRAY && type_size(type) >= 16)
            return 16;
    }
    return align;
}
