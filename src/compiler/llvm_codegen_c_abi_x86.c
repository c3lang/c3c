...
static bool type_is_union_struct_with_simd_vector(Type *type)
{
    ...
    VECEACH(members, i)
    {
        ...
        if (type_is_simd_vector(member_type))
            return true;
        if (type_is_union_struct_with_simd_vector(type))
            return true;
    }
    return false;
}
...
static unsigned x86_stack_alignment(Type *type, unsigned alignment)
{
    ...
    if (alignment < MIN_ABI_STACK_ALIGN)
        return 0;
...
static bool x86_should_return_type_in_reg(Type *type)
{
    ...
    if (!platform_target.x86.is_mcu_api && !is_power_of_two(size))
        return false;
    ...
    {
        Type *member_type = members[i]->type;
        if (!x86_should_return_type_in_reg(member_type))
            return false;
    }
    return true;
}
...
ABIArgInfo *x86_classify_return(CallABI call, Regs *regs, Type *type)
{
    ...
    if (type_is_integer(type) && type_size(type) > 8)
        return create_indirect_return_x86(type, regs);
...
static inline bool x86_is_mmxtype(Type *type)
{
    ...
    if (type->type_kind != TYPE_VECTOR)
        return false;
    if (type_size(type->array.base) >= 8)
        return false;
    if (!type_is_integer(type->array.base))
        return false;
...
static bool x86_try_use_free_regs(Regs *regs, Type *type)
{
    ...
    if (!platform_target.x86.use_soft_float && type_is_float(type))
        return false;
...
static ABIArgInfo *x86_classify_argument(CallABI call, Regs *regs, Type *type)
{
    ...
    if ((is_vec_call || is_reg_call) && type_is_homogenous_aggregate(type, &base, &elements))
    {
...
void c_abi_func_create_x86(FunctionPrototype *prototype)
{
    ...
    if (prototype->ret_by_ref)
    {
        prototype->ret_by_ref_abi_info =
            x86_classify_argument(prototype->call_abi, &regs, type_get_ptr(type_lowering(prototype->ret_by_ref_type)));
    }
