...
static ABIArgInfo *riscv_coerce_and_expand_fpcc_struct(AbiType field1, unsigned field1_offset, AbiType field2,
                                                       unsigned field2_offset)
{
...
static bool riscv_detect_fpcc_struct_internal(Type *type, unsigned current_offset, AbiType *field1_ref,
                                              unsigned *field1_offset, AbiType *field2_ref, unsigned *field2_offset)
{
    ...
    if (is_int || is_float)
    {
        if (is_int && size > platform_target.riscv.xlen)
            return false;
        // Can't be eligible if larger than the FP registers. Half precision isn't
        // currently supported on RISC-V and the ABI hasn't been confirmed, so
        // default to the integer ABI in that case.
        if (is_float && (size > flen || size < 4))
            return false;
        // Can't be eligible if an integer type was already found (int+int pairs
        // are not eligible).
        if (is_int && abi_type_is_valid(*field1_ref) && abi_type_is_integer(*field1_ref))
            return false;
    ...
    if (type->type_kind == TYPE_ARRAY)
    {
        ...
        {
            if (!riscv_detect_fpcc_struct_internal(element_type, current_offset, field1_ref, field1_offset, field2_ref,
                                                   field2_offset))
                return false;
    ...
    if (type_is_structlike(type))
    {
        ...
        VECEACH(members, i)
        {
            ...
            if (!riscv_detect_fpcc_struct_internal(member->type, (unsigned)(current_offset + member->offset),
...
static bool riscv_detect_fpcc_struct(Type *type, AbiType *field1_ref, unsigned *field1_offset, AbiType *field2_ref,
                                     unsigned *field2_offset, unsigned *gprs, unsigned *fprs)
{
    ...
    *fprs = 0;
    ...
    bool is_candidate =
        riscv_detect_fpcc_struct_internal(type, 0, field1_ref, field1_offset, field2_ref, field2_offset);

    // Not really a candidate if we have a single int but no float.
    if (abi_type_is_valid(*field1_ref) && !abi_type_is_valid(*field2_ref) && !abi_type_is_float(*field1_ref))
        return false;
    if (!is_candidate)
        return false;
    if (abi_type_is_valid(*field1_ref))
...
static ABIArgInfo *riscv_classify_argument_type(Type *type, bool is_fixed, unsigned *gprs, unsigned *fprs)
{
    ...
    if (is_fixed && platform_target.riscv.flen && type->type_kind == TYPE_STRUCT)
    {
        ...
        bool is_candidate =
            riscv_detect_fpcc_struct(type, &field1, &offset1, &field2, &offset2, &needed_gprs, &needed_fprs);
...
void c_abi_func_create_riscv(FunctionPrototype *prototype)
{
    ...
    {
        prototype->ret_by_ref_abi_info = riscv_classify_argument_type(
            type_get_ptr(type_lowering(prototype->ret_by_ref_type)), true, &arg_gprs_left, &arg_fprs_left);
    }
