...
{
    if (prototype->tb_prototype)
        return prototype->tb_prototype;
    ...
    {
        add_func_type_param(type_get_ptr(type_lowering(prototype->ret_by_ref_type)), prototype->ret_by_ref_abi_info,
                            &params);
    }
    ...
    TB_FunctionPrototype *tb_proto = tb_prototype_create(module, tilde_call_convention(prototype->call_abi),
...
void tinybackend_store_value(TbContext *c, TBEValue *destination, TBEValue *value)
{
    ...
    case TBE_ADDRESS: {
        ...
        TB_Register copy_size = tb_inst_uint(c->f, size <= UINT32_MAX ? TB_TYPE_I32 : TB_TYPE_I64, size);
...
static void tinybackend_emit_const_expr(TbContext *c, TBEValue *value, Expr *expr)
{
    ...
    case CONST_INTEGER: {
        ...
        case TYPE_U128:
            ...
            break;
        default:
            reg = type_kind_is_signed(expr->const_expr.ixx.type) ? tb_inst_sint(c->f, dt, i.low)
                                                                 : tb_inst_uint(c->f, dt, i.low);
            break;
        }
...
TB_Register tilde_value_rvalue_get(TbContext *c, TBEValue *value)
{
    ...
    return tilde_load_aligned(c, tbtype(value->type), value->reg,
    ...
    case BINARYOP_SUB:
        if (lhs_type->type_kind == TYPE_POINTER)
        {
            ...
            if (lhs_type == rhs_type)
            {
                ...
                val = LLVMBuildExactSDiv(c->builder, val, llvm_const_int(c, type_iptrdiff,
            type_abi_alignment(lhs_type->pointer)), ""); break;
            }
    ...
    VECEACH(module->units, j)
    {
        ...
        VECEACH(unit->functions, i)
        {
            ...
            if (decl->func_decl.body)
                tilde_emit_function_body(c, decl);
        }
    }
