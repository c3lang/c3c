...
static void header_print_type(FILE *file, Type *type)
{
    ...
    case CT_TYPES:
        UNREACHABLE
    case TYPE_BITSTRUCT:
        TODO case TYPE_FAILABLE : case TYPE_FAILABLE_ANY :
            // If this is reachable then we are not doing the proper lowering.
            UNREACHABLE case TYPE_VOID : OUTPUT("void");
        return;
    case TYPE_BOOL:
        OUTPUT("bool");
        return;
    ...
    case TYPE_FLEXIBLE_ARRAY:
        TODO case TYPE_FUNC : TODO case TYPE_STRUCT : OUTPUT("struct %s__", type->decl->extname);
        return;
    case TYPE_UNION:
...
static void header_gen_decl(FILE *file, int indent, Decl *decl)
{
    ...
    case DECL_FUNC:
    case DECL_BITSTRUCT:
        TODO case DECL_TYPEDEF : case DECL_DISTINCT : TODO case DECL_STRUCT : header_gen_struct(file, indent, decl);
        return;
    case DECL_UNION:
...
{
    TODO CompilationUnit *unit = module->units[0];
