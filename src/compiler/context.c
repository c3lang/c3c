                                               ...
                                               bool is_private)
{
    ...
    else
    {
        ...
        {
            SEMA_ERROR(module_name, "Module name here '%s' did not match actual module '%s'.", module_name->module,
                       module->name->module);
            return false;
        }
    }
...
bool context_set_module_from_filename(ParseContext *context)
{
    ...
    const char *module_name =
        symtab_add(global_context.scratch_buffer, global_context.scratch_buffer_len,
                   fnv1a(global_context.scratch_buffer, (uint32_t)global_context.scratch_buffer_len), &type);

    if (type != TOKEN_IDENT)
    {
        sema_error(context,
                   "Generating a filename from the file '%s' resulted in a name that is a reserved keyword, "
...
{
    if (!decl->module || decl->module == unit->module || !decl->extname)
        return;
    VECEACH(unit->external_symbol_list, i)
    {
        if (decl == unit->external_symbol_list[i])
            return;
    }
...
void unit_register_global_decl(CompilationUnit *unit, Decl *decl)
{
    ...
    Decl *old;
    if ((old = htable_set(&unit->local_symbols, decl->name, decl)))
        goto ERR;
    if ((old = htable_set(&unit->module->symbols, decl->name, decl)))
        goto ERR;
    return;
ERR:
    assert(decl != old);
