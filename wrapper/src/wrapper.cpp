...
#define LINK_SIG                                                                                                       \
    bool link(llvm::ArrayRef<const char *> args, llvm::raw_ostream &stdoutOS, llvm::raw_ostream &stderrOS,             \
              bool exitEarly, bool disableOutput);
#define CALL_ARGS arg_vector, output, output_err, false, false
#else
#define LINK_SIG                                                                                                       \
    bool link(llvm::ArrayRef<const char *> args, bool canExitEarly, llvm::raw_ostream &stdoutOS,                       \
...
static bool llvm_link(ObjFormat format, const char **args, int arg_count, const char **error_string)
{
    ...
    {
    case ELF:
        if (lld::elf::link(CALL_ARGS))
            return true;
        break;
    case MACHO:
#if LLVM_VERSION_MAJOR > 13
        if (lld::macho::link(CALL_ARGS))
            return true;
#else
        if (lld::mach_o::link(CALL_ARGS))
            return true;
#endif
        break;
    case WASM:
        if (lld::wasm::link(CALL_ARGS))
            return true;
        break;
    case COFF:
        if (lld::coff::link(CALL_ARGS))
            return true;
        break;
    case MINGW:
        if (lld::mingw::link(CALL_ARGS))
            return true;
        break;
    default:
        exit(-1);
...
extern "C"
{
    ...
    LLVMValueRef LLVMConstGEP2(LLVMTypeRef Ty, LLVMValueRef ConstantVal, LLVMValueRef *ConstantIndices,
                               unsigned NumIndices)
    {
        llvm::ArrayRef<llvm::Constant *> IdxList(llvm::unwrap<llvm::Constant>(ConstantIndices, NumIndices), NumIndices);
        llvm::Constant *Val = llvm::unwrap<llvm::Constant>(ConstantVal);
        return wrap(llvm::ConstantExpr::getGetElementPtr(llvm::unwrap(Ty), Val, IdxList));
    }

    LLVMValueRef LLVMConstInBoundsGEP2(LLVMTypeRef Ty, LLVMValueRef ConstantVal, LLVMValueRef *ConstantIndices,
                                       unsigned NumIndices)
    {
        llvm::ArrayRef<llvm::Constant *> IdxList(llvm::unwrap<llvm::Constant>(ConstantIndices, NumIndices), NumIndices);
        llvm::Constant *Val = llvm::unwrap<llvm::Constant>(ConstantVal);
